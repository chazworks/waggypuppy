...
$profile_help = '<p>'
    . __('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using waggypuppy.')
    . '</p>'
    .
    '<p>'
    . __('You can change your password, turn on keyboard shortcuts, change the color scheme of your waggypuppy administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.')
    . '</p>'
    .
    '<p>'
    . __('You can select the language you wish to use while using the waggypuppy administration screen without affecting the language site visitors see.')
    . '</p>'
    .
    '<p>'
...
get_current_screen()->set_help_sidebar(
    ...
    .
    '<p>'
    . __('<a href="https://wp.org/documentation/article/users-your-profile-screen/">Documentation on User Profiles</a>')
    . '</p>'
    .
    '<p>'
...
if (IS_PROFILE_PAGE && isset($_GET['newuseremail']) && $current_user->ID) {
    ...
    if ($new_email && hash_equals($new_email['hash'], $_GET['newuseremail'])) {
        ...
        if (is_multisite()
            && $wpdb->get_var($wpdb->prepare("SELECT user_login FROM {$wpdb->signups} WHERE user_login = %s",
            ...
            >
                ...
                <table class="form-table" role="presentation">
                    <tr class="user-user-login-wrap">
                        ...
                        <td><input type="text" name="user_login" id="user_login"
                                   value="<?php echo esc_attr($profile_user->user_login); ?>" readonly="readonly"
                    ...
                    <?php if (is_multisite()
                        ...
                        <tr class="user-super-admin-wrap">
                            ...
                            <td>
                                <?php if (0 !== strcasecmp($profile_user->user_email, get_site_option('admin_email'))
                    ...
                    <tr class="user-last-name-wrap">
                        ...
                        <td><input type="text" name="last_name" id="last_name"
                                   value="<?php echo esc_attr($profile_user->last_name); ?>" class="regular-text"/></td>
                    </tr>
                    ...
                    <tr class="user-nickname-wrap">
                        ...
                        <td><input type="text" name="nickname" id="nickname"
                                   value="<?php echo esc_attr($profile_user->nickname); ?>" class="regular-text"/></td>
                    </tr>
                        ...
                        <td>
                            <select name="display_name" id="display_name">
                                ...
                                if (!empty($profile_user->first_name) && !empty($profile_user->last_name)) {
                                    $public_display['display_firstlast'] = $profile_user->first_name
                                        . ' '
                                        . $profile_user->last_name;
                                    $public_display['display_lastfirst'] = $profile_user->last_name
                                        . ' '
                                ...
                                if (!in_array($profile_user->display_name, $public_display,
                                    ...
                                    $public_display = ['display_displayname' => $profile_user->display_name]
                                        + $public_display;
                                }
                ...
                <table class="form-table" role="presentation">
                    <tr class="user-email-wrap">
                        <th><label for="email"><?php _e('Email'); ?> <span
                                    class="description"><?php _e('(required)'); ?></span></label></th>
                        <td>
                            <input type="email" name="email" id="email" aria-describedby="email-description"
                                   value="<?php echo esc_attr($profile_user->user_email); ?>" class="regular-text ltr"/>
                            ...
                            if ($new_email && $new_email['newemail'] !== $current_user->user_email
                                ...
                                $pending_change_message .= sprintf(
                                    ' <a href="%1$s">%2$s</a>',
                                    esc_url(wp_nonce_url(self_admin_url('profile.php?dismiss='
                                        . $current_user->ID
                                        . '_new_email'), 'dismiss-' . $current_user->ID . '_new_email')),
                                    __('Cancel'),
                                );
                                wp_admin_notice(
                    ...
                    <tr class="user-url-wrap">
                        ...
                        <td><input type="url" name="url" id="url"
                                   value="<?php echo esc_attr($profile_user->user_url); ?>" class="regular-text code"/>
                        </td>
                    </tr>
                        ...
                        <tr class="user-<?php echo $name; ?>-wrap">
                            ...
                            <td>
                                <input type="text" name="<?php echo $name; ?>" id="<?php echo $name; ?>"
                                       value="<?php echo esc_attr($profile_user->$name); ?>" class="regular-text"/>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </table>
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr class="user-pass2-wrap hide-if-js">
                        ...
                        <td>
                            <input type="password" name="pass2" id="pass2" class="regular-text" value=""
                                   autocomplete="new-password" spellcheck="false" aria-describedby="pass2-desc"/>
                            <?php if (IS_PROFILE_PAGE) : ?>
                            ...
                            <?php if (!wp_is_site_protected_by_basic_auth('front')) : ?>
                                <div class="create-application-password form-wrap">
                                    <div class="form-field">
                                        <label
                                            for="new_application_password_name"><?php _e('New Application Password Name'); ?></label>
                                        <input type="text" size="30" id="new_application_password_name"
                                               name="new_application_password_name" class="input" aria-required="true"
                                    ...
                                    <button type="button" name="do_new_application_password"
                ...
                <?php if (count($profile_user->caps) > count($profile_user->roles)
                    && (true
        ...
        <div class="notice notice-success is-dismissible new-application-password-notice" role="alert">
            <p class="application-password-display">
                ...
                <button type="button" class="button copy-button"
                        data-clipboard-text="{{ data.password }}"><?php _e('Copy'); ?></button>
