<?php
/**
 * waggypuppy Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
switch ($action) {
    ...
    case 'move-down-menu-item':
        ...
        if (is_nav_menu_item($menu_item_id)) {
            $menus = isset($_REQUEST['menu']) ? [(int)$_REQUEST['menu']]
                : wp_get_object_terms($menu_item_id, 'nav_menu', ['fields' => 'ids']);
            ...
            if (!is_wp_error($menus) && !empty($menus[0])) {
                ...
                if (!empty($dbids_to_orders[$menu_item_id])
                    ...
                    if (!empty($menu_item_data['menu_item_parent'])
                        ...
                        if (!is_wp_error($parent_object)) {
                            ...
                            if (!empty($dbids_to_orders[$parent_db_id])
                                ...
                                */
                                ...
                                $_possible_parent_id = (int)get_post_meta($orders_to_dbids[$dbids_to_orders[$parent_db_id]
                                - 1], '_menu_item_menu_item_parent', true);
                        ...
                        $menu_item_data['menu_item_parent'] = (int)$orders_to_dbids[$dbids_to_orders[$menu_item_id]
                        - 1];
...
if (!$locations_screen) : // Main tab.
    ...
    $menu_management = '<p>'
        ...
        . '</p>';
    $menu_management .= '<ul><li>'
        . __('To edit an existing menu, <strong>choose a menu from the dropdown and click Select</strong>')
        . '</li>';
    $menu_management .= '<li>'
        . __('If you have not yet created any menus, <strong>click the &#8217;create a new menu&#8217; link</strong> to get started')
        . '</li></ul>';
    $menu_management .= '<p>'
        . __('You can assign theme locations to individual menus by <strong>selecting the desired settings</strong> at the bottom of the menu editor. To assign menus to all theme locations at once, <strong>visit the Manage Locations tab</strong> at the top of the screen.')
        . '</p>';
    ...
    $editing_menus = '<p>'
        ...
        . '</p>';
    $editing_menus .= '<p>'
        . __('<strong>Clicking the arrow to the right of any menu item</strong> in the editor will reveal a standard group of settings. Additional settings such as link target, CSS classes, link relationships, and link descriptions can be enabled and disabled via the Screen Options tab.')
        . '</p>';
    $editing_menus .= '<ul><li>'
        . __('Add one or several items at once by <strong>selecting the checkbox next to each item and clicking Add to Menu</strong>')
        . '</li>';
    $editing_menus .= '<li>'
        . __('To add a custom link, <strong>expand the Custom Links section, enter a URL and link text, and click Add to Menu</strong>')
        . '</li>';
    $editing_menus .= '<li>'
        . __('To reorganize menu items, <strong>drag and drop items with your mouse or use your keyboard</strong>. Drag or move a menu item a little to the right to make it a submenu')
        . '</li>';
    $editing_menus .= '<li>'
        . __('Delete a menu item by <strong>expanding it and clicking the Remove link</strong>')
        . '</li></ul>';
    ...
    $locations_overview = '<p>'
        . __('This screen is used for globally assigning menus to locations defined by your theme.')
        . '</p>';
    $locations_overview .= '<ul><li>'
        . __('To assign menus to one or more theme locations, <strong>select a menu from each location&#8217;s dropdown</strong>. When you are finished, <strong>click Save Changes</strong>')
        . '</li>';
    $locations_overview .= '<li>'
        . __('To edit a menu currently assigned to a theme location, <strong>click the adjacent &#8217;Edit&#8217; link</strong>')
        . '</li>';
    $locations_overview .= '<li>'
        . __('To add a new menu instead of assigning an existing one, <strong>click the &#8217;Use new menu&#8217; link</strong>. Your new menu will be automatically assigned to that theme location')
        . '</li></ul>';
...
get_current_screen()->set_help_sidebar(
    ...
    .
    '<p>'
    . __('<a href="https://wp.org/documentation/article/appearance-menus-screen/">Documentation on Menus</a>')
    . '</p>'
    .
    '<p>'
                            ...
                            <td class="menu-location-menus">
                                <select name="menu-locations[<?php echo $_location; ?>]"
                                    ...
                                    foreach ($nav_menus as $menu) :
                                        $data_orig = '';
                                        $selected = isset($menu_locations[$_location])
                                            && $menu_locations[$_location]
                                            === $menu->term_id;
                                ...
                                <div class="locations-row-links">
                                    <?php if (isset($menu_locations[$_location])
                                        && 0
                                        !== $menu_locations[$_location]) : ?>
                ...
                <form method="get" action="<?php echo esc_url(admin_url('nav-menus.php')); ?>">
                    ...
                    <select name="menu" id="select-menu-to-edit">
                        ...
                        <?php foreach ((array)$nav_menus as $_nav_menu) : ?>
                            <option
                                value="<?php echo esc_attr($_nav_menu->term_id); ?>" <?php selected($_nav_menu->term_id,
                            ...
                            <div id="nav-menu-header">
                                <div class="major-publishing-actions wp-clearfix">
                                    ...
                                    <div class="publishing-action">
                                        <?php submit_button(empty($nav_menu_selected_id) ? __('Create Menu')
                                            : __('Save Menu'), 'primary large menu-save', 'save_menu', false,
                                            ['id' => 'save_menu_header']); ?>
                                        ...
                                        <?php if (!$add_new_screen) : ?>
                                            <div id="nav-menu-bulk-actions-top"
                                                ...
                                                <label class="bulk-select-button" for="bulk-select-switcher-top">
                                                    ...
                                                    <span
                                                        class="bulk-select-button-label"><?php _e('Bulk Select'); ?></span>
                                                </label>
                                            </div>
                                        <?php endif; ?>
                                    ...
                                    <div class="menu-settings" <?php echo $no_menus_style; ?>>
                                        ...
                                        <?php if (current_theme_supports('menus')) : ?>
                                            ...
                                            <fieldset class="menu-settings-group menu-theme-locations">
                                                <legend
                                                    class="menu-settings-group-name howto"><?php _e('Display location'); ?></legend>
                                                <?php
                                                foreach ($locations as $location => $description) :
                                                    $checked = false;
                                                    ...
                                                    <div class="menu-settings-input checkbox-input">
                                                        ...
                                                        <?php if (!empty($menu_locations[$location])
