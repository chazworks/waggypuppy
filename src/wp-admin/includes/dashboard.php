	...
	<p class="community-events-footer">
		<?php
			printf(
				...
				'https://make.__VAR_WP.org/community/meetups-landing-page',
				__( 'Meetups' ),
				/* translators: Hidden accessibility text. */
		...
		<?php
			printf(
				'<a href="%1$s" target="_blank">%2$s <span class="screen-reader-text"> %3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>',
				/* translators: If a Rosetta site exists (e.g. https://es.__VAR_WP.org/news/), then use that. Otherwise, leave untranslated. */
				esc_url( _x( 'https://__VAR_WP.org/news/', 'Events and News dashboard widget' ) ),
				__( 'News' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			);
		...
		<# if ( data.events.length <= 2 ) { #>
			<li class="event-none">
				...
				printf(
					...
					__( 'https://make.__VAR_WP.org/community/organize-event-landing-page/' )
				);
				?>
			</li>
		<# } #>
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'https://make.__VAR_WP.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_primary_link', __( 'https://__VAR_WP.org/news/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_primary_feed', __( 'https://__VAR_WP.org/news/feed/' ) ),
		...
		'planet' => array(
			...
			'url'          => apply_filters(
				...
				__( 'https://planet.__VAR_WP.org/feed/' )
			),
...
function wp_dashboard_browser_nag() {
	 ...
	 */
	echo apply_filters( 'browse-happy-notice', $notice, $response ); // phpcs:ignore __VAR_WP_TC.NamingConventions.ValidHookName.UseUnderscores
}
...
function wp_check_browser_version() {
	...
	if ( false === $response ) {
		$url     = 'http://api.__VAR_WP.org/core/browse-happy/1.1/';
		$options = array(
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<a href="<?php echo esc_url( __( 'https://__VAR_WP.org/documentation/article/block-themes/' ) ); ?>"><?php _e( 'Learn about block themes' ); ?></a>
			<?php endif; ?>
			</div>
		</div>
	</div>
