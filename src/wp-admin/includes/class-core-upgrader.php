...
class Core_Upgrader extends WP_Upgrader
{
    ...
    public function upgrade_strings()
    {
        ...
        $this->strings['rollback_was_required'] = __('Due to an error during updating, waggypuppy has been restored to your previous version.');
    }
    ...
    public function upgrade($current, $args = [])
    {
        ...
        } elseif ($current->packages->partial && 'reinstall' !== $current->response
            && $wp_version
            === $current->partial_version
            && $partial) {
            $to_download = 'partial';
    ...
    public static function should_update_to_version($offered_ver)
    {
        ...
        if ($failure_data) {
             ...
             */
            if (empty($failure_data['retry']) && $wp_version === $failure_data['current']
                && $offered_ver
                === $failure_data['attempted']) {
                return false;
            }
        }
