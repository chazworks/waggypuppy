...
// <!-- --------------------------------------------------------------------------------------- -->
    ...
    function parselisting($line)
    {
        ...
        if ($is_windows
            && preg_match("/([0-9]{2})-([0-9]{2})-([0-9]{2}) +([0-9]{2}):([0-9]{2})(AM|PM) +([0-9]+|<DIR>) +(.+)/",
                $line, $lucifer)) {
            $b = [];
            if ($lucifer[3] < 70) {
                $lucifer[3] += 2000;
    ...
    function SetType($mode = FTP_AUTOASCII)
    {
        ...
        $this->_type = $mode;
        $this->SendMSG("Transfer type: " . ($this->_type == FTP_BINARY
                ? "binary"
                : ($this->_type == FTP_ASCII ? "ASCII"
                    : "auto ASCII")));
        return true;
    }
    ...
    function connect($server = null)
    {
        ...
        $this->SendMSG("Connected to remote host \""
            . $this->_fullhost
            . ":"
            . $this->_port
        ...
        foreach ($list as $el) {
            if ($el["type"] == "d") {
                if (!$this->mget($remote . "/" . $el["name"], $local . "/" . $el["name"], $continious)) {
                    $this->PushError("mget", "cannot copy folder", "Cannot copy remote folder \""
                        . $remote
                        . "/"
                        . $el["name"]
                        . "\" to local \""
                ...
                if (!$this->get($remote . "/" . $el["name"], $local . "/" . $el["name"])) {
                    $this->PushError("mget", "cannot copy file", "Cannot copy remote file \""
                        . $remote
                        . "/"
                        . $el["name"]
                        . "\" to local \""
