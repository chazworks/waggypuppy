...
switch ($wp_list_table->current_action()) {
    ...
    default:
        ...
        $location = apply_filters("handle_bulk_actions-{$screen}", $location, $wp_list_table->current_action(), $tags); // phpcs:ignore __VAR_WP.NamingConventions.ValidHookName.UseUnderscores
        break;
}
...
if ('category' === $taxonomy || 'link_category' === $taxonomy || 'post_tag' === $taxonomy) {
    ...
    if ('category' === $taxonomy) {
        $help .= '<p>' . __('<a href="https://wp.org/documentation/article/posts-categories-screen/">Documentation on Categories</a>') . '</p>';
    } elseif ('link_category' === $taxonomy) {
        $help .= '<p>' . __('<a href="https://codex.wp.org/Links_Link_Categories_Screen">Documentation on Link Categories</a>') . '</p>';
    } else {
        $help .= '<p>' . __('<a href="https://wp.org/documentation/article/posts-tags-screen/">Documentation on Tags</a>') . '</p>';
    }
 ...
 */
do_action("after-{$taxonomy}-table", $taxonomy);  // phpcs:ignore __VAR_WP.NamingConventions.ValidHookName.UseUnderscores
