...
function render_block_core_post_navigation_link($attributes, $content)
{
    ...
    // Set default values.
    $format = '%link';
    $link = 'next' === $navigation_type ? _x('Next', 'label for next post link')
    ...
    if (isset($attributes['showTitle']) && $attributes['showTitle']) {
        ...
        if (!$attributes['linkLabel']) {
            ...
            if ($label) {
                $link = '<span class="post-navigation-link__label">'
                    . wp_kses_post($label)
                 ...
                 */
                $label = 'next' === $navigation_type ? _x('Next:', 'label before the title of the next post')
                    : _x('Previous:', 'label before the title of the previous post');
                $link = sprintf(
    ...
    if (isset($attributes['arrow']) && 'none' !== $attributes['arrow'] && isset($arrow_map[$attributes['arrow']])) {
        ...
        if ('next' === $navigation_type) {
            $format = '%link<span class="wp-block-post-navigation-link__arrow-next is-arrow-'
                . $attributes['arrow']
                . '" aria-hidden="true">'
                . $arrow
                . '</span>';
        } else {
            $format = '<span class="wp-block-post-navigation-link__arrow-previous is-arrow-'
                . $attributes['arrow']
                . '" aria-hidden="true">'
                . $arrow
                . '</span>%link';
     ...
     *
     * @see https://developer.wp.org/reference/functions/get_previous_post_link/
     * @see https://developer.wp.org/reference/functions/get_next_post_link/
     */
