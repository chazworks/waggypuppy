...
function block_core_page_list_render_nested_page_list(
    ...
    foreach ((array)$nested_pages as $page) {
        ...
        if (((0 < $depth && !$is_nested) || $is_nested)
            && isset($colors['overlay_css_classes'], $colors['overlay_inline_styles'])) {
        ...
        if (isset($page['children']) && $is_navigation_child && $open_submenus_on_click) {
            ...
            $markup .= '<a class="wp-block-pages-list__item__link'
                . esc_attr($navigation_child_content_class)
                . '" href="'
                . esc_url($page['link'])
                . '"'
        ...
        if (isset($page['children'])) {
            ...
            $markup .= block_core_page_list_render_nested_page_list($open_submenus_on_click, $show_submenu_icons,
                $is_navigation_child, $page['children'], $is_nested, $active_page_ancestor_ids, $colors, $depth + 1);
            $markup .= '</ul>';
        }
        $markup .= '</li>';
    }
...
function render_block_core_page_list($attributes, $content, $block)
{
    ...
    $open_submenus_on_click = array_key_exists('openSubmenusOnClick', $block->context)
        ? $block->context['openSubmenusOnClick'] : false;

    $show_submenu_icons = array_key_exists('showSubmenuIcon', $block->context) ? $block->context['showSubmenuIcon']
        : false;

    $wrapper_markup = $is_nested ? '%2$s' : '<ul %1$s>%2$s</ul>';

    $items_markup = block_core_page_list_render_nested_page_list($open_submenus_on_click, $show_submenu_icons,
        $is_navigation_child, $nested_pages, $is_nested, $active_page_ancestor_ids, $colors);
