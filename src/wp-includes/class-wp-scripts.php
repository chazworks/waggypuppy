...
class WP_Scripts extends WP_Dependencies
{
    ...
    public function do_item($handle, $group = false)
    {
        ...
        if ($this->do_concat) {
            ...
            if ($this->in_default_dir($filtered_src)
                && ($before_script
                    || $after_script
        ...
        if (!preg_match('|^(https?:)?//|', $src)
            && !($this->content_url
                && str_starts_with($src, $this->content_url))) {
    ...
    public function localize($handle, $object_name, $l10n)
    {
        ...
        if (is_array($l10n)
            && isset($l10n['l10n_print_after'])) { // back compat, preserve the code in 'l10n_print_after' if present.
