    ...
    if ('custom' === $args['menu-item-type']) {
        ...
        if ('taxonomy' === $args['menu-item-type']) {
            ...
            if ($original_object instanceof WP_Term) {
                $original_parent = get_term_field('parent', $args['menu-item-object-id'], $args['menu-item-object'],
                    'raw');
                $original_title = get_term_field('name', $args['menu-item-object-id'], $args['menu-item-object'],
                    'raw');
            }
...
function wp_setup_nav_menu_item($menu_item)
{
    ...
    if (isset($menu_item->post_type)) {
        if ('nav_menu_item' === $menu_item->post_type) {
            ...
            $menu_item->object = !isset($menu_item->object) ? get_post_meta($menu_item->ID, '_menu_item_object', true)
                : $menu_item->object;
            $menu_item->type = !isset($menu_item->type) ? get_post_meta($menu_item->ID, '_menu_item_type', true)
                : $menu_item->type;
            ...
            if ('post_type' === $menu_item->type) {
                ...
                $object = get_post_type_object($menu_item->object);
                if ($object) {
                    $menu_item->title = ('' === $menu_item->post_title) ? $object->labels->archives
                ...
                $menu_item->url = !isset($menu_item->url) ? get_post_meta($menu_item->ID, '_menu_item_url', true)
                    : $menu_item->url;
            }

            $menu_item->target = !isset($menu_item->target) ? get_post_meta($menu_item->ID, '_menu_item_target', true)
                : $menu_item->target;
            ...
            $menu_item->xfn = !isset($menu_item->xfn) ? get_post_meta($menu_item->ID, '_menu_item_xfn', true)
                : $menu_item->xfn;
        } else {
            $menu_item->db_id = 0;
...
function wp_get_associated_nav_menu_items($object_id = 0, $object_type = 'post_type', $taxonomy = '')
{
    ...
    foreach ((array)$menu_items as $menu_item) {
        if (isset($menu_item->ID) && is_nav_menu_item($menu_item->ID)) {
            ...
            } elseif ('taxonomy' === $object_type && 'taxonomy' === $menu_item_type
    ...
    foreach ($common_slug_groups as $slug_group) {
        ...
        foreach ($slug_group as $slug) {
            ...
            foreach ($registered_nav_menus as $new_location => $name) {
                // ...actually match!
                if (is_string($new_location) && false === stripos($new_location, $slug)
                ...
                foreach ($old_nav_menu_locations as $location => $menu_id) {
                    ...
                    foreach ($slug_group as $slug) {
                        ...
                        if (is_string($location) && false === stripos($location, $slug)
