...
class getid3_ac3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        $thisfile_ac3_raw_bsi['bsid'] = (getid3_lib::LittleEndian2Int(substr($tempAC3header, 5, 1)) & 0xF8)
        ...
        if ($thisfile_ac3_raw_bsi['bsid'] <= 8) {
            ...
            if ($thisfile_ac3_raw_bsi['frmsizecod']
                > 37) { // binary: 100101 - see Table 5.18 Frame Size Code Table (1 word = 16 bits)
                $this->warning('Unexpected ac3.bsi.frmsizecod value: '
            ...
            $thisfile_ac3['dialogue_normalization2'] = '-'
                ...
                . 'dB';  // This indicates how far the average dialogue level is below digital 100 percent. Valid values are 1-31. The value of 0 is reserved. The values of 1 to 31 are interpreted as -1 dB to -31 dB with respect to digital 100 percent.
            ...
            if ($thisfile_ac3_raw_bsi['flags']['timecod1'] & 0x01) {
                ...
                $thisfile_ac3['timecode1'] += (($thisfile_ac3_raw_bsi['timecod1'] & 0x3E00) >> 9)
                    * 3600;  // The first 5 bits of this 14-bit field represent the time in hours, with valid values of 0�23
                $thisfile_ac3['timecode1'] += (($thisfile_ac3_raw_bsi['timecod1'] & 0x01F8) >> 3)
                    * 60;  // The next 6 bits represent the time in minutes, with valid values of 0�59
                $thisfile_ac3['timecode1'] += (($thisfile_ac3_raw_bsi['timecod1'] & 0x0003) >> 0)
                    * 8;  // The final 3 bits represents the time in 8 second increments, with valid values of 0�7 (representing 0, 8, 16, ... 56 seconds)
            }
            if ($thisfile_ac3_raw_bsi['flags']['timecod1'] & 0x02) {
                ...
                $thisfile_ac3['timecode2'] += (($thisfile_ac3_raw_bsi['timecod2'] & 0x3800) >> 11)
                    * 1;              // The first 3 bits of this 14-bit field represent the time in seconds, with valid values from 0�7 (representing 0-7 seconds)
                $thisfile_ac3['timecode2'] += (($thisfile_ac3_raw_bsi['timecod2'] & 0x07C0) >> 6) * (1
                        / 30);        // The next 5 bits represents the time in frames, with valid values from 0�29 (one frame = 1/30th of a second)
                $thisfile_ac3['timecode2'] += (($thisfile_ac3_raw_bsi['timecod2'] & 0x003F) >> 0) * ((1 / 30)
                        / 60);  // The final 6 bits represents fractions of 1/64 of a frame, with valid values from 0�63
            }
            ...
            $this->error('E-AC3 parsing is incomplete and experimental in this version of getID3 ('
                . $this->getid3->version()
            ...
            if ($thisfile_ac3_raw_bsi['flags']['mixmdat']) { // Mixing metadata
                ...
                if (($thisfile_ac3_raw_bsi['acmod'] & 0x01)
                    && ($thisfile_ac3_raw_bsi['acmod']
                        > 2)) { // if three front channels exist
                ...
                if ($thisfile_ac3_raw_bsi['strmtyp'] == 0) { // if independent stream
                    ...
                    if ($thisfile_ac3_raw_bsi['acmod']
                        == 0) { // if 1+1 mode (dual mono, so some items need a second value)
                    ...
                    if ($thisfile_ac3_raw_bsi['flags']['frmmixcfginfo']) { // mixing configuration information
                        if ($thisfile_ac3_raw_bsi['numblkscod'] == 0) {
                            ...
                            for ($blk = 0; $blk < $thisfile_ac3_raw_bsi['numblkscod']; $blk++) {
                                ...
                                if ($thisfile_ac3_raw_bsi['flags']['blkmixcfginfo'
                                . $blk]) { // mixing configuration information
            ...
            if (($thisfile_ac3_raw_bsi['strmtyp'] == 0)
                && ($thisfile_ac3_raw_bsi['numblkscod']
                    != 3)) { //  if both surround channels exist
            ...
            $this->error('Bit stream identification is version '
        ...
        if (isset($thisfile_ac3_raw_bsi['frmsizecod'])) {
            $thisfile_ac3['frame_length'] = self::frameSizeLookup($thisfile_ac3_raw_bsi['frmsizecod'],
            ...
            $thisfile_ac3['bitrate'] = ($thisfile_ac3_raw_bsi['frmsiz'] + 1)
                * 16
                * 30; // The frmsiz field shall contain a value one less than the overall size of the coded syncframe in 16-bit words. That is, this field may assume a value ranging from 0 to 2047, and these values correspond to syncframe sizes ranging from 1 to 2048.
    ...
    public static function frameSizeLookup($frmsizecod, $fscod)
    {
        ...
        return (isset($frameSizeLookup[$framesizeid][$fscod]) ? $frameSizeLookup[$framesizeid][$fscod] + $paddingBytes
            : false);
    }
