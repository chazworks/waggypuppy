...
class getid3_mp3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (((isset($info['id3v2']['headerlength']) && ($info['avdataoffset'] > $info['id3v2']['headerlength']))
            ...
            if (isset($info['id3v2']['headerlength'])) {
                $synchoffsetwarning .= '(ID3v2 header ends at '
            ...
            if (isset($info['audio']['bitrate_mode']) && ($info['audio']['bitrate_mode'] == 'cbr')) {
                if (!empty($info['id3v2']['headerlength'])
                    && (($info['avdataoffset'] - $info['id3v2']['headerlength'])
                ...
                } elseif (empty($info['id3v2']['headerlength'])
                    && ($info['avdataoffset']
                        == $info['mpeg']['audio']['framelength'])) {
                    $synchoffsetwarning .= '. This is a known problem with some versions of LAME (3.90 - 3.92) DLL in CBR mode.';
                    $info['audio']['codec'] = 'LAME';
        ...
        $CurrentDataLAMEversionString = (!empty($CurrentDataLAMEversionString) ? $CurrentDataLAMEversionString
            : (isset($info['audio']['encoder']) ? $info['audio']['encoder'] : ''));
        if (!empty($CurrentDataLAMEversionString)
            ...
            if (($PossiblyLongerLAMEversion_String = strstr($PossiblyLongerLAMEversion_Data,
                ...
                if (substr($PossiblyLongerLAMEversion_String, 0, strlen($CurrentDataLAMEversionString))
                    ...
                    if (empty($info['audio']['encoder'])
                        ...
                        if (!empty($info['audio']['encoder'])
                            ...
                            && ($info['audio']['encoder'] == $info['mpeg']['audio']['LAME']['short_version'])) {
        ...
        if (isset($info['fileformat']) && ($info['fileformat'] == 'mp3')) {
            switch ($info['audio']['dataformat']) {
                ...
                default:
                    $this->warning('Expecting [audio][dataformat] to be mp1/mp2/mp3 when fileformat == mp3, [audio][dataformat] actually "'
        ...
        if (!isset($info['playtime_seconds']) && isset($info['audio']['bitrate']) && ($info['audio']['bitrate'] > 0)) {
            ...
            $xingVBRheaderFrameLength = ((isset($info['mpeg']['audio']['VBR_frames'])
                && isset($info['mpeg']['audio']['framelength'])) ? $info['mpeg']['audio']['framelength'] : 0);

            $info['playtime_seconds'] = ($info['avdataend'] - $info['avdataoffset'] - $xingVBRheaderFrameLength)
                * 8
                / $info['audio']['bitrate'];
        }
    ...
    public function GuessEncoderOptions()
    {
        ...
        if (isset($thisfile_mpeg_audio['VBR_method'])
            && ($thisfile_mpeg_audio['VBR_method'] == 'Fraunhofer')
            && !empty($thisfile_mpeg_audio['VBR_quality'])) {
            $encoder_options = 'VBR q' . $thisfile_mpeg_audio['VBR_quality'];
        } elseif (!empty($thisfile_mpeg_audio_lame['preset_used'])
            && isset($thisfile_mpeg_audio_lame['preset_used_id'])
            && (!in_array($thisfile_mpeg_audio_lame['preset_used_id'], $NamedPresetBitrates))) {
        ...
        if (!empty($thisfile_mpeg_audio_lame['encoding_flags']['nogap_prev'])
                ...
                case '--r3mix':
                    ...
                    switch ($ExplodedOptions[1]) {
                        ...
                        case 'r3mix':
                            ...
                            if (!isset($ExpectedLowpass[$ExplodedOptions[1]
                                    . '|'
                                    . $thisfile_mpeg_audio_lame['lowpass_frequency']])
                                && ($thisfile_mpeg_audio_lame['lowpass_frequency'] < 22050)
                                && (round($thisfile_mpeg_audio_lame['lowpass_frequency'] / 1000)
        ...
        if (isset($thisfile_mpeg_audio_lame['raw']['source_sample_freq'])) {
            if (($thisfile_mpeg_audio['sample_rate'] == 44100)
                && ($thisfile_mpeg_audio_lame['raw']['source_sample_freq'] != 1)) {
                $encoder_options .= ' --resample 44100';
            } elseif (($thisfile_mpeg_audio['sample_rate'] == 48000)
                            ...
                            case '--alt-preset':
                                switch ($ExplodedOptions[1]) {
                                    ...
                                    default:
                                        ...
                                        if (!isset($ExpectedResampledRate[$ExplodedOptions[1]
                                            . '|'
    ...
    public function decodeMPEGaudioHeader(
        ...
        if ($MPEGaudioHeaderValidCache[$head4_key]) {
            ...
            $this->warning('Invalid MPEG audio header ('
            ...
            switch ($thisfile_mpeg_audio['channelmode']) {
                case 'mono':
                    if (($thisfile_mpeg_audio['bitrate'] == 'free') || ($thisfile_mpeg_audio['bitrate'] <= 192000)) {
                        ...
                        $this->error($thisfile_mpeg_audio['bitrate']
                ...
                case 'dual channel':
                    if (($thisfile_mpeg_audio['bitrate'] == 'free')
                        ...
                        $this->error(intval(round($thisfile_mpeg_audio['bitrate'] / 1000))
                            . 'kbps not allowed in Layer 2, '
                            . $thisfile_mpeg_audio['channelmode']
                            . '.');
                        return false;
        ...
        if (substr($headerstring, 4 + 32, 4) == 'VBRI') {
            ...
            for ($i = 0; $i < $thisfile_mpeg_audio['VBR_seek_offsets']; $i++) {
                ...
                $thisfile_mpeg_audio['VBR_offsets_relative'][$i] = ($Fraunhofer_OffsetN
                    * $thisfile_mpeg_audio['VBR_seek_scale']);
                $thisfile_mpeg_audio['VBR_offsets_absolute'][$i] = ($Fraunhofer_OffsetN
                        * $thisfile_mpeg_audio['VBR_seek_scale']) + $previousbyteoffset;
            ...
            if ((substr($headerstring, $VBRidOffset, strlen('Xing')) == 'Xing')
...
//				}
                ...
                $thisfile_mpeg_audio['xing_flags']['frames'] = (bool)($thisfile_mpeg_audio['xing_flags_raw']
                    & 0x00000001);
                $thisfile_mpeg_audio['xing_flags']['bytes'] = (bool)($thisfile_mpeg_audio['xing_flags_raw']
                    & 0x00000002);
                $thisfile_mpeg_audio['xing_flags']['toc'] = (bool)($thisfile_mpeg_audio['xing_flags_raw'] & 0x00000004);
                $thisfile_mpeg_audio['xing_flags']['vbr_scale'] = (bool)($thisfile_mpeg_audio['xing_flags_raw']
                    & 0x00000008);
                ...
                if (!empty($thisfile_mpeg_audio['VBR_frames'])) {
                    ...
                    if (!empty($thisfile_mpeg_audio['VBR_bytes'])) {
                        ...
                        $used_filesize -= (isset($info['id3v2']['headerlength'])
                            ? intval($info['id3v2']['headerlength']) : 0);
                        $used_filesize -= (isset($info['id3v1']) ? 128 : 0);
                        $used_filesize -= (isset($info['tag_offset_end']) ? $info['tag_offset_end']
                            - $info['tag_offset_start'] : 0);
                        $this->warning('MP3.Xing header missing VBR_bytes, assuming MPEG audio portion of file is '
                    ...
                    if ($thisfile_mpeg_audio['layer'] == '1') {
                        ...
                        $info['audio']['bitrate'] = ($framelengthfloat / 4) * $thisfile_mpeg_audio['sample_rate'] * (2
                                / $info['audio']['channels']) / 12;
                    } else {
                        // Bitrate = ((FrameLengthInBytes - Padding) * SampleRate) / 144
                        //$info['audio']['bitrate'] = (($framelengthfloat - intval($thisfile_mpeg_audio['padding'])) * $thisfile_mpeg_audio['sample_rate']) / 144;
                        $info['audio']['bitrate'] = $framelengthfloat * $thisfile_mpeg_audio['sample_rate'] * (2
                                / $info['audio']['channels']) / 144;
                    }
                ...
                if (substr($headerstring, $VBRidOffset + 120, 4) == 'LAME') {
                    ...
                    if (strlen($thisfile_mpeg_audio_lame['numeric_version']) > 0) {
                        ...
                        if ((($thisfile_mpeg_audio_lame['integer_version'][0] * 1000)
                                + $thisfile_mpeg_audio_lame['integer_version'][1])
                            >= 3090) { // cannot use string version compare, may have "LAME3.90" or "LAME3.100" -- see https://github.com/JamesHeinrich/getID3/issues/207
                            ...
                            if ($thisfile_mpeg_audio_lame_raw['RGAD_track'] != 0) {
                                $thisfile_mpeg_audio_lame_RGAD_track['raw']['name'] = ($thisfile_mpeg_audio_lame_raw['RGAD_track']
                                        & 0xE000) >> 13;
                                $thisfile_mpeg_audio_lame_RGAD_track['raw']['originator'] = ($thisfile_mpeg_audio_lame_raw['RGAD_track']
                                        & 0x1C00) >> 10;
                                $thisfile_mpeg_audio_lame_RGAD_track['raw']['sign_bit'] = ($thisfile_mpeg_audio_lame_raw['RGAD_track']
                                        & 0x0200) >> 9;
                                $thisfile_mpeg_audio_lame_RGAD_track['raw']['gain_adjust'] = $thisfile_mpeg_audio_lame_raw['RGAD_track']
                                    & 0x01FF;
                            ...
                            if ($thisfile_mpeg_audio_lame_raw['RGAD_album'] != 0) {
                                $thisfile_mpeg_audio_lame_RGAD_album['raw']['name'] = ($thisfile_mpeg_audio_lame_raw['RGAD_album']
                                        & 0xE000) >> 13;
                                $thisfile_mpeg_audio_lame_RGAD_album['raw']['originator'] = ($thisfile_mpeg_audio_lame_raw['RGAD_album']
                                        & 0x1C00) >> 10;
                                $thisfile_mpeg_audio_lame_RGAD_album['raw']['sign_bit'] = ($thisfile_mpeg_audio_lame_raw['RGAD_album']
                                        & 0x0200) >> 9;
                                $thisfile_mpeg_audio_lame_RGAD_album['raw']['gain_adjust'] = $thisfile_mpeg_audio_lame_raw['RGAD_album']
                                    & 0x01FF;
                            ...
                            $thisfile_mpeg_audio_lame['encoding_flags']['nspsytune'] = (bool)($EncodingFlagsATHtype
                                & 0x10);
                            $thisfile_mpeg_audio_lame['encoding_flags']['nssafejoint'] = (bool)($EncodingFlagsATHtype
                                & 0x20);
                            $thisfile_mpeg_audio_lame['encoding_flags']['nogap_next'] = (bool)($EncodingFlagsATHtype
                                & 0x40);
                            $thisfile_mpeg_audio_lame['encoding_flags']['nogap_prev'] = (bool)($EncodingFlagsATHtype
                                & 0x80);
                            ...
                            if ($thisfile_mpeg_audio_lame_raw['vbr_method'] == 2) { // Average BitRate (ABR)
                                ...
                                // ignore
                            } elseif ($thisfile_mpeg_audio_lame['raw']['abrbitrate_minbitrate']
                                > 0) { // Variable BitRate (VBR) - minimum bitrate
                                $thisfile_mpeg_audio_lame['bitrate_min'] = $thisfile_mpeg_audio_lame['raw']['abrbitrate_minbitrate'];
                            }

                            // bytes $B1-$B3  Encoder delays
                            $EncoderDelays = getid3_lib::BigEndian2Int(substr($headerstring,
                                $LAMEtagOffsetContant + 0xB1, 3));
                            ...
                            // byte $B4  Misc
                            $MiscByte = getid3_lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xB4,
                                1));
                            ...
                            $thisfile_mpeg_audio_lame['mp3_gain_db'] = (getid3_lib::RGADamplitude2dB(2) / 4)
                            ...
                            if (!empty($thisfile_mpeg_audio_lame['preset_used_id'])
                                && empty($thisfile_mpeg_audio_lame['preset_used'])) {
                                $this->warning('Unknown LAME preset used ('
                                    . $thisfile_mpeg_audio_lame['preset_used_id']
                                    . ') - please report to info@getid3.org');
                            }
                            if (($thisfile_mpeg_audio_lame['short_version'] == 'LAME3.90.')
                                && !empty($thisfile_mpeg_audio_lame['preset_used_id'])) {
                            ...
                            $ExpectedNumberOfAudioBytes = (($thisfile_mpeg_audio_lame['audio_bytes'] > 0)
                                ? $thisfile_mpeg_audio_lame['audio_bytes'] : $thisfile_mpeg_audio['VBR_bytes']);
        ...
        if (($ExpectedNumberOfAudioBytes > 0)
            ...
            if ($ExpectedNumberOfAudioBytes > ($info['avdataend'] - $info['avdataoffset'])) {
                if ($this->isDependencyFor('matroska') || $this->isDependencyFor('riff')) {
                    ...
                    $this->warning('Probable truncated file: expecting '
                        . $ExpectedNumberOfAudioBytes
                        . ' bytes of audio data, only found '
                        . ($info['avdataend'] - $info['avdataoffset'])
                        . ' (short by '
                        . ($ExpectedNumberOfAudioBytes - ($info['avdataend'] - $info['avdataoffset']))
                        . ' bytes)');
                }
            } else {
                if ((($info['avdataend'] - $info['avdataoffset']) - $ExpectedNumberOfAudioBytes) == 1) {
                    ...
                    $this->warning('Too much data in file: expecting '
                        . $ExpectedNumberOfAudioBytes
                        . ' bytes of audio data, found '
                        . ($info['avdataend'] - $info['avdataoffset'])
                        . ' ('
                        . (($info['avdataend'] - $info['avdataoffset']) - $ExpectedNumberOfAudioBytes)
                        . ' bytes too many)');
                }
            }
        }
        ...
        if (($thisfile_mpeg_audio['bitrate'] == 'free') && empty($info['audio']['bitrate'])) {
            if (($offset == $info['avdataoffset']) && empty($thisfile_mpeg_audio['VBR_frames'])) {
                ...
                if ($framebytelength > 0) {
                    ...
                    if ($thisfile_mpeg_audio['layer'] == '1') {
                        // BitRate = (((FrameLengthInBytes / 4) - Padding) * SampleRate) / 12
                        $info['audio']['bitrate'] = ((($framebytelength / 4) - intval($thisfile_mpeg_audio['padding']))
                                * $thisfile_mpeg_audio['sample_rate']) / 12;
                    } else {
                        // Bitrate = ((FrameLengthInBytes - Padding) * SampleRate) / 144
                        $info['audio']['bitrate'] = (($framebytelength - intval($thisfile_mpeg_audio['padding']))
            ...
            switch ($thisfile_mpeg_audio['bitrate_mode']) {
                ...
                case 'abr':
                    ...
                    if (($thisfile_mpeg_audio['version'] == '1') && ($thisfile_mpeg_audio['layer'] == 1)) {
                        $bytes_per_frame = 384;
                    } elseif ((($thisfile_mpeg_audio['version'] == '2') || ($thisfile_mpeg_audio['version'] == '2.5'))
                        && ($thisfile_mpeg_audio['layer'] == 3)) {
                        $bytes_per_frame = 576;
                    }
                    $thisfile_mpeg_audio['VBR_bitrate'] = (isset($thisfile_mpeg_audio['VBR_bytes'])
                        ? (($thisfile_mpeg_audio['VBR_bytes'] / $thisfile_mpeg_audio['VBR_frames']) * 8)
                        * ($info['audio']['sample_rate'] / $bytes_per_frame) : 0);
        ...
        if ($recursivesearch) {
            ...
            if (!empty($this->getid3->info['mp3_validity_check_bitrates'])
                ...
                if (count(array_keys($this->getid3->info['mp3_validity_check_bitrates'])) == 1) {
                    ...
                    $deviation_cbr_from_header_bitrate = abs($thisfile_mpeg_audio['VBR_bitrate']
                            - $cbr_bitrate_in_short_scan) / $cbr_bitrate_in_short_scan;
        ...
        for ($i = 0; $i < $this->mp3_valid_check_frames; $i++) {
            ...
            if ($this->decodeMPEGaudioHeader($nextframetestoffset, $nextframetestarray, false)) {
                ...
                if ($ScanAsCBR) {
                    ...
                    if (!isset($nextframetestarray['mpeg']['audio']['bitrate'])
                        || !isset($firstframetestarray['mpeg']['audio']['bitrate'])
                ...
                if (isset($nextframetestarray['mpeg']['audio']['framelength'])
            ...
            } elseif (!empty($firstframetestarray['mpeg']['audio']['framelength'])
                && (($nextframetestoffset
                        + $firstframetestarray['mpeg']['audio']['framelength']) > $info['avdataend'])) {
                // it's not the end of the file, but there's not enough data left for another frame, so assume it's garbage/padding and return OK
                return true;
            } else {
                // next frame is not valid, note the error and fail, so scanning can contiue for a valid frame sequence
                $this->warning('Frame at offset ('
                    . $offset
    ...
    public function FreeFormatFrameLength($offset, $deepscan = false)
    {
        ...
        if (!$framelength) {
            ...
            if (!$framelength) {
                $this->error('Cannot find next free-format synch pattern ('
        ...
        if ($deepscan) {
            ...
            while ($nextoffset < ($info['avdataend'] - 6)) {
                ...
                if ((substr($NextSyncPattern, 1, strlen($SyncPattern1)) == $SyncPattern1)
                    || (substr($NextSyncPattern, 1, strlen($SyncPattern2)) == $SyncPattern2)) {
                    // good - found where expected
                    $ActualFrameLengthValues[] = $framelength;
                } elseif ((substr($NextSyncPattern, 0, strlen($SyncPattern1)) == $SyncPattern1)
                    || (substr($NextSyncPattern, 0, strlen($SyncPattern2)) == $SyncPattern2)) {
                    // ok - found one byte earlier than expected (last frame wasn't padded, first frame was)
                    $ActualFrameLengthValues[] = ($framelength - 1);
                    $nextoffset--;
                } elseif ((substr($NextSyncPattern, 2, strlen($SyncPattern1)) == $SyncPattern1)
                    || (substr($NextSyncPattern, 2, strlen($SyncPattern2)) == $SyncPattern2)) {
                ...
                if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
                    ...
                    if ($next4[0] == "\xFF") {
                        ...
                        if ($MPEGaudioHeaderValidCache[$next4]) {
                            ...
                            $Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]] = isset($Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]])
                                ? ++$Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]] : 1;
                            $Distribution['layer'][$LongMPEGlayerLookup[$head4]] = isset($Distribution['layer'][$LongMPEGlayerLookup[$head4]])
                                ? ++$Distribution['layer'][$LongMPEGlayerLookup[$head4]] : 1;
                            $Distribution['version'][$LongMPEGversionLookup[$head4]] = isset($Distribution['version'][$LongMPEGversionLookup[$head4]])
                                ? ++$Distribution['version'][$LongMPEGversionLookup[$head4]] : 1;
                            $Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])] = isset($Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])])
                                ? ++$Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])] : 1;
                            $Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]] = isset($Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]])
                                ? ++$Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]] : 1;
                            if (++$frames_scanned >= $max_frames_scan) {
                                $pct_data_scanned = getid3_lib::SafeDiv($this->ftell() - $info['avdataoffset'],
                                    $info['avdataend'] - $info['avdataoffset']);
                                $this->warning('too many MPEG audio frames to scan, only scanned first '
                                    . $max_frames_scan
                                    . ' frames ('
                                ...
                                foreach ($Distribution as $key1 => $value1) {
                                    foreach ($value1 as $key2 => $value2) {
                                        $Distribution[$key1][$key2] = $pct_data_scanned ? round($value2
                                            / $pct_data_scanned) : 1;
                                    }
                                }
                                break;
        ...
        while ($SynchSeekOffset < $sync_seek_buffer_size) {
            if ((($avdataoffset + $SynchSeekOffset) < $info['avdataend']) && !$this->feof()) {
                if ($SynchSeekOffset > $sync_seek_buffer_size) {
                    ...
                    $this->error('Could not find valid MPEG audio synch within the first '
            ...
            if (($header[$SynchSeekOffset] == "\xFF")
                && ($header[($SynchSeekOffset + 1)]
                    > "\xE0")) { // possible synch detected
                if (++$SyncSeekAttempts >= $SyncSeekAttemptsMax) {
                    ...
                    // if a synch's not found within the first 128k bytes, then give up
                    $this->error('Could not find valid MPEG audio synch after scanning '
                        . $SyncSeekAttempts
                        . ' candidate offsets');
                    if (isset($info['audio']['bitrate'])) {
                        unset($info['audio']['bitrate']);
                ...
                if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset, $dummy, true)) {
                    ...
                    if (isset($FirstFrameThisfileInfo)
                        && isset($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode'])
                        ...
                        if (!(abs($info['audio']['bitrate'] - $FirstFrameThisfileInfo['audio']['bitrate']) <= 1)) {
                            ...
                            $GarbageOffsetStart = $FirstFrameAVDataOffset
                                + $FirstFrameThisfileInfo['mpeg']['audio']['framelength'];
                            $GarbageOffsetEnd = $avdataoffset + $SynchSeekOffset;
                            if ($this->decodeMPEGaudioHeader($GarbageOffsetEnd, $dummy, true, true)) {
                                $info = $dummy;
                                $info['avdataoffset'] = $GarbageOffsetEnd;
                                $this->warning('apparently-valid VBR header not used because could not find '
                                ...
                                $this->warning('using data from VBR header even though could not find '
                                    . $this->mp3_valid_check_frames
                                    . ' consecutive MPEG-audio frames immediately after VBR header (garbage data for '
                                    . ($GarbageOffsetEnd - $GarbageOffsetStart)
                                    . ' bytes between '
                    ...
                    if ($BitrateHistogram) {
                        ...
                        for ($current_segment = 0; $current_segment < $max_scan_segments; $current_segment++) {
                            ...
                            $scan_start_offset[$current_segment] = max($this->ftell(),
                                $info['avdataoffset'] + round($current_segment * (($info['avdataend']
                                            - $info['avdataoffset']) / $max_scan_segments)));
                                ...
                                for ($j = 0; $j < (strlen($buffer_4k) - 4); $j++) {
                                    if (($buffer_4k[$j] == "\xFF")
                                        ...
                                        if ($this->decodeMPEGaudioHeader($scan_start_offset[$current_segment] + $j,
                                            ...
                                            $calculated_next_offset = $scan_start_offset[$current_segment]
                                                + $j
                            ...
                            while (($synchstartoffset < $info['avdataend'])
                                && $this->decodeMPEGaudioHeader($synchstartoffset, $dummy, false, false, $FastMode)) {
                                $FastMode = true;
                                ...
                                if ($frames_scan_per_segment
                                    ...
                                    $this_pct_scanned = getid3_lib::SafeDiv($this->ftell()
                        ...
                        if ($pct_data_scanned > 0) {
                            $this->warning('too many MPEG audio frames to scan, only scanned '
                                . $frames_scanned
                                . ' frames in '
                                . $max_scan_segments
                                . ' segments ('
        ...
        if (!isset($AudioFrameLengthCache[$bitrate][$version][$layer][$padding][$samplerate])) {
            ...
            if ($bitrate != 'free') {
                ...
                if ($samplerate > 0) {
                    ...
                    $NewFramelength = floor($NewFramelength / $SlotLength)
        ...
        if (!isset($bit_rate_table[$round_bit_rate])) {
            if ($round_bit_rate > max($standard_bit_rates)) {
                ...
                foreach ($standard_bit_rates as $standard_bit_rate) {
                    if ($round_bit_rate >= $standard_bit_rate + (($bit_rate_table[$round_bit_rate] - $standard_bit_rate)
                            / 2)) {
                        break;
                    }
    ...
    public static function LAMEmiscSourceSampleFrequencyLookup($SourceSampleFrequencyID)
    {
        ...
        return (isset($LAMEmiscSourceSampleFrequencyLookup[$SourceSampleFrequencyID])
    ...
    public static function LAMEsurroundInfoLookup($SurroundInfoID)
    {
        ...
        return (isset($LAMEsurroundInfoLookup[$SurroundInfoID]) ? $LAMEsurroundInfoLookup[$SurroundInfoID]
            : 'reserved');
    }
