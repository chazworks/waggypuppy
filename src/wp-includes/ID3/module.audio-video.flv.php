    ...
    public function Analyze()
    {
        ...
        if ($info['flv']['header']['signature'] != self::magic) {
            $this->error('Expecting "'
                ...
                . $info['avdataoffset']
                . ', found "'
                . getid3_lib::PrintHexBytes($info['flv']['header']['signature'])
                . '"');
        ...
        while ((($this->ftell() + 16) < $info['avdataend'])
                ...
                case GETID3_FLV_TAG_VIDEO:
                    ...
                    if (!$found_video) {
                        ...
                        if (!empty($info['video']['resolution_x']) && !empty($info['video']['resolution_y'])) {
                            $info['video']['pixel_aspect_ratio'] = $info['video']['resolution_x']
                            ...
                            if (isset($info['flv']['meta']['onMetaData'][$sourcekey])) {
                                switch ($sourcekey) {
                                    ...
                                    case 'height':
                                        ...
                                        break;
                                    case 'audiodatarate':
                                        $info['audio'][$destkey] = getid3_lib::CastAsInt(round($info['flv']['meta']['onMetaData'][$sourcekey]
                                            * 1000));
                                        break;
                                    case 'videodatarate':
                                    case 'frame_rate':
...
class AVCSequenceParameterSetReader
{
    ...
    public function readData()
    {
        ...
        if ($profile > 0) {
            ...
            $this->width = (($pic_width_in_mbs_minus1 + 1) * 16)
                - ($frame_crop_left_offset * 2)
                - ($frame_crop_right_offset * 2);
            $this->height = ((2 - $frame_mbs_only_flag) * ($pic_height_in_map_units_minus1 + 1) * 16)
    ...
    {
        $result = (getid3_lib::BigEndian2Int(substr($this->sps, $this->currentBytes, 1)) >> (7 - $this->currentBits))
            & 0x01;
        $this->skipBits(1);
        return $result;
    }
