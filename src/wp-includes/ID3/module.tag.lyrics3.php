...
class getid3_lyrics3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (!getid3_lib::intValueSupported($info['filesize'])) {
            $this->warning('Unable to check for Lyrics3 because file is larger than '
        ...
        if ($lyrics3end == 'LYRICSEND') {
            ...
            $lyrics3size = (int)strrev(substr(strrev($lyrics3_id3v1), 9, 6))
                + 6
    ...
    public function getLyrics3Data($endoffset, $version, $length)
    {
        ...
        if (substr($rawdata, 0, 11) != 'LYRICSBEGIN') {
            if (strpos($rawdata, 'LYRICSBEGIN') !== false) {
                $this->warning('"LYRICSBEGIN" expected at ' . $endoffset . ' but actually found at ' . ($endoffset
                        + strpos($rawdata, 'LYRICSBEGIN')) . ' - this is invalid for Lyrics3 v' . $version);
                ...
                $this->error('"LYRICSBEGIN" expected at '
                    . $endoffset
                    . ' but found "'
                    . substr($rawdata, 0, 11)
                    . '" instead');
        ...
        switch ($version) {
            case 1:
                if (substr($rawdata, strlen($rawdata) - 9, 9) == 'LYRICSEND') {
                    ...
                    $this->error('"LYRICSEND" expected at '
            ...
            case 2:
                if (substr($rawdata, strlen($rawdata) - 9, 9) == 'LYRICS200') {
                        ...
                        foreach ($imagestrings as $key => $imagestring) {
                            if (strpos($imagestring, '||') !== false) {
                                ...
                                $ParsedLyrics3['images'][$key]['description'] = (isset($imagearray[1]) ? $imagearray[1]
                                    : '');
                                $ParsedLyrics3['images'][$key]['timestamp'] = $this->Lyrics3Timestamp2Seconds(isset($imagearray[2])
                                    ? $imagearray[2] : '');
                            }
                        }
                    }
                    ...
                    $this->error('"LYRICS200" expected at '
                        . ($this->ftell() - 11 + $length - 9)
                        . ' but found "'
                        . substr($rawdata, strlen($rawdata) - 9, 9)
                        . '" instead');
