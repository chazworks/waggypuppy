...
class getID3
{
    ...
    public function __construct()
    {
        ...
        if (version_compare(PHP_VERSION, $required_php_version, '<')) {
            $this->startup_error .= 'getID3() requires PHP v'
        ...
        } elseif (preg_match('#([0-9]+) ?G#i', $memoryLimit,
        ...
        if (($mbstring_func_overload = (int)ini_get('mbstring.func_overload')) && ($mbstring_func_overload & 0x02)) {
            ...
            $this->startup_error .= 'WARNING: php.ini contains "mbstring.func_overload = '
        ...
        if (GETID3_OS_ISWINDOWS && !defined('GETID3_HELPERAPPSDIR')) {
            $helperappsdir = GETID3_INCLUDEPATH
                . '..'
                . DIRECTORY_SEPARATOR
    ...
    public function openfile($filename, $filesize = null, $fp = null)
    {
        try {
            ...
            if (($fp != null) && ((get_resource_type($fp) == 'file') || (get_resource_type($fp) == 'stream'))) {
                $this->fp = $fp;
            } elseif ((is_readable($filename) || file_exists($filename))
                && is_file($filename)
                ...
                throw new getid3_exception('Could not open "'
                    . $filename
                    . '" ('
            ...
            if ($this->option_max_2gb_check) {
                ...
                if (($fseek < 0)
                    ...
                    if ($real_filesize === false) {
                        ...
                        unset($this->info['filesize']);
                        fclose($this->fp);
                        throw new getid3_exception('PHP seems to think the file is larger than ' . round(PHP_INT_MAX
                                / 1073741824) . 'GB, but filesystem reports it as ' . number_format($real_filesize
                                / 1073741824, 3) . 'GB, please report to info@getid3.org');
                    }
                    $this->info['filesize'] = $real_filesize;
                    $this->warning('File is larger than '
                        . round(PHP_INT_MAX / 1073741824)
        ...
        try {
            ...
            if (!$this->option_tag_id3v2) {
                ...
                if ((substr($header, 0, 3) == 'ID3') && (strlen($header) == 10)) {
                    ...
                    $this->info['avdataoffset'] += getid3_lib::BigEndian2Int(substr($header, 6, 4), 1)
            ...
            if (isset($determined_format['fail_id3'])
            ...
            if (!file_exists(GETID3_INCLUDEPATH . $determined_format['include'])) {
                fclose($this->fp);
                return $this->error('Format not supported, module "'
            ...
            if (!empty($determined_format['iconv_req'])
                && !function_exists('mb_convert_encoding')
                && !function_exists('iconv')
                && !in_array($this->encoding, ['ISO-8859-1', 'UTF-8', 'UTF-16LE', 'UTF-16BE', 'UTF-16'])) {
                $errormessage = 'mb_convert_encoding() or iconv() support is required for this module ('
                    . $determined_format['include']
            ...
            foreach (get_object_vars($this) as $getid3_object_vars_key => $getid3_object_vars_value) {
                if (preg_match('#^options_([^_]+)_([^_]+)_(.+)$#i', $getid3_object_vars_key, $matches)) {
                    ...
                    $GOVgroup = (($GOVgroup == 'audiovideo') ? 'audio-video'
                        : $GOVgroup); // variable names can only contain 0-9a-z_ so standardize here
    ...
    public function GetFileFormat(&$filedata, $filename = '')
    {
        ...
        if (preg_match('#\\.mp[123a]$#i', $filename)) {
            ...
            $info['include'] = 'module.' . $info['group'] . '.' . $info['module'] . '.php';
            return $info;
        } elseif (preg_match('#\\.mp[cp\\+]$#i', $filename)
            ...
            $info['include'] = 'module.' . $info['group'] . '.' . $info['module'] . '.php';
            return $info;
        } elseif (preg_match('#\\.cue$#i', $filename)
            && preg_match('#FILE "[^"]+" (BINARY|MOTOROLA|AIFF|WAVE|MP3)#', $filedata)) {
    ...
    public function HandleAllTags()
    {
        ...
        foreach ($tags as $comment_name => $tagname_encoding_array) {
            ...
            if (!empty($this->info[$comment_name]['comments'])) {
                ...
                $this->CharConvert($this->info['tags'][$tag_name],
                    $this->info[$comment_name]['encoding']);           // only copy gets converted!
    ...
    public function getHashdata($algorithm)
    {
        ...
        if (!empty($this->info['fileformat']) && !empty($this->info['dataformat'])
            ...
            if (preg_match('#(1|ON)#i', ini_get('safe_mode'))) {
                $this->warning('Failed making system call to vorbiscomment.exe - '
                    . $algorithm
            ...
            if (!empty($this->info['avdataoffset'])
    ...
    public function ChannelsBitratePlaytimeCalculations()
    {
        ...
        if (isset($this->info['video']['dataformat'])
            && $this->info['video']['dataformat']
            && (!isset($this->info['video']['bitrate']) || ($this->info['video']['bitrate'] == 0))) {
            // if video bitrate not set
            if (isset($this->info['audio']['bitrate'])
                ...
                if (isset($this->info['playtime_seconds']) && ($this->info['playtime_seconds'] > 0)) {
                    ...
                    if (isset($this->info['avdataend']) && isset($this->info['avdataoffset'])) {
                        ...
                        $this->info['bitrate'] = round((($this->info['avdataend'] - $this->info['avdataoffset']) * 8)
        ...
        if ((!isset($this->info['playtime_seconds']) || ($this->info['playtime_seconds'] <= 0))
            ...
            $this->info['playtime_seconds'] = (($this->info['avdataend'] - $this->info['avdataoffset']) * 8)
                / $this->info['bitrate'];
        }

        if (!isset($this->info['bitrate']) && !empty($this->info['playtime_seconds'])) {
            $this->info['bitrate'] = (($this->info['avdataend'] - $this->info['avdataoffset']) * 8)
                / $this->info['playtime_seconds'];
        }
        if (isset($this->info['bitrate'])
    ...
    public function CalculateCompressionRatioVideo()
    {
        ...
        $BitrateUncompressed = $this->info['video']['resolution_x']
    ...
    {
        if (empty($this->info['audio']['bitrate'])
            || empty($this->info['audio']['channels'])
            || empty($this->info['audio']['sample_rate'])
            || !is_numeric($this->info['audio']['sample_rate'])) {
            return false;
        }
        $this->info['audio']['compression_ratio'] = $this->info['audio']['bitrate'] / ($this->info['audio']['channels']
                * $this->info['audio']['sample_rate']
                * (!empty($this->info['audio']['bits_per_sample']) ? $this->info['audio']['bits_per_sample'] : 16));

        if (!empty($this->info['audio']['streams'])) {
            foreach ($this->info['audio']['streams'] as $streamnumber => $streamdata) {
                if (!empty($streamdata['bitrate'])
                    && !empty($streamdata['channels'])
                    && !empty($streamdata['sample_rate'])) {
                    $this->info['audio']['streams'][$streamnumber]['compression_ratio'] = $streamdata['bitrate']
                        / ($streamdata['channels']
                            * $streamdata['sample_rate']
                            * (!empty($streamdata['bits_per_sample']) ? $streamdata['bits_per_sample'] : 16));
                }
            }
        }
        return true;
    ...
    public function CalculateReplayGain()
    {
        if (isset($this->info['replay_gain'])) {
            ...
            if (isset($this->info['replay_gain']['track']['adjustment'])) {
                $this->info['replay_gain']['track']['volume'] = $this->info['replay_gain']['reference_volume']
            ...
            if (isset($this->info['replay_gain']['track']['peak'])) {
                $this->info['replay_gain']['track']['max_noclip_gain'] = 0
                    - getid3_lib::RGADamplitude2dB($this->info['replay_gain']['track']['peak']);
            }
            if (isset($this->info['replay_gain']['album']['peak'])) {
                $this->info['replay_gain']['album']['max_noclip_gain'] = 0
                    - getid3_lib::RGADamplitude2dB($this->info['replay_gain']['album']['peak']);
            }
        }
        return true;
    }
...
abstract class getid3_handler
{
    ...
    public function AnalyzeString($string)
    {
        ...
        $saved_filesize = (isset($this->getid3->info['filesize']) ? $this->getid3->info['filesize']
    ...
    protected function fread($bytes)
    {
        ...
        if (!getid3_lib::intValueSupported($pos)) {
            throw new getid3_exception('cannot fread('
                . $bytes
                . ' from '
                . $this->ftell()
        ...
        */
        ...
        do {
            ...
            if (($this->getid3->memory_limit > 0)
                && (($bytes / $this->getid3->memory_limit)
    ...
    public function saveAttachment($name, $offset, $length, $image_mime = null)
    {
        ...
        try {
            ...
            if ($this->getid3->option_save_attachments === getID3::ATTACHMENTS_NONE) {
                ...
                $dest = $dir . DIRECTORY_SEPARATOR . $name . ($image_mime ? '.'
                ...
                $bytesleft = $length;
                while ($bytesleft > 0) {
                    if (($buffer = $this->fread(min($buffersize, $bytesleft))) === false
                        || ($byteswritten = fwrite($fp_dest, $buffer)) === false
                        || ($byteswritten === 0)) {
                        throw new Exception($buffer === false ? 'not enough data to read'
