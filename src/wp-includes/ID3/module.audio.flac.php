...
class getid3_flac extends getid3_handler
{
    ...
    public function parseMETAdata()
    {
        ...
        do {
            ...
            switch ($BlockTypeText) {
                ...
                default:
                    $this->warning('Unhandled METADATA_BLOCK_HEADER.BLOCK_TYPE ('
                        . $BlockType
                        . ') at offset '
                        . $BlockOffset);
            }
        ...
        if (isset($info['flac']['STREAMINFO'])) {
            ...
            $info['flac']['uncompressed_audio_bytes'] = $info['flac']['STREAMINFO']['samples_stream']
            ...
            if (!empty($info['flac']['compressed_audio_bytes'])) {
                $info['flac']['compression_ratio'] = $info['flac']['compressed_audio_bytes']
    ...
    private function parseSTREAMINFO($BlockData)
    {
        ...
        if (!empty($info['flac']['STREAMINFO']['sample_rate'])) {
            ...
            $info['audio']['bits_per_sample'] = $info['flac']['STREAMINFO']['bits_per_sample'];
            $info['playtime_seconds'] = $info['flac']['STREAMINFO']['samples_stream']
                / $info['flac']['STREAMINFO']['sample_rate'];
            if ($info['playtime_seconds'] > 0) {
                if (!$this->isDependencyFor('matroska')) {
                    $info['audio']['bitrate'] = (($info['avdataend'] - $info['avdataoffset']) * 8)
    ...
    private function parseCUESHEET($BlockData)
    {
        ...
        $offset += 8;
        $info['flac']['CUESHEET']['flags']['is_cd'] = (bool)(getid3_lib::BigEndian2Int(substr($BlockData, $offset, 1))
            & 0x80);
        $offset += 1;
