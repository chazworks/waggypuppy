...
class getid3_apetag extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (!getid3_lib::intValueSupported($info['filesize'])) {
            $this->warning('Unable to check for APEtags because file is larger than '
        ...
        if (isset($thisfile_ape['footer']['flags']['header']) && $thisfile_ape['footer']['flags']['header']) {
            $this->fseek($thisfile_ape['tag_offset_end']
            ...
            $thisfile_ape['tag_offset_start'] = $thisfile_ape['tag_offset_end']
                - $thisfile_ape['footer']['raw']['tagsize'];
        ...
        if (isset($info['id3v1']['tag_offset_start'])
        ...
        for ($i = 0; $i < $thisfile_ape['footer']['raw']['tag_items']; $i++) {
            ...
            if (strstr(substr($APEtagData, $offset), "\x00") === false) {
                $this->error('Cannot find null-byte (0x00) separator between ItemKey #'
                    . $i
            ...
            switch (strtolower($item_key)) {
                ...
                case 'replaygain_track_gain':
                    if (preg_match('#^([\\-\\+][0-9\\.,]{8})( dB)?$#', $thisfile_ape_items_current['data'][0],
                        $matches)) {
                        $thisfile_replaygain['track']['adjustment'] = (float)str_replace(',', '.',
                            $matches[1]); // float casting will see "0,95" as zero!
                        $thisfile_replaygain['track']['originator'] = 'unspecified';
                    } else {
                        $this->warning('MP3gainTrackGain value in APEtag appears invalid: "'
                ...
                case 'replaygain_track_peak':
                    if (preg_match('#^([0-9\\.,]{8})$#', $thisfile_ape_items_current['data'][0], $matches)) {
                        ...
                        if ($thisfile_replaygain['track']['peak'] <= 0) {
                            $this->warning('ReplayGain Track peak from APEtag appears invalid: '
                        ...
                        $this->warning('MP3gainTrackPeak value in APEtag appears invalid: "'
                            . $thisfile_ape_items_current['data'][0]
                            . '"');
                    }
                    break;
                ...
                case 'replaygain_album_gain':
                    if (preg_match('#^([\\-\\+][0-9\\.,]{8})( dB)?$#', $thisfile_ape_items_current['data'][0],
                        ...
                        $this->warning('MP3gainAlbumGain value in APEtag appears invalid: "'
                            . $thisfile_ape_items_current['data'][0]
                            . '"');
                    }
                    break;
                ...
                case 'replaygain_album_peak':
                    if (preg_match('#^([0-9\\.,]{8})$#', $thisfile_ape_items_current['data'][0], $matches)) {
                        ...
                        if ($thisfile_replaygain['album']['peak'] <= 0) {
                            $this->warning('ReplayGain Album peak from APEtag appears invalid: '
                        ...
                        $this->warning('MP3gainAlbumPeak value in APEtag appears invalid: "'
                            . $thisfile_ape_items_current['data'][0]
                            . '"');
                    }
                    break;
                ...
                case 'mp3gain_undo':
                    if (preg_match('#^[\\-\\+][0-9]{3},[\\-\\+][0-9]{3},[NW]$#',
                        $thisfile_ape_items_current['data'][0])) {
                        [$mp3gain_undo_left, $mp3gain_undo_right, $mp3gain_undo_wrap] = explode(',',
                            $thisfile_ape_items_current['data'][0]);
                        $thisfile_replaygain['mp3gain']['undo_left'] = intval($mp3gain_undo_left);
                        $thisfile_replaygain['mp3gain']['undo_right'] = intval($mp3gain_undo_right);
                        $thisfile_replaygain['mp3gain']['undo_wrap'] = (($mp3gain_undo_wrap == 'Y') ? true : false);
                    } else {
                        $this->warning('MP3gainUndo value in APEtag appears invalid: "'
                            . $thisfile_ape_items_current['data'][0]
                            . '"');
                    }
                    break;
                ...
                case 'mp3gain_minmax':
                    if (preg_match('#^[0-9]{3},[0-9]{3}$#', $thisfile_ape_items_current['data'][0])) {
                        [$mp3gain_globalgain_min, $mp3gain_globalgain_max] = explode(',',
                            $thisfile_ape_items_current['data'][0]);
                        $thisfile_replaygain['mp3gain']['globalgain_track_min'] = intval($mp3gain_globalgain_min);
                        $thisfile_replaygain['mp3gain']['globalgain_track_max'] = intval($mp3gain_globalgain_max);
                    } else {
                        $this->warning('MP3gainMinMax value in APEtag appears invalid: "'
                            . $thisfile_ape_items_current['data'][0]
                            . '"');
                    }
                    break;
                ...
                case 'mp3gain_album_minmax':
                    if (preg_match('#^[0-9]{3},[0-9]{3}$#', $thisfile_ape_items_current['data'][0])) {
                        [$mp3gain_globalgain_album_min, $mp3gain_globalgain_album_max] = explode(',',
                            $thisfile_ape_items_current['data'][0]);
                        $thisfile_replaygain['mp3gain']['globalgain_album_min'] = intval($mp3gain_globalgain_album_min);
                        $thisfile_replaygain['mp3gain']['globalgain_album_max'] = intval($mp3gain_globalgain_album_max);
                    } else {
                        $this->warning('MP3gainAlbumMinMax value in APEtag appears invalid: "'
                            . $thisfile_ape_items_current['data'][0]
                            . '"');
                    }
                    break;
                ...
                case 'cover art (studio)':
                    ...
                    [$thisfile_ape_items_current['filename'], $thisfile_ape_items_current['data']] = explode("\x00",
                        $thisfile_ape_items_current['data'], 2);
                    $thisfile_ape_items_current['data_offset'] = $thisfile_ape_items_current['offset']
                    ...
                    do {
                        ...
                        if ($this->inline_attachments === true) {
                            ...
                            if ($this->inline_attachments < $thisfile_ape_items_current['data_length']) {
                                ...
                                $this->warning('attachment at '
                                    ...
                                    . number_format($thisfile_ape_items_current['data_length'])
                                    . ' bytes)');
                                unset($thisfile_ape_items_current['data']);
                                break;
                            }
                        ...
                        if (is_string($this->inline_attachments)) {
                            $destination_filename = $this->inline_attachments
                                . DIRECTORY_SEPARATOR
    ...
    public function APEcontentTypeFlagLookup($contenttypeid)
    {
        ...
        return (isset($APEcontentTypeFlagLookup[$contenttypeid]) ? $APEcontentTypeFlagLookup[$contenttypeid]
            : 'invalid');
    }
