...
class getid3_ogg extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if ($this->ftell() >= $this->getid3->fread_buffer_size()) {
            $this->error('Could not find start of Ogg page in the first '
        ...
        if (substr($filedata, 0, 4) == 'fLaC') {
            ...
            $info['ogg']['pageheader']['theora']['quality'] = ($info['ogg']['pageheader']['theora']['flags'] & 0xFC00)
                >> 10;
            $info['ogg']['pageheader']['theora']['kfg_shift'] = ($info['ogg']['pageheader']['theora']['flags'] & 0x03E0)
                >> 5;
            $info['ogg']['pageheader']['theora']['pixel_format_id'] = ($info['ogg']['pageheader']['theora']['flags']
                    & 0x0018) >> 3;
            $info['ogg']['pageheader']['theora']['reserved'] = ($info['ogg']['pageheader']['theora']['flags'] & 0x0007)
                >> 0; // should be 0
            ...
            if ($info['ogg']['pageheader']['theora']['frame_rate_denominator'] > 0) {
                $info['video']['frame_rate'] = (float)$info['ogg']['pageheader']['theora']['frame_rate_numerator']
                    / $info['ogg']['pageheader']['theora']['frame_rate_denominator'];
            }
            if ($info['ogg']['pageheader']['theora']['pixel_aspect_denominator'] > 0) {
                $info['video']['pixel_aspect_ratio'] = (float)$info['ogg']['pageheader']['theora']['pixel_aspect_numerator']
                    / $info['ogg']['pageheader']['theora']['pixel_aspect_denominator'];
            }
            $this->warning('Ogg Theora (v3) not fully supported in this version of getID3 ['
                . $this->getid3->version()
            ...
            $info['ogg']['skeleton']['fishead']['version'] = $info['ogg']['skeleton']['fishead']['raw']['version_major']
                . '.'
                . $info['ogg']['skeleton']['fishead']['raw']['version_minor'];
            $info['ogg']['skeleton']['fishead']['presentationtime'] = getid3_lib::SafeDiv($info['ogg']['skeleton']['fishead']['raw']['presentationtime_numerator'],
                $info['ogg']['skeleton']['fishead']['raw']['presentationtime_denominator']);
            $info['ogg']['skeleton']['fishead']['basetime'] = getid3_lib::SafeDiv($info['ogg']['skeleton']['fishead']['raw']['basetime_numerator'],
            ...
            do {
                ...
                if (substr($filedata, 0, 8) == "fisbone\x00") {
                    ...
                    $this->error('Ogg Theora (v1) not correctly handled in this version of getID3 ['
                        . $this->getid3->version()
                        . ']');
                    //break;
            ...
            $this->error('Ogg Skeleton not correctly handled in this version of getID3 ['
                . $this->getid3->version()
                . ']');
            //return false;
            ...
            $info['ogg']['flac']['header']['header_packets'] = getid3_lib::BigEndian2Int(substr($filedata, 7, 2))
            ...
            if ($info['ogg']['flac']['header']['magic'] != 'fLaC') {
                $this->error('Ogg-FLAC expecting "fLaC", found "'
                    . $info['ogg']['flac']['header']['magic']
                    . '" ('
                    . trim(getid3_lib::PrintHexBytes($info['ogg']['flac']['header']['magic']))
                    . ')');
                return false;
            }
            ...
            if (!empty($info['flac']['STREAMINFO']['sample_rate'])) {
                ...
                $info['playtime_seconds'] = getid3_lib::SafeDiv($info['flac']['STREAMINFO']['samples_stream'],
                    $info['flac']['STREAMINFO']['sample_rate']);
            }
        } else {
            $this->error('Expecting one of "vorbis", "Speex", "OpusHead", "vorbis", "fishhead", "theora", "fLaC" identifier strings, found "'
                . substr($filedata, 0, 8)
                . '"');
            unset($info['ogg']);
        ...
        if (!getid3_lib::intValueSupported($info['avdataend'])) {
            $this->warning('Unable to parse Ogg end chunk file (PHP does not support file operations beyond '
            ...
            if ($LastOggSpostion = strpos($LastChunkOfOgg, 'SggO')) {
                ...
                if (!empty($info['audio']['sample_rate'])) {
                    $info['ogg']['bitrate_average'] = (($info['avdataend'] - $info['avdataoffset']) * 8)
        ...
        if (isset($info['audio']['bitrate']) && !isset($info['playtime_seconds'])) {
            ...
            $info['playtime_seconds'] = (float)((($info['avdataend'] - $info['avdataoffset']) * 8)
                / $info['audio']['bitrate']);
        }
            ...
            if ($info['audio']['dataformat'] == 'vorbis') {
                ...
                if (preg_match('/^Xiph.Org/', $info['audio']['encoder'])) {
                    if ($info['audio']['bitrate_mode'] == 'abr') {
                        ...
                        $info['audio']['encoder_options'] = '-q '
                            . $this->get_quality_from_nominal_bitrate($info['ogg']['bitrate_nominal']);
                    }
                }

                if (empty($info['audio']['encoder_options']) && !empty($info['ogg']['bitrate_nominal'])) {
                    $info['audio']['encoder_options'] = 'Nominal bitrate: '
    ...
    public function ParseOpusPageHeader(&$filedata, &$filedataoffset, &$oggpageinfo)
    {
        ...
        $info['ogg']['pageheader']['opus']['input_sample_rate'] = getid3_lib::LittleEndian2Int(substr($filedata,
            $filedataoffset, 4));
        $filedataoffset += 4;
    ...
    public function ParseVorbisComments()
    {
        ...
        for ($i = 0; $i < $CommentsCount; $i++) {
            if ($i >= 10000) {
                ...
                $this->warning('Unexpectedly large number ('
                    . $CommentsCount
            ...
            $info['avdataoffset'] = $ThisFileInfo_ogg_comments_raw[$i]['dataoffset']
                + $ThisFileInfo_ogg_comments_raw[$i]['size']
                + 4;

            $commentdataoffset += 4;
            while ((strlen($commentdata) - $commentdataoffset) < $ThisFileInfo_ogg_comments_raw[$i]['size']) {
                if (($ThisFileInfo_ogg_comments_raw[$i]['size'] > $info['avdataend'])
                    || ($ThisFileInfo_ogg_comments_raw[$i]['size'] < 0)) {
                    $this->warning('Invalid Ogg comment size (comment #'
                        . $i
                        . ', claims to be '
                        . number_format($ThisFileInfo_ogg_comments_raw[$i]['size'])
                ...
                if ($oggpageinfo = $this->ParseOggPageHeader()) {
                    ...
                    if (!isset($info['ogg']['pageheader'][$VorbisCommentPage])) {
                        $this->warning('undefined Vorbis Comment page "'
                            . $VorbisCommentPage
                            . '" at offset '
                            . $this->ftell());
                        break;
                    ...
                    if ($readlength <= 0) {
                        $this->warning('invalid length Vorbis Comment page "'
                            . $VorbisCommentPage
                            . '" at offset '
                            . $this->ftell());
                        break;
            ...
            if (!$commentstring) {
                ...
                $this->warning('[known problem with CDex >= v1.40, < v1.50b7] Invalid Ogg comment name/value pair ['
                    . $i
                    . ']: '
                    . $commentstring);
            }
