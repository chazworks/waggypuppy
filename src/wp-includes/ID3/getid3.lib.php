...
class getid3_lib
{
    ...
    public static function BigEndian2Float($byteword)
    {
        ...
        if (($exponent == (pow(2, $exponentbits) - 1)) && ($fraction != 0)) {
            ...
            $floatvalue = pow(2, ($exponent - (pow(2, $exponentbits - 1) - 1))) * (1
    ...
    public static function BigEndian2Int($byteword, $synchsafe = false, $signed = false)
    {
        ...
        if ($signed && !$synchsafe) {
            ...
            if ($bytewordlen <= PHP_INT_SIZE) {
                ...
                throw new Exception('ERROR: Cannot have signed integers larger than '
                    . (8 * PHP_INT_SIZE)
                    . '-bits ('
                    . strlen($byteword)
    ...
    public static function Dec2Bin($number)
    {
        if (!is_numeric($number)) {
            ...
            trigger_error('TypeError: Dec2Bin(): Argument #1 ($number) must be numeric, ' . gettype($number) . ' given',
                E_USER_WARNING);
            return '';
        }
        $bytes = [];
        ...
        foreach ($bytes as $i => $byte) {
            $binstring = (($i == count($bytes) - 1) ? decbin($byte) : str_pad(decbin($byte), 8, '0', STR_PAD_LEFT))
                . $binstring;
        }
        return $binstring;
    }
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0, 1)) + (float)(self::BigEndian2Int(substr($rawdata, 1, 1))
                / pow(2, 8));
    }
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0, 2)) + (float)(self::BigEndian2Int(substr($rawdata, 2, 2))
                / pow(2, 16));
    }
    ...
    public static function XML2array($XMLstring)
    {
        if (function_exists('simplexml_load_string') && function_exists('libxml_disable_entity_loader')) {
            ...
            // https://core.trac.wp.org/changeset/29378
    ...
    public static function CopyFileParts($filename_source, $filename_dest, $offset, $length)
    {
        if (!self::intValueSupported($offset + $length)) {
            throw new Exception('cannot copy file portion, it extends beyond the '
            ...
            if (($fp_dest = fopen($filename_dest, 'wb'))) {
                if (fseek($fp_src, $offset) == 0) {
                    ...
                    while (($byteslefttowrite > 0)
                        && ($buffer = fread($fp_src, min($byteslefttowrite, getID3::FREAD_BUFFER_SIZE)))) {
    ...
    public static function iconv_fallback($in_charset, $out_charset, $string)
    {
        ...
        if (function_exists('mb_convert_encoding')) {
            if ((strtoupper($in_charset) == 'UTF-16') && (substr($string, 0, 2) != "\xFE\xFF")
                && (substr($string, 0, 2)
                    != "\xFF\xFE")) {
        ...
        if (!empty($ThisFileInfo['tags'])) {
                    ...
                    foreach ($tagdata as $key => $value) {
                        if (!empty($value)) {
                            if (empty($ThisFileInfo['comments'][$tagname])) {
                                ...
                                foreach ($ThisFileInfo['comments'][$tagname] as $existingkey => $existingvalue) {
                                    ...
                                    if (($newvaluelength <= $oldvaluelength)
                                        && (substr($existingvalue, 0, $newvaluelength) == trim($value))) {
                                        // new value is identical but shorter-than (or equal-length to) one already in comments - skip
                                        break 2;
                                    }

                                    if (function_exists('mb_convert_encoding')) {
                                        if (trim($value) == trim(substr(mb_convert_encoding($existingvalue,
                                                $ThisFileInfo['id3v1']['encoding'], $ThisFileInfo['encoding']), 0,
                                                30))) {
                                ...
                                foreach ($ThisFileInfo['comments'][$tagname] as $existingkey => $existingvalue) {
                                    ...
                                    if (($newvaluelengthMB == $oldvaluelengthMB)
                                        && ($existingvalue
                                            == getid3_lib::iconv_fallback('UTF-8', 'ASCII', $value))) {
                                        // https://github.com/JamesHeinrich/getID3/issues/338
                                    ...
                                    if ((strlen($existingvalue) > 10)
                                        && ($newvaluelength > $oldvaluelength)
                                        && (substr(trim($value), 0, strlen($existingvalue)) == $existingvalue)) {
                                        $ThisFileInfo['comments'][$tagname][$existingkey] = trim($value);
                                        break;
                                    }
                                }
                            ...
                            if (is_array($value)
                                ...
                                || !in_array(trim($value), $ThisFileInfo['comments'][$tagname])) {
            ...
            if (!empty($ThisFileInfo['comments'])) {
                ...
                foreach ($StandardizeFieldNames as $badkey => $goodkey) {
                    if (array_key_exists($badkey, $ThisFileInfo['comments'])
