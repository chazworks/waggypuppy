...
const EBML_ID_CONTENTENCKEYID = 0x07E2; //         [47][E2] -- For public key algorithms this is the ID of the public key the data was encrypted with.
...
const EBML_ID_STEREOMODE = 0x13B8; //         [53][B8] -- Stereo-3D video mode.
const EBML_ID_OLDSTEREOMODE = 0x13B9; //         [53][B9] -- Bogus StereoMode value used in old versions of libmatroska. DO NOT USE. (0: mono, 1: right eye, 2: left eye, 3: both eyes).
...
class getid3_matroska extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (isset($info['matroska']['info']) && is_array($info['matroska']['info'])) {
            foreach ($info['matroska']['info'] as $key => $infoarray) {
                if (isset($infoarray['Duration'])) {
                    ...
                    $info['playtime_seconds'] = $infoarray['Duration'] * ((isset($infoarray['TimecodeScale'])
            ...
            foreach ($info['matroska']['tracks']['tracks'] as $key => $trackarray) {
                ...
                switch ($trackarray['TrackType']) {
                    case 1: // Video
                        ...
                        $track_info['display_unit'] = self::displayUnit(isset($trackarray['DisplayUnit'])
                            ? $trackarray['DisplayUnit'] : 0);
                        $track_info['display_x'] = (isset($trackarray['DisplayWidth']) ? $trackarray['DisplayWidth']
                            : $trackarray['PixelWidth']);
                        $track_info['display_y'] = (isset($trackarray['DisplayHeight']) ? $trackarray['DisplayHeight']
                    ...
                    case 2: // Audio
                        $track_info['sample_rate'] = (isset($trackarray['SamplingFrequency'])
                        ...
                        switch ($trackarray['CodecID']) {
                            case 'A_PCM/INT/LIT':
                            case 'A_PCM/INT/BIG':
                                $track_info['bitrate'] = $track_info['sample_rate']
                                    * $track_info['channels']
                                    * $trackarray['BitDepth'];
                                break;
                            ...
                            case 'A_MPEG/L2':
                            case 'A_FLAC':
                                $module_dataformat = ($track_info['dataformat'] == 'mp2' ? 'mp3'
                                    : ($track_info['dataformat'] == 'eac3' ? 'ac3' : $track_info['dataformat']));
                                getid3_lib::IncludeDependency(GETID3_INCLUDEPATH
                                    . 'module.audio.'
                                    . $module_dataformat
                                    . '.php', __FILE__, true);

                                if (!isset($info['matroska']['track_data_offsets'][$trackarray['TrackNumber']])) {
                                    $this->warning('Unable to parse audio data ['
                                        . basename(__FILE__)
                                        . ':'
                                        . __LINE__
                                ...
                                if ($track_info['dataformat'][0] == 'm' || $track_info['dataformat'] == 'flac') {
                                    $getid3_temp->info['avdataend'] = $info['matroska']['track_data_offsets'][$trackarray['TrackNumber']]['offset']
                                        + $info['matroska']['track_data_offsets'][$trackarray['TrackNumber']]['length'];
                                }

                                // analyze
                                $class = 'getid3_' . $module_dataformat;
                                $header_data_key = $track_info['dataformat'][0] == 'm' ? 'mpeg'
                            ...
                            case 'A_AAC/MPEG4/LC/SBR':
                                $this->warning($trackarray['CodecID']
                            ...
                            default:
                                $this->warning('Unhandled audio type "' . (isset($trackarray['CodecID'])
                                        ? $trackarray['CodecID'] : '') . '"');
                                break;
                        }
        ...
        // process attachments
        if (isset($info['matroska']['attachments'])
            ...
            switch ($top_element['id']) {
                ...
                case EBML_ID_SEGMENT:
                    ...
                    while ($this->getEBMLelement($element_data, $top_element['end'])) {
                        if ($element_data['id'] != EBML_ID_CLUSTER
                            || !$this->hide_clusters) { // collect clusters only if required
                        ...
                        switch ($element_data['id']) {
                                    ...
                                    switch ($seek_entry['id']) {
                                        case EBML_ID_SEEK: // Contains a single seek entry to an EBML element
                                            while ($this->getEBMLelement($sub_seek_entry, $seek_entry['end'], true)) {
                                                switch ($sub_seek_entry['id']) {
                                                    ...
                                                    case EBML_ID_SEEKPOSITION:
                                                        $seek_entry['target_offset'] = $element_data['offset']
                                                            + getid3_lib::BigEndian2Int($sub_seek_entry['data']);
                                                        break;
                                            ...
                                            if (!isset($seek_entry['target_id'])) {
                                                $this->warning('seek_entry[target_id] unexpectedly not set at '
                                                    . $seek_entry['offset']);
                                                break;
                                            }
                                            if (($seek_entry['target_id'] != EBML_ID_CLUSTER)
                                                || !$this->hide_clusters) { // collect clusters only if required
                                            ...
                                            while ($this->getEBMLelement($subelement, $track_entry['end'], [
                                                ...
                                                switch ($subelement['id']) {
                                                    case EBML_ID_TRACKUID:
                                                        $track_entry[$subelement['id_name']] = getid3_lib::PrintHexBytes($subelement['data'],
                                                            true, false);
                                                        break;
                                ...
                                while ($this->getEBMLelement($subelement, $element_data['end'], true)) {
                                    switch ($subelement['id']) {
                                        ...
                                        case EBML_ID_DATEUTC:
                                            ...
                                            . '_unix'] = self::EBMLdate2unix($info_entry[$subelement['id_name']]);
                                            break;
                                                        ...
                                                        while ($this->getEBMLelement($sub_sub_subelement,
                                                            ...
                                                            switch ($sub_sub_subelement['id']) {
                                                                case EBML_ID_TARGETTYPEVALUE:
                                                                    ...
                                                                    $targets_entry[strtolower($sub_sub_subelement['id_name'])
                                                                    . '_long'] = self::TargetTypeValue($targets_entry[$sub_sub_subelement['id_name']]);
                                                                    break;
                                                                ...
                                                                case EBML_ID_TAGATTACHMENTUID:
                                                                    $targets_entry[$sub_sub_subelement['id_name']][] = getid3_lib::PrintHexBytes($sub_sub_subelement['data'],
                                                                        true, false);
                                                                    break;
                            ...
                            case EBML_ID_CLUSTER: // The lower level element containing the (monolithic) Block structure.
                                ...
                                if (!$this->parse_whole_file) {
                                    if (isset($info['matroska']['info']) && is_array($info['matroska']['info'])) {
                                        if (isset($info['matroska']['tracks']['tracks'])
                                            && is_array($info['matroska']['tracks']['tracks'])) {
                                            if (count($info['matroska']['track_data_offsets'])
                                                == count($info['matroska']['tracks']['tracks'])) {
                                                return;
                                            }
                                        }
                                    }
    ...
    private function getEBMLelement(&$element, $parent_end, $get_data = false)
    {
        ...
        // get raw data
        $dont_parse = (in_array($element['id'], $this->unuseful_elements)
            || $element['id_name']
            == dechex($element['id']));
    ...
    private function HandleEMBLClusterBlock($element, $block_type, &$info)
    {
        ...
        $block_data['flags']['lacing'] = (($block_data['flags_raw'] & 0x06)
            >> 1);  // 00=no lacing; 01=Xiph lacing; 11=EBML lacing; 10=fixed-size lacing
        if ($block_type == EBML_ID_CLUSTERSIMPLEBLOCK) {
        ...
        if ($block_data['flags']['lacing'] > 0) {
            $block_data['lace_frames'] = getid3_lib::BigEndian2Int($this->readEBMLelementData(1))
                + 1; // Number of frames in the lace-1 (uint8)
            if ($block_data['flags']['lacing'] != 0x02) {
                for ($i = 1; $i
                < $block_data['lace_frames']; $i++) { // Lace-coded size of each frame of the lace, except for the last one (multiple uint8). *This is not used with Fixed-size lacing as it is calculated automatically from (total size of lace) / (number of frames in lace).
                ...
                if ($block_data['flags']['lacing']
                    == 0x01) { // calc size of the last frame only for Xiph lacing, till EBML sizes are now anyway determined incorrectly
                    $block_data['lace_frames_size'][] = $element['end']
                        - $this->current_offset
        ...
        if (!isset($info['matroska']['track_data_offsets'][$block_data['tracknumber']])) {
            ...
            $info['matroska']['track_data_offsets'][$block_data['tracknumber']]['length'] = $element['end']
                - $this->current_offset;
