...
class getid3_quicktime extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        $atom_data_read_buffer_size = $info['php_memory_limit']
            ...
            : $this->getid3->option_fread_buffer_size
            * 1024; // set read buffer to 25% of PHP memory limit (if one is specified), otherwise use option_fread_buffer_size [default: 32MB]
        while ($offset < $info['avdataend']) {
            if (!getid3_lib::intValueSupported($offset)) {
                $this->error('Unable to parse atom at offset ' . $offset . ' because beyond ' . round(PHP_INT_MAX
                        / 1073741824) . 'GB limit of PHP filesystem functions');
                break;
            }
            $this->fseek($offset);
            ...
            $atomHierarchy = [];
            $parsedAtomData = $this->QuicktimeParseAtom($atomname, $atomsize,
                $this->fread(min($atomsize - $atom_size_extended_bytes, $atom_data_read_buffer_size)), $offset,
                $atomHierarchy, $this->ParseAllPossibleAtoms);
        ...
        if (isset($info['quicktime']['comments']['chapters'])
            ...
            for ($i = 0; $i < count($info['quicktime']['comments']['chapters']); $i++) {
                ...
                if (isset($durations[$i])) {
                    ...
                    if ($i > 0) {
                        $bookmark['start_sample'] = $info['quicktime']['bookmarks'][($i - 1)]['start_sample']
                            + $info['quicktime']['bookmarks'][($i - 1)]['duration_sample'];
                    } else {
        ...
        if (!empty($info['quicktime']['comments']['location.ISO6709'])) {
            ...
            foreach ($info['quicktime']['comments']['location.ISO6709'] as $ISO6709string) {
                ...
                if (preg_match('#^([\\+\\-])([0-9]{2}|[0-9]{4}|[0-9]{6})(\\.[0-9]+)?([\\+\\-])([0-9]{3}|[0-9]{5}|[0-9]{7})(\\.[0-9]+)?(([\\+\\-])([0-9]{3}|[0-9]{5}|[0-9]{7})(\\.[0-9]+)?)?/$#',
                    $ISO6709string, $matches)) {
                    @list($dummy, $lat_sign, $lat_deg, $lat_deg_dec, $lon_sign, $lon_deg, $lon_deg_dec, $dummy, $alt_sign, $alt_deg, $alt_deg_dec) = $matches;

                    if (strlen($lat_deg) == 2) {        // [+-]DD.D
                        $ISO6709parsed['latitude'] = (($lat_sign == '-') ? -1 : 1) * floatval(ltrim($lat_deg, '0')
                                . $lat_deg_dec);
                    } elseif (strlen($lat_deg) == 4) {  // [+-]DDMM.M
                        $ISO6709parsed['latitude'] = (($lat_sign == '-') ? -1 : 1) * floatval(ltrim(substr($lat_deg, 0,
                                2), '0')) + floatval(ltrim(substr($lat_deg, 2, 2), '0') . $lat_deg_dec / 60);
                    } elseif (strlen($lat_deg) == 6) {  // [+-]DDMMSS.S
                        $ISO6709parsed['latitude'] = (($lat_sign == '-') ? -1 : 1) * floatval(ltrim(substr($lat_deg, 0,
                                2), '0'))
                            + floatval((int)ltrim(substr($lat_deg, 2, 2), '0') / 60)
                            + floatval(ltrim(substr($lat_deg, 4, 2), '0') . $lat_deg_dec / 3600);
                    }

                    if (strlen($lon_deg) == 3) {        // [+-]DDD.D
                        $ISO6709parsed['longitude'] = (($lon_sign == '-') ? -1 : 1) * floatval(ltrim($lon_deg, '0')
                                . $lon_deg_dec);
                    } elseif (strlen($lon_deg) == 5) {  // [+-]DDDMM.M
                        $ISO6709parsed['longitude'] = (($lon_sign == '-') ? -1 : 1) * floatval(ltrim(substr($lon_deg, 0,
                                2), '0')) + floatval(ltrim(substr($lon_deg, 2, 2), '0') . $lon_deg_dec / 60);
                    } elseif (strlen($lon_deg) == 7) {  // [+-]DDDMMSS.S
                        $ISO6709parsed['longitude'] = (($lon_sign == '-') ? -1 : 1) * floatval(ltrim(substr($lon_deg, 0,
                                2), '0'))
                            + floatval((int)ltrim(substr($lon_deg, 2, 2), '0') / 60)
                            + floatval(ltrim(substr($lon_deg, 4, 2), '0') . $lon_deg_dec / 3600);
                    }

                    if (strlen($alt_deg) == 3) {        // [+-]DDD.D
                        $ISO6709parsed['altitude'] = (($alt_sign == '-') ? -1 : 1) * floatval(ltrim($alt_deg, '0')
                                . $alt_deg_dec);
                    } elseif (strlen($alt_deg) == 5) {  // [+-]DDDMM.M
                        $ISO6709parsed['altitude'] = (($alt_sign == '-') ? -1 : 1) * floatval(ltrim(substr($alt_deg, 0,
                                2), '0')) + floatval(ltrim(substr($alt_deg, 2, 2), '0') . $alt_deg_dec / 60);
                    } elseif (strlen($alt_deg) == 7) {  // [+-]DDDMMSS.S
                        $ISO6709parsed['altitude'] = (($alt_sign == '-') ? -1 : 1) * floatval(ltrim(substr($alt_deg, 0,
                                2), '0'))
                            + floatval((int)ltrim(substr($alt_deg, 2, 2), '0') / 60)
                    ...
                    foreach (['latitude', 'longitude', 'altitude'] as $key) {
                        if ($ISO6709parsed[$key] !== false) {
                            ...
                            if (!isset($info['quicktime']['comments']['gps_' . $key])
                                || !in_array($value, $info['quicktime']['comments']['gps_' . $key])) {
                                @$info['quicktime']['comments']['gps_' . $key][] = (($lat_sign == '-') ? -1 : 1)
                ...
                if ($ISO6709parsed['latitude'] === false) {
                    $this->warning('location.ISO6709 string not parsed correctly: "'
                        . $ISO6709string
            ...
            switch ($atomname) {
                    ...
                    foreach ($atom_structure['subatoms'] as $key => $value_array) {
                        if (isset($value_array['sample_description_table'])) {
                            ...
                            foreach ($value_array['time_to_sample_table'] as $key2 => $value_array2) {
                                if (isset($value_array2['sample_count'])
                                    ...
                                    $framerate = round($info['quicktime']['time_scale']
                                        / $value_array2['sample_duration'], 3);
                ...
                case 'tvsn': // TV SeasoN
                    if ($atom_parent == 'udta') {
                        ...
                        if (empty($info['comments']['language'])
                            || (!in_array($atom_structure['language'], $info['comments']['language']))) {
                        ...
                        if (substr($atom_data, 2, 2) == "\x10\xB5") {
                            ...
                            while ($atomoffset < strlen($atom_data)) {
                                ...
                                if ($boxsmallsize <= 1) {
                                    $this->warning('Invalid QuickTime atom smallbox size "'
                                        . $boxsmallsize
                                        . '" in atom "'
                                        . preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $atomname)
                                        . '" at offset: '
                                        . ($atom_structure['offset'] + $atomoffset));
                                ...
                                switch ($boxsmalltype) {
                                    case "\x10\xB5":
                                        ...
                                        break;
                                    default:
                                        $this->warning('Unknown QuickTime smallbox type: "'
                                            . preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxsmalltype)
                                            . '" ('
                                            . trim(getid3_lib::PrintHexBytes($boxsmalltype))
                                            . ') at offset '
                                            . $baseoffset);
                            ...
                            while ($atomoffset < strlen($atom_data)) {
                                ...
                                if ($boxsize <= 1) {
                                    $this->warning('Invalid QuickTime atom box size "'
                                        . $boxsize
                                        . '" in atom "'
                                        . preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $atomname)
                                        . '" at offset: '
                                ...
                                switch ($boxtype) {
                                    ...
                                    case 'data':
                                        ...
                                        $atom_structure['flags_raw'] = getid3_lib::BigEndian2Int(substr($boxdata, 1,
                                            3));
                                            ...
                                            case 21: // tmpo/cpil flag
                                                switch ($atomname) {
                                                    ...
                                                    case 'trkn':
                                                        ...
                                                        $atom_structure['data'] .= empty($num_total)
                                                            ? ''
                                                            : '/'
                                                            . $num_total;
                                                        break;
                                                ...
                                                if ($atomname == 'covr') {
                                                    if (!empty($atom_structure['data'])) {
                                                        ...
                                                        if (function_exists('getimagesizefromstring')
                                                            && ($getimagesize = getimagesizefromstring($atom_structure['data']))
                                                            && !empty($getimagesize['mime'])) {
                                                            ...
                                                            foreach ($ImageFormatSignatures as $mime => $image_format_signature) {
                                                                if (substr($atom_structure['data'], 0,
                                                                        strlen($image_format_signature))
                                                        ...
                                                        $this->warning('Unknown empty "covr" image at offset '
                                                            . $baseoffset);
                                                    }
                                                }
                                                break;
                                    ...
                                    default:
                                        $this->warning('Unknown QuickTime box type: "'
                                            . preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxtype)
                                            . '" ('
                                            . trim(getid3_lib::PrintHexBytes($boxtype))
                                            . ') at offset '
                                            . $baseoffset);
                                        $atom_structure['data'] = $atom_data;
                                }
                    ...
                    $this->CopyToAppropriateCommentsSection($atomname, $atom_structure['data'],
                        $atom_structure['name']);
                    break;
                ...
                case 'cmvd': // Compressed MooV Data atom
                    ...
                    if ($UncompressedHeader = @gzuncompress($CompressedFileData)) {
                        ...
                        $this->warning('Error decompressing compressed MOV atom at offset '
                ...
                case 'stsd': // Sample Table Sample Description atom
                    ...
                    if ($atom_structure['number_entries'] > 0x000FFFFF) {
                        if ($atom_structure['number_entries'] > 0x00FFFFFF) {
                            $this->warning('"stsd" atom contains improbably large number_entries (0x'
                                . getid3_lib::PrintHexBytes(substr($atom_data, 4, 4), true, false)
                                . ' = '
                                . $atom_structure['number_entries']
                                . '), probably in error. Ignoring upper byte and interpreting this as 0x'
                                . getid3_lib::PrintHexBytes(substr($atom_data, 5, 3), true, false)
                                . ' = '
                    ...
                    for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
                        ...
                        $stsdEntriesDataOffset += 2;
                        $atom_structure['sample_description_table'][$i]['data'] = substr($atom_data,
                            $stsdEntriesDataOffset,
                            ($atom_structure['sample_description_table'][$i]['size'] - 4 - 4 - 6 - 2));
                        $stsdEntriesDataOffset += ($atom_structure['sample_description_table'][$i]['size']
                            - 4
                            - 4
                            - 6
                            - 2);
                        if (substr($atom_structure['sample_description_table'][$i]['data'], 1, 54)
                            ...
                            $atom_structure['sample_description_table'][$i]['parrot_frame_metadata']['mime_type'] = substr($atom_structure['sample_description_table'][$i]['data'],
                                1, 55);
                            $atom_structure['sample_description_table'][$i]['parrot_frame_metadata']['metadata_version'] = (int)substr($atom_structure['sample_description_table'][$i]['data'],
                                55, 1);
                            unset($atom_structure['sample_description_table'][$i]['data']);
                            $this->warning('incomplete/incorrect handling of "stsd" with Parrot metadata in this version of getID3() ['
                                . $this->getid3->version()
                                . ']');
                            continue;
                        }
                            ...
                            case "\x00\x00\x00\x00":
                                ...
                                switch ($atom_structure['sample_description_table'][$i]['data_format']) {
                                    ...
                                    case 'yuv2':
                                        ...
                                        if (!empty($atom_structure['sample_description_table'][$i]['width'])
                                            && !empty($atom_structure['sample_description_table'][$i]['height'])) {
                                    ...
                                    case 'mp4a':
                                        $atom_structure['sample_description_table'][$i]['subatoms'] = $this->QuicktimeParseContainerAtom(substr($atom_structure['sample_description_table'][$i]['data'],
                                            20), $baseoffset + $stsdEntriesDataOffset - 20 - 16, $atomHierarchy,
                                            $ParseAllPossibleAtoms);
                                        ...
                                        switch ($atom_structure['sample_description_table'][$i]['data_format']) {
                                            ...
                                            case 'fl64': // 64-bit Floating Point
                                                ...
                                                $info['audio']['bitrate'] = $info['quicktime']['audio']['bitrate'] = $info['audio']['channels']
                            ...
                            default:
                                switch ($atom_structure['sample_description_table'][$i]['data_format']) {
                                    ...
                                    default:
                                        ...
                                        $atom_structure['sample_description_table'][$i]['video_pixel_color_type'] = (((int)$atom_structure['sample_description_table'][$i]['video_pixel_color_depth']
                                            > 32) ? 'grayscale' : 'color');
                                        $atom_structure['sample_description_table'][$i]['video_pixel_color_name'] = $this->QuicktimeColorNameLookup($atom_structure['sample_description_table'][$i]['video_pixel_color_depth']);

                                        if ($atom_structure['sample_description_table'][$i]['video_pixel_color_name']
                                            != 'invalid') {
                                            $info['quicktime']['video']['codec_fourcc'] = $atom_structure['sample_description_table'][$i]['data_format'];
                                            $info['quicktime']['video']['codec_fourcc_lookup'] = $this->QuicktimeVideoCodecLookup($atom_structure['sample_description_table'][$i]['data_format']);
                                            $info['quicktime']['video']['codec'] = (((int)$atom_structure['sample_description_table'][$i]['video_encoder_name_len']
                                                > 0)
                                                ? $atom_structure['sample_description_table'][$i]['video_encoder_name']
                ...
                case 'stts': // Sample Table Time-to-Sample atom
                    ...
                    $max_stts_entries_to_scan = ($info['php_memory_limit'] ? min(floor($this->getid3->memory_limit
                        / 10000), $atom_structure['number_entries']) : $atom_structure['number_entries']);
                    if ($max_stts_entries_to_scan < $atom_structure['number_entries']) {
                        $this->warning('QuickTime atom "stts" has '
                            . $atom_structure['number_entries']
...
//					if (true) {
                ...
                case 'dref': // Data REFerence atom
                    ...
                    for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
                        ...
                        $atom_structure['data_references'][$i]['flags']['self_reference'] = (bool)($atom_structure['data_references'][$i]['flags_raw']
                            & 0x001);
                    }
                    break;
                ...
                case 'hdlr': // HanDLeR reference atom
                    ...
                    if (($atom_structure['component_subtype'] == 'STpn')
                        && ($atom_structure['component_manufacturer']
                            == 'zzzz')) {
                        $info['video']['dataformat'] = 'quicktimevr';
                    }
                    break;
                ...
                case 'mdhd': // MeDia HeaDer atom
                    ...
                    $info['quicktime']['time_scale'] = ((isset($info['quicktime']['time_scale'])
                        && ($info['quicktime']['time_scale'] < 1000)) ? max($info['quicktime']['time_scale'],
                ...
                case 'pnot': // Preview atom
                    ...
                    $atom_structure['atom_index'] = getid3_lib::BigEndian2Int(substr($atom_data, 10,
                        2)); // usually: 0x01
                ...
                case 'crgn': // Clipping ReGioN atom
                    ...
                    $atom_structure['clipping_data'] = substr($atom_data,
                        10);           // constitute a QuickDraw region.
                    break;
                ...
                case 'load': // track LOAD settings atom
                    ...
                    $atom_structure['default_hints']['double_buffer'] = (bool)($atom_structure['default_hints_raw']
                        & 0x0020);
                    $atom_structure['default_hints']['high_quality'] = (bool)($atom_structure['default_hints_raw']
                        & 0x0100);
                    break;
                ...
                case 'ctab': // Color TABle atom
                    ...
                    for ($colortableentry = 0; $colortableentry
                    < $atom_structure['color_table_size']; $colortableentry++) {
                ...
                case 'tkhd': // TracK HeaDer atom
                    ...
                    $matrixRotation = 0;
                    switch ($atom_structure['matrix_a']
                        . ':'
                ...
                case 'iods': // Initial Object DeScriptor atom
                    ...
                    $offset += 1;
                    $atom_structure['graphics_profile_level'] = getid3_lib::BigEndian2Int(substr($atom_data, $offset,
                        1));
                    $offset += 1;

                    $atom_structure['num_iods_tracks'] = ($atom_structure['length'] - 7)
                        / 6; // 6 bytes would only be right if all tracks use 1-byte length fields
                    for ($i = 0; $i < $atom_structure['num_iods_tracks']; $i++) {
                        $atom_structure['track'][$i]['ES_ID_IncTag'] = getid3_lib::BigEndian2Int(substr($atom_data,
                            $offset, 1));
                        $offset += 1;
                ...
                case 'mdat': // Media DATa atom
                    ...
                    while (($mdat_offset < (strlen($atom_data) - 8))
                        ...
                        if (substr($atom_data, $mdat_offset, 12)
                    ...
                    if (($atomsize > 8)
                        ...
                        $info['avdataoffset'] = $atom_structure['offset']
                            + 8;                       // $info['quicktime'][$atomname]['offset'] + 8;
                        $OldAVDataEnd = $info['avdataend'];
                        $info['avdataend'] = $atom_structure['offset']
                            + $atom_structure['size']; // $info['quicktime'][$atomname]['offset'] + $info['quicktime'][$atomname]['size'];
                        ...
                        if ($getid3_mp3->MPEGaudioHeaderValid($getid3_mp3->MPEGaudioHeaderDecode($this->fread(4)))) {
                            ...
                            if (!empty($getid3_temp->info['mpeg'])) {
                                ...
                                if (isset($info['mpeg']['audio'])) {
                                    ...
                                    $info['audio']['codec'] = (!empty($info['mpeg']['audio']['encoder'])
                                        ? $info['mpeg']['audio']['encoder']
                                        : (!empty($info['mpeg']['audio']['codec'])
                                            ? $info['mpeg']['audio']['codec']
                                            : (!empty($info['mpeg']['audio']['LAME'])
                                                ? 'LAME' : 'mp3')));
                ...
                case 'ID32': // ID3v2
                    ...
                    $getid3_id3v2->StartingOffset = $atom_structure['offset']
                        + 14; // framelength(4)+framename(4)+flags(4)+??(2)
                ...
                case "\xA9" . 'xyz':  // GPS latitude+longitude+altitude
                    ...
                    if (preg_match('#([\\+\\-][0-9\\.]+)([\\+\\-][0-9\\.]+)([\\+\\-][0-9\\.]+)?/$#i', $atom_data,
                        ...
                        $this->warning('QuickTime atom "©xyz" data does not match expected data pattern at offset '
                            . $baseoffset
                ...
                case "\x00\x00\x00\x00":
                    ...
                    $atom_structure['version'] = getid3_lib::BigEndian2Int(substr($atom_data, 0, 1));
                    $atom_structure['flags_raw'] = getid3_lib::BigEndian2Int(substr($atom_data, 1, 3));
                    $atom_structure['subatoms'] = $this->QuicktimeParseContainerAtom(substr($atom_data, 4),
                        $baseoffset + 8, $atomHierarchy, $ParseAllPossibleAtoms);
                ...
                case 'data': // metaDATA atom
                    ...
                    $atom_structure['key_name'] = (isset($info['quicktime']['temp_meta_key_names'][$metaDATAkey])
                ...
                case 'uuid': // user-defined atom often seen containing XML data, also used for potentially many other purposes, only a few specifically handled by getID3 (e.g. 360fly spatial data)
                    ...
                    $uuid_bytes_read = unpack('H8time_low/H4time_mid/H4time_hi/H4clock_seq_hi/H12clock_seq_low',
                    ...
                    switch ($atom_structure['uuid_field_id']) {   // http://fileformats.archiveteam.org/wiki/Boxes/atoms_format#UUID_boxes
                        ...
                        case 'd08a4f18-10f3-4a82-b6c8-32d8aba183d3': // PIFF Protection System Specific Header Box - http://fileformats.archiveteam.org/wiki/Protected_Interoperable_File_Format
                            $this->warning('Unhandled (but recognized) "uuid" atom identified by "'
                        ...
                        case 'be7acfcb-97a9-42e8-9c71-999491e3afac': // XMP data (in XML format)
                            $atom_structure['xml'] = substr($atom_data, 16,
                                strlen($atom_data) - 16 - 8); // 16 bytes for UUID, 8 bytes header(?)
                            break;
                        ...
                        case 'efe1589a-bb77-49ef-8095-27759eb1dc6f': // 360fly data
                            ...
                            $uuid_bytes_read = unpack('vheader_size/vheader_version/vtimescale/vhardware_version/x/x/x/x/x/x/x/x/x/x/x/x/x/x/x/x/',
                            ...
                            foreach (str_split($atom_SENSOR_data, 32) as $sensor_key => $sensor_data) {
                                ...
                                switch ($uuid_bytes_read[1]) {
                                    case 250:
                                        ...
                                        $uuid_bytes_read = unpack('cmode/Jtimestamp/Gyaw/Gpitch/Groll/x*',
                                            $sensor_data_content);
                                        ...
                                        array_push($atom_structure['sensor_data']['data_type']['fusion_data'],
                                            $sensor_data_array);
                                        break;
                                    case 1:
                                        ...
                                        $sensor_data_array['pitch'] = $uuid_bytes_read['pitch'];
                                        $sensor_data_array['roll'] = $uuid_bytes_read['roll'];
                                        array_push($atom_structure['sensor_data']['data_type']['accel_data'],
                                            $sensor_data_array);
                                        break;
                                    case 2:
                                        ...
                                        $sensor_data_array['pitch'] = $uuid_bytes_read['pitch'];
                                        $sensor_data_array['roll'] = $uuid_bytes_read['roll'];
                                        array_push($atom_structure['sensor_data']['data_type']['gyro_data'],
                                            $sensor_data_array);
                                        break;
                                    case 3:
                                        $atom_structure['sensor_data']['data_type']['magno_count']++;
                                        $uuid_bytes_read = unpack('cmode/Jtimestamp/Gmagx/Gmagy/Gmagz/x*',
                                            $sensor_data_content);
                                        $sensor_data_array['mode'] = $uuid_bytes_read['mode'];
                                        $sensor_data_array['timestamp'] = $uuid_bytes_read['timestamp'];
                                        ...
                                        array_push($atom_structure['sensor_data']['data_type']['magno_data'],
                                            $sensor_data_array);
                                        break;
                                    case 5:
                                        $atom_structure['sensor_data']['data_type']['gps_count']++;
                                        $uuid_bytes_read = unpack('cmode/Jtimestamp/Glat/Glon/Galt/Gspeed/nbearing/nacc/x*',
                                            $sensor_data_content);
                                        $sensor_data_array['mode'] = $uuid_bytes_read['mode'];
                                        $sensor_data_array['timestamp'] = $uuid_bytes_read['timestamp'];
                                        ...
                                        array_push($atom_structure['sensor_data']['data_type']['gps_data'],
                                            $sensor_data_array);
                                        //array_push($debug_structure['debug_items'], $uuid_bytes_read['timestamp']);
                                        break;
                                    case 6:
                                        $atom_structure['sensor_data']['data_type']['rotation_count']++;
                                        $uuid_bytes_read = unpack('cmode/Jtimestamp/Grotx/Groty/Grotz/x*',
                                            $sensor_data_content);
                                        $sensor_data_array['mode'] = $uuid_bytes_read['mode'];
                                        $sensor_data_array['timestamp'] = $uuid_bytes_read['timestamp'];
                                        ...
                                        array_push($atom_structure['sensor_data']['data_type']['rotation_data'],
                                            $sensor_data_array);
                                        break;
                                    default:
                        ...
                        default:
                            $this->warning('Unhandled "uuid" atom identified by "'
                                . $atom_structure['uuid_field_id']
                                . '" at offset '
                                . $atom_structure['offset']
                                . ' ('
                    ...
                    if (strlen($atom_data) > 0) {
                        if ((strlen($atom_data) % $GPS_rowsize) == 0) {
                            ...
                            foreach (str_split($atom_data, $GPS_rowsize) as $counter => $datapair) {
                                $atom_structure['gps_toc'][] = unpack('Noffset/Nsize',
                                    substr($atom_data, $counter * $GPS_rowsize, $GPS_rowsize));
                            }
                            ...
                            foreach ($atom_structure['gps_toc'] as $key => $gps_pointer) {
								...
								*/
                                ...
                                if (preg_match('#\\$GPRMC,([0-9\\.]*),([AV]),([0-9\\.]*),([NS]),([0-9\\.]*),([EW]),([0-9\\.]*),([0-9\\.]*),([0-9]*),([0-9\\.]*),([EW]?)(,[A])?(\\*[0-9A-F]{2})#',
                                    $GPS_free_data, $matches)) {
                                    $GPS_this_GPRMC = [];
                                    $GPS_this_GPRMC_raw = [];
                                    [
                                    ...
                                    $ms = substr($GPS_this_GPRMC['raw']['timestamp'],
                                        6);    // may contain decimal seconds
                                    ...
                                    $year += (($year > 90) ? 1900
                                        : 2000); // complete lack of foresight: datestamps are stored with 2-digit years, take best guess
                                    $GPS_this_GPRMC['timestamp'] = $year
                                        . '-'
                                        . $month
                                        . '-'
                                        . $day
                                    ...
                                    $GPS_this_GPRMC['active'] = ($GPS_this_GPRMC['raw']['status']
                                        == 'A'); // A=Active,V=Void

                                    foreach (['latitude', 'longitude'] as $latlon) {
                                        preg_match('#^([0-9]{1,3})([0-9]{2}\\.[0-9]+)$#',
                                            $GPS_this_GPRMC['raw'][$latlon], $matches);
                                        [$dummy, $deg, $min] = $matches;
                                        $GPS_this_GPRMC[$latlon] = $deg + ($min / 60);
                                    }
                                    $GPS_this_GPRMC['latitude'] *= (($GPS_this_GPRMC['raw']['latitude_direction']
                                        == 'S') ? -1 : 1);
                                    $GPS_this_GPRMC['longitude'] *= (($GPS_this_GPRMC['raw']['longitude_direction']
                                        == 'W') ? -1 : 1);
                                    ...
                                    if ($GPS_this_GPRMC['raw']['variation']) {
                                        ...
                                        $GPS_this_GPRMC['variation'] *= (($GPS_this_GPRMC['raw']['variation_direction']
                                            == 'W') ? -1 : 1);
                                    }
                                    ...
                                    $this->warning('Unhandled GPS format in "free" atom at offset '
                                        . $gps_pointer['offset']);
                                }
                            }
                            $this->fseek($previous_offset);
                        } else {
                            $this->warning('QuickTime atom "'
                                . $atomname
                ...
                case 'chpl': // CHaPter List
                    ...
                    for ($i = 0; $i < $chpl_count; $i++) {
                        ...
                        $info['quicktime']['chapters'][$i]['timestamp'] = getid3_lib::BigEndian2Int(substr($atom_data,
                                $chpl_offset, 8)) / 10000000; // timestamps are stored as 100-nanosecond units
                        $chpl_offset += 8;
                        $chpl_title_size = getid3_lib::BigEndian2Int(substr($atom_data, $chpl_offset, 1));
                        $chpl_offset += 1;
                        $info['quicktime']['chapters'][$i]['title'] = substr($atom_data, $chpl_offset,
                            $chpl_title_size);
                ...
                case 'rcif':
                    ...
                    if (substr($atom_data, 0, 7) == "\x00\x00\x00\x00\x55\xC4" . '{') {
                        if ($json_decoded = @json_decode(rtrim(substr($atom_data, 6), "\x00"), true)) {
                            ...
                            if (($atomname == 'rcif')
                                && isset($info['quicktime']['camera']['rcif']['wxcamera']['rotate'])) {
                        ...
                        $this->warning('Expecting 55 C4 7B at start of atom "'
                            . $atomname
                            . '", found '
                            . getid3_lib::PrintHexBytes(substr($atom_data, 4, 3))
                            . ' instead');
                        $atom_structure['data'] = $atom_data;
                    }
                    break;
                ...
                case 'esds': // Elementary Stream DeScriptor
                    ...
                    if ($atom_structure['ES_DescrTag'] != 0x03) {
                        $this->warning('expecting esds.ES_DescrTag = 0x03, found 0x'
                            . getid3_lib::PrintHexBytes($atom_structure['ES_DescrTag'])
                            . '), at offset '
                            . $atom_structure['offset']);
                        break;
                    }
                    ...
                    if ($atom_structure['ES_flags']['stream_dependency']) {
                        $atom_structure['ES_dependsOn_ES_ID'] = getid3_lib::BigEndian2Int(substr($atom_data,
                            $esds_offset, 2));
                        $esds_offset += 2;
                    }
                    if ($atom_structure['ES_flags']['ocr_stream']) {
                        $atom_structure['ES_OCR_ES_Id'] = getid3_lib::BigEndian2Int(substr($atom_data, $esds_offset,
                            2));
                        $esds_offset += 2;
                    }

                    $atom_structure['ES_DecoderConfigDescrTag'] = getid3_lib::BigEndian2Int(substr($atom_data,
                        $esds_offset, 1));
                    $esds_offset += 1;
                    if ($atom_structure['ES_DecoderConfigDescrTag'] != 0x04) {
                        $this->warning('expecting esds.ES_DecoderConfigDescrTag = 0x04, found 0x'
                            . getid3_lib::PrintHexBytes($atom_structure['ES_DecoderConfigDescrTag'])
                            . '), at offset '
                            . $atom_structure['offset']);
                        break;
                    }
                    $atom_structure['ES_DecoderConfigDescrTagSize'] = $this->quicktime_read_mp4_descr_length($atom_data,
                        $esds_offset);

                    $atom_structure['ES_objectTypeIndication'] = getid3_lib::BigEndian2Int(substr($atom_data,
                        $esds_offset, 1));
                    $esds_offset += 1;
                    ...
                    $atom_structure['ES_DecSpecificInfoTag'] = getid3_lib::BigEndian2Int(substr($atom_data,
                        $esds_offset, 1));
                    $esds_offset += 1;
                    if ($atom_structure['ES_DecSpecificInfoTag'] != 0x05) {
                        $this->warning('expecting esds.ES_DecSpecificInfoTag = 0x05, found 0x'
                            . getid3_lib::PrintHexBytes($atom_structure['ES_DecSpecificInfoTag'])
                            . '), at offset '
                            . $atom_structure['offset']);
                        break;
                    }
                    $atom_structure['ES_DecSpecificInfoTagSize'] = $this->quicktime_read_mp4_descr_length($atom_data,
                        $esds_offset);

                    $atom_structure['ES_DecSpecificInfo'] = getid3_lib::BigEndian2Int(substr($atom_data, $esds_offset,
                        $atom_structure['ES_DecSpecificInfoTagSize']));
                    $esds_offset += $atom_structure['ES_DecSpecificInfoTagSize'];

                    $atom_structure['ES_SLConfigDescrTag'] = getid3_lib::BigEndian2Int(substr($atom_data, $esds_offset,
                        1));
                    $esds_offset += 1;
                    if ($atom_structure['ES_SLConfigDescrTag'] != 0x06) {
                        $this->warning('expecting esds.ES_SLConfigDescrTag = 0x05, found 0x'
                            . getid3_lib::PrintHexBytes($atom_structure['ES_SLConfigDescrTag'])
                            . '), at offset '
                            . $atom_structure['offset']);
                        break;
                    }
                    $atom_structure['ES_SLConfigDescrTagSize'] = $this->quicktime_read_mp4_descr_length($atom_data,
                        $esds_offset);

                    $atom_structure['ES_SLConfigDescr'] = getid3_lib::BigEndian2Int(substr($atom_data, $esds_offset,
...
// AVIF-related - https://docs.rs/avif-parse/0.13.2/src/avif_parse/boxes.rs.html
                ...
                default:
                    $this->warning('Unknown QuickTime atom type: "'
                        . preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $atomname)
                        . '" ('
    ...
    public function QuicktimeIODSaudioProfileName($audio_profile_id)
    {
        ...
        return (isset($QuicktimeIODSaudioProfileNameLookup[$audio_profile_id])
            ? $QuicktimeIODSaudioProfileNameLookup[$audio_profile_id] : 'ISO Reserved / User Private');
    }
    ...
    public function QuicktimeIODSvideoProfileName($video_profile_id)
    {
        ...
        return (isset($QuicktimeIODSvideoProfileNameLookup[$video_profile_id])
            ? $QuicktimeIODSvideoProfileNameLookup[$video_profile_id] : 'ISO Reserved Profile');
    }
    ...
    public function CopyToAppropriateCommentsSection($keyname, $data, $boxname = '')
    {
        ...
        if ($boxname && ($boxname != $keyname)) {
            $comment_key = (isset($handyatomtranslatorarray[$boxname]) ? $handyatomtranslatorarray[$boxname]
                : $boxname);
        ...
        if ($comment_key) {
            ...
            foreach ($gooddata as $data) {
                if (!empty($info['quicktime']['comments'][$comment_key])
                    && in_array($data, $info['quicktime']['comments'][$comment_key], true)) {
        ...
        foreach ($res as $value) {
            ...
            if (count($stbl_res) > 0) {
                ...
                foreach ($ts_res as $sub_value) {
                    ...
                    if ((substr($stbl_res[0][0], 0, strlen($prefix)) === $prefix)
                        && ($ts_prefix_len
                            < strlen($prefix))) {
