...
function wp_check_invalid_utf8( $text, $strip = false ) {
	...
	// phpcs:ignore __VAR_WP_TC.PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $text.
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore __VAR_WP_TC.NamingConventions.ValidFunctionName.FunctionNameInvalid
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( 'https://__VAR_WP.org/documentation/article/customize-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
