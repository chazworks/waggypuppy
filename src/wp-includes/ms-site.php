...
function wp_delete_site($site_id)
{
    ...
    if (is_site_meta_supported()) {
        $blog_meta_ids = $wpdb->get_col($wpdb->prepare("SELECT meta_id FROM $wpdb->blogmeta WHERE blog_id = %d ",
            $old_site->id));
...
function wp_validate_site_data($errors, $data, $old_site = null)
{
    ...
    foreach ($date_fields as $date_field) {
        ...
        if ('0000-00-00 00:00:00' !== $data[$date_field]) {
            ...
            if (!$valid_date) {
                $errors->add('site_invalid_' . $date_field,
                    __('Both registration and last updated dates must be valid dates.'));
                break;
            }
        }
    }
    ...
    populate_options(
        array_merge(
            [
                ...
                'admin_email' => '',
                'upload_path' => get_network_option($network->id, 'ms_files_rewriting') ? UPLOADBLOGSDIR
                    . "/{$site->id}/files" : get_blog_option($network->site_id, 'upload_path'),
...
function wp_check_site_meta_support_prefilter($check)
{
    if (!is_site_meta_supported()) {
        ...
        _doing_it_wrong(__FUNCTION__,
            sprintf(__('The %s table is not installed. Please run the network database upgrade.'),
