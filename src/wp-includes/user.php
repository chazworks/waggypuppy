...
function wp_insert_user($userdata)
{
    ...
    $custom_meta = [];
    if (array_key_exists('meta_input', $userdata)
...
###SITEURL###',
        ...
        $email_change_email['message'] = str_replace('###ADMIN_EMAIL###', get_option('admin_email'),
            $email_change_email['message']);
        $email_change_email['message'] = str_replace('###NEW_EMAIL###', $userdata['user_email'],
            $email_change_email['message']);
...
function get_password_reset_key($user)
{
    ...
    if (!($user instanceof WP_User)) {
        return new WP_Error('invalidcombo',
...
function retrieve_password($user_login = null)
{
     ...
     */
    $message .= network_site_url('wp-login.php?login=' . rawurlencode($user_login) . "&key=$key&action=rp", 'login')
        . '&wp_lang='
        . $locale
        . "\r\n\r\n";
     ...
     *
     */
    $notification_email = apply_filters('retrieve_password_notification_email', $defaults, $key, $user_login,
        $user_data);
...
###SITEURL###',
     ...
     *
     */
    $headers = apply_filters('user_request_confirmed_email_headers', $headers, $subject, $content, $request_id,
        $email_data);
...
###SITEURL###',
     ...
     *
     */
    $headers = apply_filters('user_erasure_fulfillment_email_headers', $headers, $subject, $content, $request_id,
        $email_data);
...
function wp_create_user_request($email_address = '', $action_name = '', $request_data = [], $status = 'pending')
{
    ...
    if ($requests_query->found_posts) {
        return new WP_Error('duplicate_request',
            __('An incomplete personal data request for this email address already exists.'));
    }
...
###SITEURL###',
     ...
     *
     */
    $headers = apply_filters('user_request_action_email_headers', $headers, $subject, $content, $request_id,
        $email_data);
