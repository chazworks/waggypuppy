    ...
    register_post_type(
        ...
        [
            'labels' => [
                ...
                'view_item' => ('1' === get_option('wp_attachment_pages_enabled')) ? __('View Attachment Page')
                    : __('View Media File'),
...
function get_post_status($post = null)
{
    ...
    } elseif ('attachment' === $post->post_type && !in_array($post_status, ['private', 'trash', 'auto-draft'], true)
    ) {
        /*
...
function wp_post_mime_type_where($post_mime_types, $table_alias = '')
{
    ...
    foreach ((array)$post_mime_types as $mime_type) {
        ...
        if (str_contains($mime_pattern, '%')) {
            $where_clauses[] = empty($table_alias) ? "post_mime_type LIKE '$mime_pattern'"
                : "$table_alias.post_mime_type LIKE '$mime_pattern'";
        } else {
            $where_clauses[] = empty($table_alias) ? "post_mime_type = '$mime_pattern'"
...
function wp_delete_post($post_id = 0, $force_delete = false)
{
    ...
    $comment_ids = $wpdb->get_col(
        $wpdb->prepare("SELECT comment_ID FROM $wpdb->comments WHERE comment_post_ID = %d ORDER BY comment_ID DESC",
            $post_id),
    );
...
function wp_insert_post($postarr, $wp_error = false, $fire_after_hooks = true)
{
    ...
    $ping_status = empty($postarr['ping_status']) ? get_default_comment_status($post_type, 'pingback')
        : $postarr['ping_status'];
...
function wp_unique_post_slug($slug, $post_id, $post_status, $post_type, $post_parent)
{
     ...
     *
     */
    $override_slug = apply_filters('pre_wp_unique_post_slug', null, $slug, $post_id, $post_status, $post_type,
        $post_parent);
    ...
    if ('attachment' === $post_type) {
         ...
         *
         */
        $is_bad_hierarchical_slug = apply_filters('wp_unique_post_slug_is_bad_hierarchical_slug', false, $slug,
        ...
        if ('post' === $post_type && (!$post || $post->post_name !== $slug) && preg_match('/^[0-9]+$/', $slug)) {
            ...
            if ($slug_num) {
                ...
                */
                if (0 === $postname_index
                    || ($postname_index && '%year%' === $permastructs[$postname_index - 1]
                        && 13
                        > $slug_num)
...
function wp_delete_attachment($post_id, $force_delete = false)
{
    ...
    $comment_ids = $wpdb->get_col($wpdb->prepare("SELECT comment_ID FROM $wpdb->comments WHERE comment_post_ID = %d ORDER BY comment_ID DESC",
        $post_id));
    foreach ($comment_ids as $comment_id) {
        wp_delete_comment($comment_id, true);
    }
    ...
    if ($file) {
        ...
        if ($uploads && false === $uploads['error']) {
            ...
            if (str_starts_with($file, $uploads['basedir'])) {
                ...
                $url = trailingslashit($uploads['baseurl'] . '/' . _wp_get_attachment_relative_path($file))
                    . wp_basename($file);
            } else {
...
function _prime_post_parent_id_caches(array $ids)
{
    ...
    if (!empty($non_cached_ids)) {
        $fresh_posts = $wpdb->get_results(sprintf("SELECT $wpdb->posts.ID, $wpdb->posts.post_parent FROM $wpdb->posts WHERE ID IN (%s)",
            implode(',', $non_cached_ids)));
...
function get_available_post_mime_types($type = 'attachment')
{
    ...
    if (!is_array($mime_types)) {
        $mime_types = $wpdb->get_col($wpdb->prepare("SELECT DISTINCT post_mime_type FROM $wpdb->posts WHERE post_type = %s AND post_mime_type != ''",
            $type));
    }
