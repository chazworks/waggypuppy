...
function _wp_post_revision_data($post = [], $autosave = false)
{
    ...
    $revision_data['post_name'] = $autosave ? "$post[ID]-autosave-v1"
...
function _wp_upgrade_revisions_of_post($post, $revisions)
{
    ...
    $now = time();
    $result = $wpdb->query($wpdb->prepare("INSERT IGNORE INTO `$wpdb->options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, 'off') /* LOCK */",
        $lock, $now));
