...
class WP_Query
{
    ...
    public function parse_query($query = '')
    {
        ...
        $qv['cat'] = preg_replace('|[^0-9,-]|', '',
            $qv['cat']); // Array or comma-separated list of positive or negative integers.
        ...
        if (('' !== $qv['attachment']) || !empty($qv['attachment_id'])) {
            ...
            if ($qv['day']) {
                if (!$this->is_date) {
                    ...
                    if ($qv['monthnum'] && $qv['year']
                        && !wp_checkdate($qv['monthnum'], $qv['day'], $qv['year'], $date)) {
                        $qv['error'] = '404';
                    } else {
                        $this->is_day = true;
                        $this->is_date = true;
    ...
    protected function parse_search(&$q)
    {
        ...
        foreach ($q['search_terms'] as $term) {
            ...
            foreach ($search_columns as $search_column) {
                $search_columns_parts[$search_column] = $wpdb->prepare("({$wpdb->posts}.$search_column $like_op %s)",
                    $like);
            }
        ...
        switch ($orderby) {
            ...
            case 'post__in':
                if (!empty($this->query_vars['post__in'])) {
                    $orderby_clause = "FIELD({$wpdb->posts}.ID," . implode(',',
                            array_map('absint', $this->query_vars['post__in'])) . ')';
                }
                break;
            case 'post_parent__in':
                if (!empty($this->query_vars['post_parent__in'])) {
                    $orderby_clause = "FIELD( {$wpdb->posts}.post_parent," . implode(', ',
                            array_map('absint', $this->query_vars['post_parent__in'])) . ' )';
                }
                break;
            case 'post_name__in':
    ...
    public function get_posts()
    {
        ...
        if ((isset($q['posts_per_archive_page']) && 0 != $q['posts_per_archive_page'])
            && ($this->is_archive
                || $this->is_search)) {
        ...
        if ($this->is_home && (empty($this->query) || 'true' === $q['preview'])
            && ('page'
        ...
        if ($this->is_tax) {
            if (empty($post_type)) {
                ...
                foreach (get_post_types(['exclude_from_search' => false]) as $pt) {
                    $object_taxonomies = 'attachment' === $pt ? get_taxonomies_for_attachments()
        ...
        if (isset($q['comment_count'])) {
            ...
            if (isset($q['comment_count']['value'])) {
                ...
                $where .= $wpdb->prepare(" AND {$wpdb->posts}.comment_count {$q['comment_count']['compare']} %d",
        ...
        if ('any' === $post_type) {
            ...
            if (empty($in_search_post_types)) {
                ...
                $post_type_where = " AND {$wpdb->posts}.post_type IN ('" . implode("', '",
        ...
        if ($skip_post_status) {
            ...
            if (!empty($r_status)) {
                if (!empty($q['perm']) && 'editable' === $q['perm'] && !current_user_can($edit_others_cap)) {
                    $statuswheres[] = "({$wpdb->posts}.post_author = $user_id "
                        . 'AND ('
                        . implode(' OR ', $r_status)
                        . '))';
                } else {
         ...
         *
         * See https://core.trac.wp.org/ticket/56841.
        ...
        if (null === $this->posts) {
             ...
             *
             */
            $split_the_query = apply_filters('split_the_query', $split_the_query, $this, $old_request,
                compact($pieces));
    ...
    public function the_post()
    {
        ...
        if (!$this->in_the_loop) {
            ...
            if ($post_ids) {
                _prime_post_caches($post_ids, $this->query_vars['update_post_term_cache'],
    ...
    public function generate_postdata($post)
    {
        ...
        $elements = compact('id', 'authordata', 'currentday', 'currentmonth', 'page', 'pages', 'multipage', 'more',
            'numpages');
