...
class Cookie
{
    ...
    public function __construct($name, $value, $attributes = [], $flags = [], $reference_time = null)
    {
        ...
        if (InputValidator::has_array_access($attributes) === false
            || InputValidator::is_iterable($attributes)
            === false) {
    ...
    public function path_matches($request_path)
    {
        ...
        if (strlen($request_path) > strlen($cookie_path)
            && substr($request_path, 0, strlen($cookie_path))
            === $cookie_path) {
