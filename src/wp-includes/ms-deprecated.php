<?php
/**
 * Deprecated functions from waggypuppy MU and the multisite feature. You shouldn't
 ...
 * Dashboard blog functionality was removed in WP 3.1, replaced by the user admin.
 *
...
function get_blog_list($start = 0, $num = 10, $deprecated = '')
{
    ...
    $blogs = $wpdb->get_results($wpdb->prepare("SELECT blog_id, domain, path FROM $wpdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC",
        get_current_network_id()), ARRAY_A);

    $blog_list = [];
    foreach ((array)$blogs as $details) {
        $blog_list[$details['blog_id']] = $details;
        $blog_list[$details['blog_id']]['postcount'] = $wpdb->get_var("SELECT COUNT(ID) FROM "
