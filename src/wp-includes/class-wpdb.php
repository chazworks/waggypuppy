...
class wpdb
{
    ...
    public function prepare($query, ...$args)
    {
        ...
        $split_query = preg_split("/(^|[^%]|(?:%%)+)(%(?:$allowed_format)?[sdfFi])/", $query, -1,
    ...
    public function print_error($str = '')
    {
        ...
        if ($caller) {
            ...
            $error_str = sprintf('waggypuppy database error %1$s for query %2$s made by %3$s', $str, $this->last_query,
                $caller);
        } else {
    ...
    public function log_query($query, $query_time, $query_callstack, $query_start, $query_data)
    {
         ...
         *
         */
        $query_data = apply_filters('log_query_custom_data', $query_data, $query, $query_time, $query_callstack,
            $query_start);
    ...
    public function check_database_version()
    {
        ...
        if (version_compare($this->db_version(), $required_mysql_version, '<')) {
            ...
            return new WP_Error('database_version',
                sprintf(__('<strong>Error:</strong> waggypuppy %1$s requires MySQL %2$s or higher'), $wp_version,
