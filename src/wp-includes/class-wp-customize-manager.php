...
final class WP_Customize_Manager
{
    ...
    public function __construct($args = [])
    {
        ...
        add_action('customize_register', [$this, 'register_dynamic_settings'],
                ...
                $unpublished_changeset_posts = $this->get_changeset_posts(
                    [
                        'post_status' => array_diff(get_post_stati(),
                            ['auto-draft', 'publish', 'trash', 'inherit', 'private']),
    ...
    public function import_theme_starter_content($starter_content = [])
    {
        ...
        $sidebars_widgets = isset($starter_content['widgets']) && !empty($this->widgets) ? $starter_content['widgets']
            : [];
        $attachments = isset($starter_content['attachments']) && !empty($this->nav_menus)
            ...
            foreach ($nav_menu['items'] as $nav_menu_item) {
                ...
                if (isset($nav_menu_item['object_id'])) {
                    if ('post_type' === $nav_menu_item['type']
                        && preg_match('/^{{(?P<symbol>.+)}}$/', $nav_menu_item['object_id'], $matches)
                ...
                if (empty($changeset_data[$nav_menu_item_setting_id])
                    || !empty($changeset_data[$nav_menu_item_setting_id]['starter_content'])) {
        ...
        if (!$args['exclude_changeset']) {
            foreach ($this->changeset_data() as $setting_id => $setting_params) {
                ...
                if (isset($setting_params['type']) && 'theme_mod' === $setting_params['type']) {
                    ...
                    if (preg_match($namespace_pattern, $setting_id, $matches)
                        && $this->get_stylesheet()
                        === $matches['stylesheet']) {
    ...
    public function save()
    {
        ...
        if (isset($_POST['customize_changeset_status'])) {
            ...
            if (!get_post_status_object($changeset_status)
                || !in_array($changeset_status, ['draft', 'pending', 'publish', 'future'], true)) {
        ...
        if (isset($response['setting_validities'])) {
            $response['setting_validities'] = array_map([$this, 'prepare_setting_validity_for_js'],
    ...
    public function save_changeset_post($args = [])
    {
        ...
        if ('publish' === $args['status']
            && false === has_action('transition_post_status', '_wp_customize_publish_changeset')) {
        ...
        if ($args['date_gmt']) {
            ...
            if (!$is_future_dated) {
                return new WP_Error('not_future_date',
                    __('You must supply a future date to schedule.')); // Only future dates are allowed.
            }
            ...
            $will_remain_auto_draft = (!$args['status']
                && (!$changeset_post_id
                    || 'auto-draft'
        ...
        if ($update_transactionally && $invalid_setting_count > 0) {
            $response = [
                ...
                'message' => sprintf(_n('Unable to save due to %s invalid setting.',
                    'Unable to save due to %s invalid settings.', $invalid_setting_count),
    ...
    {
        if (isset($data['post_type']) && isset($unsanitized_postarr['post_content'])
            && 'customize_changeset'
            === $data['post_type']
            || (
                'revision' === $data['post_type'] && !empty($data['post_parent'])
                && 'customize_changeset'
                === get_post_type($data['post_parent'])
            )
        ) {
    ...
    public function handle_dismiss_autosave_or_lock_request()
    {
        ...
        if ($dismiss_autosave) {
            if (empty($changeset_post_id) || 'auto-draft' === get_post_status($changeset_post_id)) {
                ...
                if ($revision) {
                    if (!current_user_can(get_post_type_object('customize_changeset')->cap->delete_post,
                        $changeset_post_id)) {
    ...
    public function render_control_templates()
    {
        ...
        <script type="text/html" id="tmpl-customize-changeset-locked-notification">
            <li class="notice notice-{{ data.type || 'info' }} {{ data.containerClasses || '' }}"
                data-code="{{ data.code }}" data-type="{{ data.type }}">
                <div
                    class="notification-message customize-changeset-locked-message {{ data.lockUser.avatar ? 'has-avatar' : '' }}">
    ...
    public function get_return_url()
    {
        ...
        if ($this->return_url) {
            ...
            && !in_array(wp_basename(parse_url($referer, PHP_URL_PATH)), $excluded_referer_basenames, true)) {
            $return_url = $referer;
    ...
    public function register_controls()
    {
        ...
        $this->add_panel(
            new WP_Customize_Themes_Panel(
                ...
                [
                    ...
                    'description' => (
                        '<p>'
                        . __('Looking for a theme? You can search or browse the wp.org theme directory, install and preview themes, then activate them right here.')
                        . '</p>'
                        .
                        '<p>'
        ...
        $this->add_control(
            new WP_Customize_Cropped_Image_Control(
                ...
                [
                    ...
                    'flex_height' => isset($custom_logo_args[0]['flex-height']) ? $custom_logo_args[0]['flex-height']
                        : null,
                    'flex_width' => isset($custom_logo_args[0]['flex-width']) ? $custom_logo_args[0]['flex-width']
                        : null,
                    'button_labels' => [
        ...
        $this->add_setting(
            new WP_Customize_Filter_Setting(
                ...
                'header_image',
                [
                    'default' => sprintf(get_theme_support('custom-header', 'default-image'),
        ...
        $section_description .= '<p id="editor-keyboard-trap-help-1">'
            . __('When using a keyboard to navigate:')
            . '</p>';
        $section_description .= '<ul>';
        $section_description .= '<li id="editor-keyboard-trap-help-2">'
            . __('In the editing area, the Tab key enters a tab character.')
            . '</li>';
        $section_description .= '<li id="editor-keyboard-trap-help-3">'
            . __('To move away from this area, press the Esc key followed by the Tab key.')
            . '</li>';
        $section_description .= '<li id="editor-keyboard-trap-help-4">'
        ...
        $section_description .= '<button type="button" class="button-link section-description-close">'
            . __('Close')
            . '</button>';
        $section_description .= '</p>';
    ...
    public function _validate_header_video($validity, $value)
    {
        ...
        if ($video) {
            ...
            if (!str_ends_with($video, '.mp4')
                && !str_ends_with($video,
                    '.mov')) { // Check for .mp4 or .mov format, which (assuming h.264 encoding) are the only cross-browser-supported formats.
                $validity->add(
                    'invalid_file_type',
                    sprintf(
