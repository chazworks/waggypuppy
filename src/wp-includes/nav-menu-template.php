...
function wp_nav_menu($args = [])
{
    ...
    if ($args->container) {
        ...
        if (is_string($args->container) && in_array($args->container, $allowed_tags, true)) {
            ...
            $aria_label = ('nav' === $args->container && $args->container_aria_label) ? ' aria-label="'
...
function _wp_menu_item_classes_by_context(&$menu_items)
{
    ...
    foreach ((array)$menu_items as $key => $menu_item) {
        ...
        if ($wp_query->is_singular && 'taxonomy' === $menu_item->type
        ...
        } elseif ((int)$menu_item->object_id === $queried_object_id
            && (
            ...
            $raw_item_url = strpos($menu_item->url, '#') ? substr($menu_item->url, 0, strpos($menu_item->url, '#'))
                : $menu_item->url;
        ...
        if (isset($parent_item->type)
            && (
                ...
                (
                    ...
                    && (
                        !isset($queried_object->term_id) || (int)$parent_item->object_id !== $queried_object->term_id
                    )
                )
            )
        ) {
