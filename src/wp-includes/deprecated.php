 ...
 *
 * @link https://developer.wp.org/themes/basics/the-loop/
 *
 * @since 1.0.1
 * @deprecated 1.5.0
 *
...
function start_wp()
{
    ...
    _deprecated_function(__FUNCTION__, '1.5.0', __('new waggypuppy Loop'));
    ...
    foreach ((array)$results as $row) {
        ...
        if ($show_updated) {
            if (!str_starts_with($row->link_updated_f, '00')) {
                $title .= ' (' . __('Last updated') . ' ' . gmdate(get_option('links_updated_date_format'),
 ...
 * @see get_terms()
 *
 * @link https://developer.wp.org/reference/functions/get_all_category_ids/
 *
 * @since 2.0.0
 */
...
function make_url_footnote($content)
{
    ...
    for ($i = 0, $c = count($matches[0]); $i < $c; $i++) {
        ...
        $link_url = ((strtolower(substr($link_url, 0, 7)) !== 'http://')
            && (strtolower(substr($link_url, 0, 8))
...
{
    _deprecated_function(__FUNCTION__, '6.0.3',
...
function wp_global_styles_render_svg_filters()
{
	 ...
	 */
    if (
        is_admin() && !get_current_screen()->is_block_editor()
    ) {
        return;
    }
    ...
    $fn_get_webfonts_from_theme_json = static function () {
        ...
        if (is_admin() || wp_is_rest_endpoint()) {
            ...
            foreach ($variations as $variation) {
                ...
                $settings['typography']['fontFamilies']['theme'] = array_merge($settings['typography']['fontFamilies']['theme'],
