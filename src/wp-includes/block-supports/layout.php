...
{
    $support_layout = block_has_support($block_type, 'layout', false)
    ...
    if ('default' === $layout_type) {
        ...
        if ('horizontal' === $layout_orientation) {
            ...
            if (!empty($layout['verticalAlignment'])
                && array_key_exists($layout['verticalAlignment'], $vertical_alignment_options)) {
                $layout_styles[] = [
                    'selector' => $selector,
...
function wp_render_layout_support_flag($block_content, $block)
{
    ...
    $block_supports_layout = block_has_support($block_type, 'layout', false)
        || block_has_support($block_type, '__experimentalLayout', false);
    ...
    if ($child_layout) {
        ...
        if (isset($child_layout['columnSpan'])
            ...
            $parent_column_width = isset($block['parentLayout']['minimumColumnWidth'])
                ? $block['parentLayout']['minimumColumnWidth'] : '12rem';
            ...
            $container_query_value = $column_span_number * $parent_column_value + ($column_span_number - 1)
                * $default_gap_value;
...
function wp_restore_group_inner_container($block_content, $block)
{
    ...
    if (wp_theme_has_theme_json() || 1 === preg_match($group_with_inner_container_regex, $block_content)
        || (isset($block['attrs']['layout']['type'])
            && ('flex' === $block['attrs']['layout']['type']
                || 'grid'
                === $block['attrs']['layout']['type']))
    ) {
        return $block_content;
    }
...
(
    ...
    return '<div class="' . implode(' ', $wrapper_classnames) . '">' . $matches[1] . implode(' ',
            $filtered_content_classnames) . $matches[3] . '</div>';
}
