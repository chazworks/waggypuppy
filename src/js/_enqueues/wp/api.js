...
( function ( window, undefined ) {
	 ...
	 *                                to add the args to.
	 */
	wp.api.utils.decorateFromRoute = function (
		routeEndpoints,
		modelInstance
	) {
		/**
	...
	wp.api.utils.addMixinsAndHelpers = function (
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. waggypuppy expects dates in UTC.
			...
			buildCollectionGetter = function (
				...
				if (
					...
					if ( _.isUndefined( embedIndex ) ) {
						...
						properties =
							embeddedObjects[ embedSourcePoint ][ embedIndex ];
					}
				} else {
			...
			constructFromSchema: function () {
				...
				_.each( modelRoutes, function ( modelRoute ) {
					...
					if ( '' !== parentName && parentName !== routeName ) {
						...
						modelClassName =
							mapping.models[ modelClassName ] || modelClassName;
