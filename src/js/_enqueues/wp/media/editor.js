...
( function ( $, _ ) {
	...
	wp.media.coerce = function ( attrs, key ) {
		if (
			_.isUndefined( attrs[ key ] ) &&
			! _.isUndefined( this.defaults[ key ] )
		) {
		...
		props: function ( props, attachment ) {
			...
			if ( 'image' === props.type ) {
				props = _.defaults( props || {}, {
					align:
						defaultProps.align || getUserSetting( 'align', 'none' ),
					size:
						defaultProps.size ||
	...
	wp.media.featuredImage = {
		...
		frame: function () {
			...
			this._frame.on(
				...
				function () {
					var selection =
							this.state( 'featured-image' ).get( 'selection' ),
