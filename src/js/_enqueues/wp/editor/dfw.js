...
( function ( window, $, undefined ) {
	...
	$( function () {
			...
			function ( event, editor ) {
				...
				function mceScroll( key ) {
					...
					editorBottom =
						...
						( advanced
							? heights.bottomHeight + heights.statusBarHeight
							: 0 );
		...
		function adjust( event ) {
					...
					if ( windowPos + pinnedToolsTop <= postBodyTop ) {
						...
						if ( windowPos > lastScrollPosition ) {
							...
							if ( fixedSideBottom ) {
								...
								fixedSideBottom = false;
								sidebarTop =
									$sideSortables.offset().top - sidebarBottom;
								footerTop = $footer.offset().top;
			...
			if ( resize ) {
				...
				if ( visual ) {
					$visualEditor.css( {
						paddingTop:
							heights.visualTopHeight + heights.menuBarHeight,
					} );
				} else {
					$textEditor.css( {
		...
		function off() {
			var height = parseInt(
				window.getUserSetting( 'ed_size', 300 ),
				10
			);
				...
				$overlay
					...
					.on( 'mousemove.focus', function ( event ) {
						...
						if ( x && y && ( nx !== x || ny !== y ) ) {
							if (
								( ny <= y &&
									ny < editorRect.top - pageYOffset ) ||
								( ny >= y &&
									ny > editorRect.bottom - pageYOffset ) ||
								( nx <= x &&
									nx < editorRect.left - pageXOffset ) ||
								( nx >= x &&
									nx > editorRect.right - pageXOffset )
							) {
								...
								if (
									( ny <=
										editorRect.top - buffer - pageYOffset ||
										ny >=
											editorRect.bottom +
												buffer -
												pageYOffset ||
