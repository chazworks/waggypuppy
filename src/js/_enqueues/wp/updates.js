...
( function ( $, wp, settings ) {
	...
	wp.updates.updatePluginSuccess = function ( response ) {
		...
		if ( 'plugins' === pagenow || 'plugins-network' === pagenow ) {
			...
			$updateMessage = $(
				'.plugin-card-' + response.slug + ', #plugin-information-footer'
			)
				.find( '.update-now' )
				...
				if (
					...
					if ( plugins[ 'auto-update-enabled' ].length ) {
						$views
							...
							.text(
								'(' +
									plugins[ 'auto-update-enabled' ].length +
									')'
							);
					} else {
				...
				if (
					...
					if ( plugins[ 'auto-update-disabled' ].length ) {
						$views
							...
							.text(
								'(' +
									plugins[ 'auto-update-disabled' ].length +
									')'
							);
					} else {
	...
	wp.updates.updateThemeError = function ( response ) {
		...
		if (
			wp.updates.maybeHandleCredentialError( response, 'update-theme' )
		) {
			return;
		}
	...
	wp.updates.installThemeSuccess = function ( response ) {
		var $card = $(
				'.wp-full-overlay-header, [data-slug=' + response.slug + ']'
			),
			$message;
					...
					if (
						...
						if ( themes[ 'auto-update-enabled' ].length ) {
							$views
								...
								.text(
									'(' +
										themes[ 'auto-update-enabled' ].length +
										')'
								);
						} else {
							$views.find( '.auto-update-enabled' ).remove();
	...
	wp.updates.deleteThemeError = function ( response ) {
		...
		if (
			wp.updates.maybeHandleCredentialError( response, 'delete-theme' )
		) {
			return;
		}
			...
			function ( event ) {
				...
				$document.on(
					...
					function ( event, response ) {
						...
						if (
							'wp-' + response.update + '-update-success' ===
							event.type
						) {
							success++;
						} else {
				...
				wp.updates.searchRequest = wp.ajax
					...
					.done( function ( response ) {
						...
						if ( ! data.s.length ) {
							$oldSubTitle.remove();
							$(
								'.subsubsub .' + data.plugin_status + ' a'
							).addClass( 'current' );
