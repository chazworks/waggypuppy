...
( function ( exports, $ ) {
				...
				if ( params.sort ) {
					...
					notifications.sort( function ( a, b ) {
						var aPriority = 0,
							bPriority = 0;
						if (
							! _.isUndefined( errorTypePriorities[ a.type ] )
						) {
							aPriority = errorTypePriorities[ a.type ];
						}
						if (
							! _.isUndefined( errorTypePriorities[ b.type ] )
						) {
			...
			onChangeExpanded: function ( expanded, args ) {
				...
				if ( expanded && ! content.hasClass( 'open' ) ) {
					...
					if ( section.panel() ) {
						...
						if (
							panel.contentContainer.hasClass( 'skip-transition' )
						) {
							panel.collapse();
						}
					}
		...
		/** @lends wp.customize.ThemesSection.prototype */ {
			...
			attachEvents: function () {
				...
				section.headerContainer.on(
					...
					function () {
						...
						if (
							section.headerContainer.find( '.filter-details' )
								.length
						) {
							section.headerContainer
								.find( '.customize-themes-section-title' )
					...
					function ( e ) {
						...
						if ( $filterToggle.hasClass( 'open' ) ) {
							...
							section.contentContainer
								...
								.css(
									'margin-top',
									section.filtersHeight + marginOffset
								);
						} else {
							section.contentContainer
								.find( '.themes' )
			...
			loadThemes: function () {
				...
				request.done( function ( data ) {
					...
					if ( 0 !== themes.length ) {
						...
						if ( 0 === section.loaded ) {
							...
							wp.a11y.speak(
								section.container.find( '.no-themes' ).text()
							);
						} else {
							section.fullyLoaded = true;
						}
				...
				request.fail( function ( data ) {
					if ( 'undefined' === typeof data ) {
						...
						wp.a11y.speak(
							section.container.find( '.unexpected-error' ).text()
						);
					} else if (
			...
			renderScreenshots: function () {
				...
				section.screenshotQueue = _.filter(
					...
					function ( control ) {
						var $imageWrapper =
								control.container.find( '.theme-screenshot' ),
						...
						var wt = section.$window.scrollTop(),
							...
							threshold = ih * 3,
							inView =
								eb >= wt - threshold && et <= wb + threshold;
	...
	api.Panel = Container.extend(
		/** @lends wp.customize.Panel.prototype */ {
			...
			embed: function () {
				...
				if (
					! panel.contentContainer.parent().is( panel.headContainer )
				) {
			...
			renderContent: function () {
				...
				if (
					0 !==
					$( '#tmpl-' + panel.templateSelector + '-content' ).length
				) {
					template = wp.template(
	...
	api.ThemesPanel = api.Panel.extend(
		/** @lends wp.customize.ThemsPanel.prototype */ {
			...
			installTheme: function ( event ) {
				...
				onInstallSuccess = function ( response ) {
					...
					if ( preview ) {
						...
						if (
							! theme ||
							api.control.has( 'installed_theme_' + theme.id )
						) {
			...
			deleteTheme: function ( event ) {
				...
				if (
					! window.confirm( api.settings.l10n.confirmDeleteTheme )
				) {
					return;
				}
			...
			embed: function () {
				...
				inject = function ( sectionId ) {
					...
					api.section( sectionId, function ( section ) {
						...
						section.deferred.embedded.done( function () {
							parentContainer = section.contentContainer.is(
								'ul'
							)
								? section.contentContainer
		...
		/** @lends wp.customize.SiteIconControl.prototype */ {
			...
			setImageFromAttachment: function ( attachment ) {
				...
				_.each( sizes, function ( size ) {
					if (
						! icon &&
						! _.isUndefined( attachment.sizes[ size ] )
					) {
		...
		/** @lends wp.customize.CodeEditorControl.prototype */ {
			...
			onTabNext: function onTabNext() {
				...
				if ( controls.length === controlIndex + 1 ) {
					$( '#customize-footer-actions .collapse-sidebar' ).trigger(
						'focus'
					);
				} else {
			...
			onUpdateErrorNotice: function onUpdateErrorNotice(
				...
				if ( 0 !== errorAnnotations.length ) {
					if ( 1 === errorAnnotations.length ) {
						message = api.l10n.customCssError.singular.replace(
							'%d',
							'1'
						);
					} else {
		...
		/** @lends wp.customize.Previewer.prototype */ {
			...
			refresh: function () {
				...
				previewer.loading = new api.PreviewFrame( {
					...
					query:
						previewer.query( { excludeCustomizedSaved: true } ) ||
						{},
	...
	api._handleSettingValidities = function handleSettingValidities( args ) {
		...
		if ( args.focusInvalidControl ) {
			...
			_( _.values( invalidSettingControls ) ).find( function (
				controls
			) {
	...
	$( function () {
		...
		api.section( 'publish_settings', function ( section ) {
			...
			function highlightScheduleButton() {
				if ( ! cancelScheduleButtonReminder ) {
					cancelScheduleButtonReminder = api.utils.highlightButton(
						btnWrapper,
						{
							delay: 1000,
			...
			/** @lends wp.customize.previewer */ {
				...
				save: function ( args ) {
					...
					submit = function () {
						 ...
						 */
						api.each( function ( setting ) {
							setting.notifications.each( function (
								notification
							) {
								if (
									'error' === notification.type &&
						...
						api.control.each( function ( control ) {
							if (
								...
								control.notifications.each( function (
									notification
								) {
									if ( 'error' === notification.type ) {
						...
						request.fail( function ( response ) {
							...
							if ( 'invalid_nonce' === response ) {
								...
								if (
									...
									api.control( 'changeset_scheduled_date' )
		...
		$( [ api.panel, api.section, api.control ] ).each( function (
			i,
			values
		) {
			...
			state.bind( 'change', function () {
				...
				if ( ! activated() ) {
					...
					if ( 'draft' === selectedChangesetStatus() ) {
						if (
							saved() &&
							selectedChangesetStatus() === changesetStatus()
						) {
		...
		footerActions
			.find( '.devices button' )
			.on( 'click', function ( event ) {
				api.previewedDevice.set(
					$( event.currentTarget ).data( 'device' )
				);
			} );
		...
		if ( title.length ) {
			api( 'blogname', function ( setting ) {
				var updateTitle = function () {
					...
					title.text(
						blogTitle.toString().trim() || api.l10n.untitledBlogName
					);
				};
		...
		function startAutosaving() {
			...
			updateChangesetWithReschedule = function () {
				if ( ! updatePending ) {
					updatePending = true;
					api.requestChangesetUpdate( {}, { autosave: true } ).always(
						function () {
							updatePending = false;
						}
					);
