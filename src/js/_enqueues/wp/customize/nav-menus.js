				...
				self.currentRequest.fail( function ( data ) {
					...
					if ( data.message ) {
						$content
							.empty()
							.append(
								$( '<li class="nothing-found"></li>' ).text(
									data.message
								)
							);
				...
				if ( ! menuNameControl ) {
					menuNameControl = new api.controlConstructor.nav_menu_name(
						menuNameControlId,
						{
							label: api.Menus.data.l10n.menuNameLabel,
							description:
								api.Menus.data.l10n.newMenuNameDescription,
							section: section.id,
							priority: 0,
						}
					);
		...
		/** @lends wp.customize.Menus.MenuItemControl.prototype */ {
			...
			refreshAdvancedAccessibilityOfItem: function ( itemToRefresh ) {
				...
				if ( isPrimaryMenuItem ) {
					...
					( parentItem = menuItem
						.prevAll(
							'.menu-item-depth-' + parseInt( depth - 1, 10 )
						)
						.first() ),
					...
					.each( function () {
						var container = $( this ),
							...
							updateSelectedMenuLabel = function (
								...
								var menuSetting = api(
									'nav_menu[' + String( selectedMenuId ) + ']'
								);
								if (
									! selectedMenuId ||
									! menuSetting ||
						...
						navMenuLocationSetting.bind( function (
							selectedMenuId
						) {
							updateCheckbox(
								selectedMenuId === control.params.menu_id
							);
		...
		/** @lends wp.customize.Menus.MenuControl.prototype */ {
			...
			ready: function () {
				...
				if ( menu ) {
					...
					if (
						0 ===
						select.find( 'option[value=' + String( menuId ) + ']' )
							.length
					) {
						select.append( new Option( name, menuId ) );
					}
			...
			_setupTitle: function () {
				...
				control.setting.bind( function ( menu ) {
					...
					var section = api.section( control.section() ),
						...
						location =
							section.headContainer.find( '.menu-in-location' ),
			...
			toggleReordering: function ( showOrHide ) {
				...
				if ( showOrHide ) {
					_( this.getMenuItemControls() ).each( function (
						formControl
					) {
				...
				reflowRecursively = function ( context ) {
					...
					_.each(
						...
						function ( menuItemControl ) {
							if (
								thisParent ===
								menuItemControl.setting().menu_item_parent
							) {
					...
					if ( currentMenuItemControls.length ) {
						_( currentMenuItemControls ).each( function (
							menuItemControl
						) {
							menuItemControl.container.removeClass(
