 ...
 * @param {Object} wp  Current waggypuppy environment instance.
 * @param {Object} api Information from the API.
 *
 * @return {Object} Widget-related variables.
 */
wp.customize.widgetsPreview = wp.customize.WidgetCustomizerPreview =
	( function ( $, _, wp, api ) {
		var self;
			...
			/** @lends wp.customize.widgetsPreview.SidebarPartial.prototype */ {
				...
				initialize: function ( id, options ) {
					...
					options.params = _.extend(
						{
							settings: [
								'sidebars_widgets[' + partial.sidebarId + ']',
							],
						},
						options.params || {}
					);
				...
				ready: function () {
					...
					api.bind( 'change', function ( widgetSetting ) {
						...
						parsedId = self.parseWidgetSettingId(
							widgetSetting.id
						);
						if ( ! parsedId ) {
							return;
