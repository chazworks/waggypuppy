			...
			_setupReorderUI: function () {
				...
				updateAvailableSidebars = function () {
					...
					$sidebarItems.each( function () {
						...
						sidebar =
							api.Widgets.registeredSidebars.get( sidebarId );
					...
					if ( r.success ) {
						...
						if ( hasSameInputsInResponse && self.liveUpdateMode ) {
							$inputs.each( function ( i ) {
								...
								sanitizedState =
									self._getInputState( $sanitizedInput );
		...
		/** @lends wp.customize.Widgets.WigetsPanel.prototype */ {
			...
			ready: function () {
				...
				panel.deferred.embedded.done( function () {
					...
					// @todo This should use the Notifications API introduced to panels. See <https://core.trac.wp.org/ticket/38794>.
					...
					updateNotice = function () {
						...
						if ( activeSectionCount !== registeredAreaCount ) {
							if ( 0 !== activeSectionCount ) {
								nonRenderedAreaCount =
									registeredAreaCount - activeSectionCount;
								someRenderedMessage =
									l10n.someAreasShown[ nonRenderedAreaCount ];
							} else {
		...
		/** @lends wp.customize.Widgets.SidebarControl.prototype */ {
			...
			_setupModel: function () {
				...
				this.setting.bind( function ( newWidgetIds, oldWidgetIds ) {
					...
					newWidgetIds = _( newWidgetIds ).filter( function (
						newWidgetId
					) {
					...
					widgetFormControls = _( newWidgetIds ).map( function (
						widgetId
					) {
					...
					_( widgetFormControls ).each( function (
						widgetFormControl
					) {
					...
					_( removedWidgetIds ).each( function ( removedWidgetId ) {
						...
						setTimeout( function () {
							...
							if (
								api.Widgets.savedWidgetIds[ removedWidgetId ]
							) {
								inactiveWidgets = api
									.value(
										'sidebars_widgets[wp_inactive_widgets]'
				...
				this.$sectionContent.sortable( {
					...
					update: function () {
						var widgetContainerIds =
								self.$sectionContent.sortable( 'toArray' ),
							widgetIds;
			...
			toggleReordering: function ( showOrHide ) {
				...
				if ( showOrHide ) {
					_( this.getWidgetFormControls() ).each( function (
						formControl
					) {
