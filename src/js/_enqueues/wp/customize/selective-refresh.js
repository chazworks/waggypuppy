...
wp.customize.selectiveRefresh = ( function ( $, api ) {
	...
	Partial = self.Partial = api.Class.extend(
		/** @lends wp.customize.SelectiveRefresh.Partial.prototype */ {
			...
			ready: function () {
				...
				$( document ).on(
					...
					function ( e ) {
						...
						_.each( partial.placements(), function ( placement ) {
							if (
								$( placement.container ).is( e.currentTarget )
							) {
								partial.showControl();
							}
						} );
			...
			renderContent: function ( placement ) {
				 ...
				 * initialize audio and video respectively. See also https://core.trac.wp.org/ticket/40144
				 */
				if ( wp.mediaelement ) {
				...
				_.each(
					...
					function ( pending, partialId ) {
						...
						if ( ! _.isArray( data.contents[ partialId ] ) ) {
							...
							placementsContents = _.map(
								...
								function ( content, i ) {
									var partialPlacement =
										partialsPlacements[ partialId ][ i ];
									if ( partialPlacement ) {
							...
							pending.deferred.resolveWith( pending.partial, [
								placementsContents,
							] );
						}
					}
	...
	api.bind( 'preview-ready', function () {
		...
		api.selectiveRefresh.editShortcutVisibility.bind( function (
			visibility
		) {
