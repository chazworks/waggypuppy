...
wp.mediaWidgets = ( function ( $ ) {
		...
		wp.media.controller.Library.extend(
			/** @lends wp.mediaWidgets.PersistentDisplaySettingsLibrary.prototype */ {
				...
				display: function getDisplaySettingsModel( model ) {
					...
					if (
						'custom' === selectedDisplaySettings.get( 'link_type' )
					) {
						display.linkUrl =
							...
							fetch: function () {
								...
								embedLinkView.dfd = wp.apiRequest( {
									...
									data: {
										url: url,
										maxwidth:
											embedLinkView.model.get( 'width' ),
										maxheight:
											embedLinkView.model.get( 'height' ),
										discover: false,
									},
									type: 'GET',
									dataType: 'json',
	...
	component.MediaWidgetControl = Backbone.View.extend(
		/** @lends wp.mediaWidgets.MediaWidgetControl.prototype */ {
			...
			initialize: function initialize( options ) {
				 ...
				 * from the start, without having to sync with hidden fields. See <https://core.trac.wp.org/ticket/33507>.
				 */
				control.listenTo(
					control.model,
					'change',
				...
				control.$el.on(
					...
					function updateLinkUrl() {
						...
						if (
							...
							control.selectedAttachment.get( 'url' ) === linkUrl
						) {
							linkType = 'file';
						}
						control.model.set( {
			...
			template: function template() {
				var control = this;
				if (
					! $( '#tmpl-widget-media-' + control.id_base + '-control' )
						.length
				) {
					throw new Error(
			 ...
			 * @return {void}
			 */
			handleMediaLibraryLinkClick: function handleMediaLibraryLinkClick(
				event
			) {
				var control = this;
			...
			selectMedia: function selectMedia() {
				...
				// Disable syncing of attachment changes back to server (except for deletions). See <https://core.trac.wp.org/ticket/40403>.
			...
			mapMediaToModelProps: function mapMediaToModelProps(
				...
				_.each( mediaFrameProps, function ( value, mediaProp ) {
					var propName =
						mediaFramePropToModelPropMap[ mediaProp ] || mediaProp;
	 ...
	 * When waggypuppy enqueues this script, it should have an inline script
