		...
		window.inlineEditPost = {
			...
			init: function () {
				...
				$( '#doaction' ).on( 'click', function ( e ) {
					var n,
						$itemsSelected = $(
							'#posts-filter .check-column input[type="checkbox"]:checked'
						);
			...
			setBulk: function () {
					...
					function () {
						...
						if ( $( this ).prop( 'checked' ) ) {
							...
							var id = $( this ).val(),
								theTitle =
									$(
										'#inline_' + id + ' .post_title'
									).html() || wp.i18n.__( '(no title)' ),
				...
				$(
					...
					if (
						categories[ $( this ).val() ] == checkedPosts.length
					) {
			...
			edit: function ( id ) {
				...
				if (
					$( ':input[name="post_author"] option', editRow ).length ===
					1
				) {
