...
( function ( $ ) {
	...
	updateInModerationText = function ( response ) {
		...
		$( '.comments-in-moderation-text' ).text(
			response.i18n_moderation_text
		);
	...
	updatePending = function ( diff, commentPostId ) {
		...
		$( 'span.pending-count' ).each( function () {
			var a = $( this ),
				n = getCount( a ) + diff;
			if ( n < 1 ) n = 0;
			a.closest( '.awaiting-mod' )[
				0 === n ? 'addClass' : 'removeClass'
			]( 'count-0' );
			updateCount( a, n );
		} );
					...
					if (
						...
						if ( total_items_i18n ) {
							...
							$( '.tablenav-pages' )
								...
								.toggleClass(
									'disabled',
									response.supplemental.total_pages ==
										$( '.current-page' ).val()
								);
						}
						updateTotalCount(
		...
		if ( typeof $.table_hotkeys != 'undefined' ) {
			...
			make_hotkeys_redirect = function ( which ) {
				return function () {
					...
					l = $(
						'.tablenav-pages .' + which + '-page:not(.disabled)'
					);
					if ( l.length )
						window.location =
							l[ 0 ].href.replace(
