				...
				var MediaElement = function MediaElement(
					...
					if ( sources !== null ) {
						...
						switch (
							...
							case 'video':
								var _sources =
										t.mediaElement.originalNode.children
											.length,
									nodeSource =
					...
					t.mediaElement.changeRenderer = function (
						...
						var rendererArray = t.mediaElement.options.renderers
							.length
					...
					var props = _mejs2.default.html5media.properties,
						...
						assignGettersSetters = function assignGettersSetters(
							propName
						) {
							if ( propName !== 'src' ) {
								var capName =
						...
						setSrc = function setSrc( value ) {
							...
							if ( typeof value === 'string' ) {
								...
								for (
									...
									var _src2 = ( 0, _media2.absolutizeUrl )(
											value[ _i2 ].src
										),
						...
						triggerAction = function triggerAction(
							...
							try {
								if (
									...
									var response =
										t.mediaElement.renderer[ methodName ](
											args
										);
									if (
										response &&
					...
					_createClass( Renderer, [
						...
						},
						{
							...
							value: function select( mediaFiles ) {
								...
								for (
									var i = 0, total = renderers.length;
									i < total;
									i++
								) {
									var key = renderers[ i ],
					...
					buildprogress: function buildprogress(
						...
						var handleMouseMove = function handleMouseMove( e ) {
								var totalStyles = getComputedStyle( t.total ),
									...
									transform = ( function () {
										if (
											...
											totalStyles.oTransform !== undefined
										) {
											return 'oTransform';
										} else if (
							...
							updateSlider = function updateSlider() {
								var seconds = t.getCurrentTime(),
									timeSliderText =
										_i18n2.default.t( 'mejs.time-slider' ),
						...
						t.broadcastCallback = function ( e ) {
							...
							if (
								...
								var label =
									_document2.default.createElement( 'span' );
								label.className =
									t.options.classPrefix + 'broadcast';
						...
						if (
							...
							if ( t.total && t.handle ) {
								...
								var newWidth = Math.round(
										( tW * nTime ) / t.getDuration()
									),
									handlePos =
										newWidth -
										Math.round( t.handle.offsetWidth / 2 );
					...
					displayCaptions: function displayCaptions() {
						...
						var t = this,
							...
							sanitize = function sanitize( html ) {
								...
								for (
									var _i12 = 0, n = allElements.length;
									_i12 < n;
									_i12++
								) {
									var attributesObj =
											allElements[ _i12 ].attributes,
										attributes =
									...
									for (
										var j = 0, total = attributes.length;
										j < total;
										j++
									) {
										if (
											attributes[ j ].name.startsWith(
												'on'
											) ||
											attributes[ j ].value.startsWith(
												'javascript'
											)
										) {
											allElements[ _i12 ].remove();
										} else if (
											attributes[ j ].name === 'style'
										) {
						...
						if ( track !== null && track.isLoaded ) {
							...
							if ( i > -1 ) {
								...
								if (
									typeof t.options.captionTextPreprocessor ===
									'function'
								)
									text =
					...
					showSlide: function showSlide( index ) {
						...
						if ( img === undefined || img.fadeIn === undefined ) {
							...
							image.addEventListener( 'load', function () {
								...
								( 0,
								_dom.fadeIn )( t.slidesContainer.querySelector( image ) );
								for (
						...
						parse: function parse( trackText ) {
							...
							if ( styleNode ) {
								...
								if ( attributes.length ) {
									styles = {};
									for (
										var i = 0, total = attributes.length;
										i < total;
										i++
									) {
										styles[
								...
								function positionVolumeHandle( volume ) {
									...
									if ( mode === 'vertical' ) {
										...
										volumeHandle.style.marginBottom =
											-parseFloat( volumeStyles.height ) /
												2 +
											'px';
									} else {
					...
					_createClass( MediaElementPlayer, [
						...
						},
						{
							...
							value: function init() {
								...
								if (
									...
									t.getElement( t.container ).dispatchEvent(
										event
									);
								}
							},
						},
						{
							...
							value: function showControls( doAnimation ) {
								...
								doAnimation =
									doAnimation === undefined || doAnimation;
						...
						},
						{
							...
							value: function _handleError( e, media, node ) {
								...
								var errorContent =
										typeof t.options.customError ===
										'function'
											? t.options.customError(
													t.media,
													t.media.originalNode
											  )
											: t.options.customError,
									imgError = '';
						...
						},
						{
							...
							value: function setFillMode() {
								...
								if ( t.node.currentStyle ) {
									if (
										t.node.currentStyle.height === '100%'
									) {
									...
									if (
										t.node.currentStyle.maxHeight === '100%'
									) {
								...
								for (
									var i = 0, total = targetElement.length;
									i < total;
									i++
								) {
						...
						},
						{
							...
							value: function setControlsSize() {
								...
								if (
									! dom.visible( t.getElement( t.container ) )
								) {
									return;
								}
						...
						},
						{
							...
							value: function addControlElement( element, key ) {
								...
								if ( t.featurePosition[ key ] !== undefined ) {
									var child = t.getElement( t.controls )
										.children[
									...
									t.getElement( t.controls ).appendChild(
										element
									);
									var children = t.getElement(
										t.controls
						...
						},
						{
							...
							value: function createIframeLayer() {
								...
								if (
									t.isVideo &&
									t.media.rendererName !== null &&
									t.media.rendererName.indexOf( 'iframe' ) >
										-1 &&
								...
								if ( t.getElement( t.container ) ) {
									...
									if ( ! posterDiv ) {
										...
										posterDiv.className =
											...
											t.options.classPrefix +
											'layer';
										t.getElement( t.layers ).appendChild(
											posterDiv
										);
									}

									var posterImg =
										posterDiv.querySelector( 'img' );
						...
						},
						{
							...
							value: function globalBind( events, callback ) {
								...
								if ( events.d ) {
									...
									for (
										var i = 0, total = eventList.length;
										i < total;
										i++
									) {
										eventList[ i ]
											.split( '.' )
											.reduce( function ( part, e ) {
						...
						},
						{
							...
							value: function buildfeatures(
								...
								for (
									var i = 0,
										total = t.options.features.length;
									i < total;
									i++
								) {
						...
						},
						{
							...
							value: function buildposter(
								...
								var posterUrl =
									media.originalNode.getAttribute( 'poster' );
							...
							value: function onkeydown( player, media, e ) {
								if (
									...
									for (
										var i = 0,
											total =
												player.options.keyActions
													.length;
										i < total;
										i++
									) {
										var keyAction =
											player.options.keyActions[ i ];
						...
						},
						{
							...
							value: function remove() {
								...
								for ( var featureIndex in t.options.features ) {
									var feature =
										t.options.features[ featureIndex ];
									if ( t[ 'clean' + feature ] ) {
										try {
											t[ 'clean' + feature ](
												t,
								...
								if (
									_typeof( t.getElement( t.container ) ) ===
									'object'
								) {
									var offscreen = t
										.getElement( t.container )
				...
				var DashNativeRenderer = {
					...
					create: function create(
						...
						_window2.default[ '__ready__' + id ] = function (
							...
							var assignMdashEvents = function assignMdashEvents(
								e
							) {
				...
				var PluginDetector = ( exports.PluginDetector = {
					...
					detectPlugin: function detectPlugin(
						...
						if (
							...
							_typeof( _constants.NAV.plugins[ pluginName ] ) ===
								'object'
						) {
							description =
				...
				var FlvNativeRenderer = {
					...
					create: function create(
						...
						_window2.default[ '__ready__' + id ] = function (
							...
							var assignFlvEvents = function assignFlvEvents(
								name,
								data
							) {
								if ( name === 'error' ) {
				...
				var HlsNativeRenderer = {
					...
					create: function create(
						...
						_window2.default[ '__ready__' + id ] = function (
							...
							var assignHlsEvents = function assignHlsEvents(
								name,
								data
							) {
								if ( name === 'hlsError' ) {
						...
						if ( total > 0 ) {
							for ( ; index < total; index++ ) {
								if (
									_renderer.renderer.renderers[
										options.prefix
									].canPlayType( mediaFiles[ index ].type )
								) {
									node.setAttribute(
										'src',
							...
							youtubeSettings = {
								...
								events: {
									onReady: function onReady( e ) {
										mediaElement.youTubeApi = youTubeApi =
											e.target;
					...
					exports.isFullScreen = isFullScreen =
						function isFullScreen() {
							if ( hasMozNativeFullScreen ) {
								...
								return (
									_document2.default.msFullscreenElement !==
									null
								);
							}
						};
				...
				function secondsToTimeCode( time ) {
					...
					for (
						...
						for (
							...
							if (
								unique.indexOf( timeFormatFrags[ i ][ j ] ) < 0
							) {
