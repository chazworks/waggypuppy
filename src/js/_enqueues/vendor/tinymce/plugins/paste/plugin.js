	...
	var paste = ( function ( domGlobals ) {
			...
			if ( dataTransfer ) {
				if ( dataTransfer.getData ) {
					...
					if ( legacyText && legacyText.length > 0 ) {
						if (
							legacyText.indexOf( mceInternalUrlPrefix ) === -1
						) {
		...
		var pasteImage = function ( editor, imageItem ) {
			...
			if ( isValidDataUriImage( editor.settings, img ) ) {
				...
				existingBlobInfo = blobCache.findFirst( function (
					cachedBlobInfo
				) {
		...
		var pasteImageData = function ( editor, e, rng ) {
			...
			if ( editor.settings.paste_data_images && dataTransfer ) {
				...
				if ( images.length > 0 ) {
					e.preventDefault();
					readBlobsAsDataUris( images ).get( function (
						blobResults
					) {
						if ( rng ) {
			...
			editor.on( 'keydown', function ( e ) {
				...
				if ( isKeyboardPasteEvent( e ) && ! e.isDefaultPrevented() ) {
					...
					if (
						keyboardPastePlainTextState &&
						global$2.webkit &&
						domGlobals.navigator.userAgent.indexOf( 'Version/' ) !==
							-1
					) {
						return;
					}
		...
		var setClipboardData = function ( evt, data, fallback, done ) {
			if (
				setHtml5Clipboard( evt.clipboardData, data.html, data.text )
			) {
				evt.preventDefault();
				done();
			} else {
