	...
	tinymce.PluginManager.add( 'wordpress', function ( editor ) {
				...
				event.content = event.content.replace(
					...
					function ( image ) {
						...
						if ( image.indexOf( 'data-wp-more="more"' ) !== -1 ) {
							...
							image.indexOf( 'data-wp-more="nextpage"' ) !== -1
						) {
							string = '<!--nextpage-->';
						} else if (
		...
		editor.on( 'preInit', function () {
			...
			editor.on( 'keydown', function ( event ) {
				...
				if ( tinymce.Env.mac ) {
					match =
						event.ctrlKey && event.altKey && event.code === 'KeyH';
				} else {
					match =
					...
					function reposition() {
						...
						var scrollX =
							...
							mceStatusbarTop = mceStatusbar
								? windowHeight -
								  mceStatusbar.getBoundingClientRect().top
								: 0,
							wpStatusbarTop = wpStatusbar
								? windowHeight -
								  wpStatusbar.getBoundingClientRect().top
								: 0,
							blockedTop = Math.max(
								0,
								wpAdminbarBottom,
							...
							spaceTop =
								selection.top + iframeRect.top - blockedTop,
							spaceBottom =
								windowHeight -
								iframeRect.top -
								selection.bottom -
								blockedBottom,
							editorHeight =
								windowHeight - blockedTop - blockedBottom,
							className = '',
							iosOffsetTop = 0,
							iosOffsetBottom = 0,
							top,
						...
						if ( typeof top === 'undefined' ) {
							top =
								scrollY + blockedTop + buffer + iosOffsetBottom;
						}
						...
						if (
							...
							( left < 0 &&
								selection.left + toolbarWidth > windowWidth ) ||
						...
						if (
							...
							if (
								...
								if ( cachedWinSize ) {
									if (
										size &&
										Math.abs( size - cachedWinSize.size ) <
											2
									) {
							...
							timeout = setTimeout( function () {
								if (
									activeToolbar &&
									typeof activeToolbar.show === 'function'
								) {
