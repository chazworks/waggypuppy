...
( function () {
	( function ( domGlobals ) {
		...
		var matchMediaQuery = function ( query ) {
			return 'matchMedia' in domGlobals.window
				? domGlobals.matchMedia( query ).matches
				: false;
		};
		opera = false;
		...
		ie11 =
			...
			( userAgent.indexOf( 'rv:' ) !== -1 ||
				nav.appName.indexOf( 'Netscape' ) !== -1 )
				? 11
				: false;
		ie12 =
			userAgent.indexOf( 'Edge/' ) !== -1 && ! ie && ! ie11 ? 12 : false;
		ie = ie || ie11 || ie12;
		gecko = ! webkit && ! ie11 && /Gecko/.test( userAgent );
		mac = userAgent.indexOf( 'Mac' ) !== -1;
		iDevice = /(iPad|iPhone)/.test( userAgent );
		fileApi =
			'FormData' in domGlobals.window &&
			'FileReader' in domGlobals.window &&
			'URL' in domGlobals.window &&
			!! URL.createObjectURL;
		phone =
			matchMediaQuery( 'only screen and (max-device-width: 480px)' ) &&
			( android || iDevice );
		tablet =
			matchMediaQuery( 'only screen and (min-width: 800px)' ) &&
			( android || iDevice );
		...
		var contentEditable =
			! iDevice ||
			fileApi ||
			parseInt( userAgent.match( /AppleWebKit\/(\d*)/ )[ 1 ], 10 ) >= 534;
		var Env = {
			opera: opera,
			webkit: webkit,
			ie: ie,
			...
			caretAfter: ie !== 8,
			range:
				domGlobals.window.getSelection && 'Range' in domGlobals.window,
			documentMode:
				ie && ! ie12 ? domGlobals.document.documentMode || 7 : 10,
			fileApi: fileApi,
		...
		var requestAnimationFrame = function ( callback, element ) {
			...
			for (
				i = 0;
				i < vendors.length && ! requestAnimationFrameFunc;
				i++
			) {
				requestAnimationFrameFunc =
					domGlobals.window[ vendors[ i ] + 'RequestAnimationFrame' ];
			}
		...
		var Delay = {
			requestAnimationFrame: function ( callback, element ) {
				...
				requestAnimationFramePromise = new promiseObj( function (
					resolve
				) {
					if ( ! element ) {
		...
		var fix = function ( originalEvent, data ) {
			...
			if ( Env.experimentalShadowDom ) {
				event.target = getTargetFromShadowDom(
					originalEvent,
					event.target
				);
			}
			if (
				...
				var eventDoc =
					event.target.ownerDocument || domGlobals.document;
		...
		var EventUtils = function () {
			...
			hasMouseEnterLeave =
				'onmouseenter' in domGlobals.document.documentElement;
			...
			var executeHandlers = function ( evt, id ) {
				...
				if ( callbackList ) {
					for ( i = 0, l = callbackList.length; i < l; i++ ) {
						...
						if (
							callback &&
							callback.func.call( callback.scope, evt ) === false
						) {
							evt.preventDefault();
						}
				...
				while ( i-- ) {
					...
					if (
						...
						nativeHandler = function ( evt ) {
							evt = fix( evt || win.event );
							evt.type =
								evt.type === 'focus' ? 'focusin' : 'focusout';
							executeHandlers( evt, id );
						};
					}
						...
						while ( i-- ) {
							...
							if ( callbackList ) {
								if ( callback ) {
									...
									while ( ci-- ) {
										if (
											callbackList[ ci ].func === callback
										) {
											var nativeHandler =
												callbackList.nativeHandler;
											var fakeName =
													callbackList.fakeName,
												capture = callbackList.capture;
											callbackList = callbackList
												.slice( 0, ci )
												.concat(
													callbackList.slice( ci + 1 )
												);
											callbackList.nativeHandler =
												nativeHandler;
											callbackList.fakeName = fakeName;
						...
						if (
							...
							if (
								...
								( b.ownerDocument === preferredDoc &&
									contains$1( preferredDoc, b ) )
							) {
								return 1;
							}
							return sortInput
								? indexOf$1.call( sortInput, a ) -
										indexOf$1.call( sortInput, b )
								: 0;
						}
		...
		Expr = Sizzle.selectors = {
			...
			pseudos: {
				...
				target: function ( elem ) {
					var hash =
						domGlobals.window.location &&
						domGlobals.window.location.hash;
		...
		var trim$1 = function ( str ) {
			return str === null || str === undefined
				? ''
				: ( '' + str ).replace( whiteSpaceRegExp$1, '' );
		};
				...
				if ( length === undefined ) {
					for ( key in obj ) {
						if ( obj.hasOwnProperty( key ) ) {
							value = obj[ key ];
							if (
								callback.call( value, key, value ) === false
							) {
								break;
							}
						}
		...
		DomQuery.fn = DomQuery.prototype = {
				...
				if ( isString$1( selector ) ) {
					...
					if ( match ) {
						if ( match[ 1 ] ) {
							...
							node = getElementDocument( context ).getElementById(
								match[ 2 ]
							);
							if ( ! node ) {
								return self;
			...
			text: function ( value ) {
				...
				return self[ 0 ]
					? self[ 0 ].innerText || self[ 0 ].textContent
					: '';
			},
			append: function () {
					...
					self.each( function ( index, node ) {
						...
						if ( classState !== state ) {
							...
							if ( classState ) {
								...
								node.className += existingClassName
									? ' ' + className
									: className;
							}
						}
		...
		Tools.extend( DomQuery, {
			...
			filter: function ( expr, elems, not ) {
				...
				if ( elems.length === 1 ) {
					elems = DomQuery.find.matchesSelector( elems[ 0 ], expr )
						? [ elems[ 0 ] ]
						: [];
				} else {
		...
		each$2(
			{
				...
				prevUntil: function ( node, until ) {
					return sibling( node, 'previousSibling', 1, until ).slice(
						1
					);
				},
			},
		...
		var browsers = [
			...
			},
			{
				...
				search: function ( uastring ) {
					return (
						contains$2( uastring, 'chrome' ) &&
						! contains$2( uastring, 'chromeframe' )
					);
				},
			},
			{
				...
				search: function ( uastring ) {
					return (
						contains$2( uastring, 'msie' ) ||
						contains$2( uastring, 'trident' )
					);
				},
			},
			{
		...
		var oses = [
			...
			},
			{
				...
				search: function ( uastring ) {
					return (
						contains$2( uastring, 'iphone' ) ||
						contains$2( uastring, 'ipad' )
					);
				},
				versionRegexes: [
					/.*?version\/\ ?([0-9]+)\.([0-9]+).*/,
		...
		var prevSibling = function ( element ) {
			return Option.from( element.dom().previousSibling ).map(
				Element.fromDom
			);
		};
		var nextSibling = function ( element ) {
			return Option.from( element.dom().nextSibling ).map(
				Element.fromDom
			);
		};
			...
			if ( browser$1.isFirefox() && name( elm ) === 'table' ) {
				return firstElement( children( elm ) )
					...
					.bind( function ( caption ) {
						return firstElement( nextSiblings( caption ) ).map(
							function ( body ) {
								var bodyTop = body.dom().offsetTop;
								var captionTop = caption.dom().offsetTop;
								var captionHeight = caption.dom().offsetHeight;
								return bodyTop <= captionTop
									? -captionHeight
									: 0;
							}
						);
		...
		var getPos = function ( body, elm, rootElm ) {
			...
			if ( elm ) {
				if (
					...
					get$1( Element.fromDom( body ), 'position' ) === 'static'
				) {
					pos = elm.getBoundingClientRect();
					x =
						pos.left +
		...
		function StyleSheetLoader( document, settings ) {
			...
			var load = function ( url, loadedCallback, errorCallback ) {
				...
				var isOldWebKit = function () {
					var webKitChunks =
						domGlobals.navigator.userAgent.match( /WebKit\/(\d*)/ );
					return !! (
						webKitChunks && parseInt( webKitChunks[ 1 ], 10 ) < 536
					);
				};
				...
				if ( 'onload' in link && ! isOldWebKit() ) {
					...
					if (
						domGlobals.navigator.userAgent.indexOf( 'Firefox' ) > 0
					) {
			...
			var findPreviousNode = function (
				...
				if ( node ) {
					...
					if ( sibling ) {
						if ( ! shallow ) {
							for (
								child = sibling[ startName ];
								child;
								child = child[ startName ]
							) {
								if ( ! child[ startName ] ) {
									return child;
								}
		...
		var lazyLookup = function ( items ) {
			...
			return function ( node ) {
				lookup = lookup
					? lookup
					: mapToObject( items, constant( true ) );
		...
		var trimNode = function ( dom, node ) {
			...
			if ( NodeType.isDocument( node ) === false ) {
				if ( NodeType.isText( node ) && node.nodeValue.length > 0 ) {
					...
					if (
						children.length === 1 &&
						isBookmarkNode( children[ 0 ] )
					) {
						node.parentNode.insertBefore( children[ 0 ], node );
					}
					if (
						children.length ||
						isVoid( Element.fromDom( node ) )
					) {
						return;
					}
				}
		...
		var encodeNumeric = function ( text, attr ) {
			return text.replace(
				...
				function ( chr ) {
					...
					return (
						baseEntities[ chr ] || '&#' + chr.charCodeAt( 0 ) + ';'
					);
				}
			);
		};
		...
		var decode = function ( text ) {
			return text.replace( entityRegExp, function ( all, numeric ) {
				if ( numeric ) {
					...
					return (
						asciiMap[ numeric ] || String.fromCharCode( numeric )
					);
				}
				return (
					reverseEntities[ all ] ||
		...
		function Schema( settings ) {
			...
			var customElementsMap = {},
				specialElements = {};
			var createLookupTable = function (
				option,
				defaultValue,
				extendWith
			) {
					...
					for ( ei = 0, el = validElements.length; ei < el; ei++ ) {
						...
						if ( matches ) {
							...
							if ( attrData ) {
								...
								for (
									ai = 0, al = attrData.length;
									ai < al;
									ai++
								) {
									matches = attrRuleRegExp.exec(
										attrData[ ai ]
									);
									if ( matches ) {
										attr = {};
										...
										if (
											...
											element.attributePatterns.push(
												attr
											);
										} else {
											if ( ! attributes[ attrName ] ) {
												attributesOrder.push(
													attrName
												);
											}
			...
			var addCustomElements = function ( customElements ) {
				...
				if ( customElements ) {
					mapCache.text_block_elements = mapCache.block_elements =
						null;
					each$4( split( customElements, ',' ), function ( rule ) {
						...
						each$4( children, function ( element, elmName ) {
							if ( element[ cloneName ] ) {
								children[ elmName ] = element = extend$1(
									{},
									children[ elmName ]
								);
			...
			if ( settings.invalid_elements ) {
				each$4(
					explode$1( settings.invalid_elements ),
					function ( item ) {
						if ( elements[ item ] ) {
		...
		function Styles( settings, schema ) {
			...
			return {
				...
				parse: function ( css ) {
					...
					var urlConverterScope =
						settings.url_converter_scope || this;
		...
		var setupAttrHooks = function ( styles, settings, getContext ) {
			...
			attrHooks = {
				style: {
					...
					get: function ( $elm ) {
						var value =
							$elm.attr( 'data-mce-style' ) ||
							$elm.attr( 'style' );
						value = styles.serialize(
							styles.parse( value ),
							$elm[ 0 ].nodeName
						);
						return value;
		...
		var updateInternalStyleAttr = function ( styles, $elm ) {
			...
			var value = styles.serialize(
				styles.parse( rawValue ),
				$elm[ 0 ].nodeName
			);
			if ( ! value ) {
				value = null;
		...
		function DOMUtils( doc, settings ) {
			...
			var isBlock = function ( node ) {
				if ( typeof node === 'string' ) {
					...
					if ( type ) {
						return !! (
							type === 1 && blockElementsMap[ node.nodeName ]
						);
					}
				}
				return false;
			...
			var select = function ( selector, scope ) {
				return Sizzle(
					selector,
					get( scope ) || settings.root_element || doc,
					[]
				);
			};
			...
			var isEmpty = function ( node, elements ) {
				...
				if ( node ) {
					...
					elements =
						elements ||
						( schema ? schema.getNonEmptyElements() : null );
					...
					do {
						...
						if (
							type === 3 &&
							! whiteSpaceRegExp$2.test( node.nodeValue )
						) {
							return false;
						}
						if (
			...
			var split = function ( parentElm, splitElm, replacementElm ) {
				...
				if ( parentElm && splitElm ) {
					...
					r.setStart(
						splitElm.parentNode,
						findNodeIndex( splitElm ) + 1
					);
					r.setEnd(
						parentElm.parentNode,
						findNodeIndex( parentElm ) + 1
					);
					aft = r.extractContents();
					pa = parentElm.parentNode;
					pa.insertBefore(
						TrimNode.trimNode( self, bef ),
						parentElm
					);
					if ( replacementElm ) {
			...
			var getContentEditable = function ( node ) {
				if ( node && NodeType.isElement( node ) ) {
					var contentEditable = node.getAttribute(
		...
		var ScriptLoader = function () {
			...
			var loadScript = function ( url, success, failure ) {
				...
				elm.onerror = error;
				(
					domGlobals.document.getElementsByTagName( 'head' )[ 0 ] ||
		...
		var identify = function ( editor, annotationName ) {
			...
			return closest.bind( function ( c ) {
				return getAttr( c, '' + dataAnnotationId() ).bind( function (
					uid
				) {
					return getAttr( c, '' + dataAnnotation() ).map( function (
						name
					) {
		...
		var findMarkers = function ( editor, uid ) {
			...
			return descendants$1(
				body,
				'[' + dataAnnotationId() + '="' + uid + '"]'
			);
		};
		var findAll = function ( editor, name ) {
			var body = Element.fromDom( editor.getBody() );
			var markers = descendants$1(
				body,
				'[' + dataAnnotation() + '="' + name + '"]'
			);
			var directory = {};
			each( markers, function ( m ) {
				...
				var nodesAlready = directory.hasOwnProperty( uid )
					? directory[ uid ]
					: [];
		...
		var setup = function ( editor, registry ) {
			...
			var updateCallbacks = function ( name, f ) {
				...
				var data = callbackMap.hasOwnProperty( name )
					? callbackMap[ name ]
					: initData();
		...
		var setup$1 = function ( editor, registry ) {
			var identifyParserNode = function ( span ) {
				var optAnnotation = Option.from(
					span.attributes.map[ dataAnnotation() ]
				);
		...
		var create$1 = function () {
			...
			var lookup = function ( name ) {
				return annotations.hasOwnProperty( name )
					? Option.from( annotations[ name ] ).map( function ( a ) {
							return a.settings;
					  } )
					: Option.none();
			};
		...
		var isBeforeInline = function ( pos ) {
			...
			return (
				...
				( pos.isAtStart() &&
					isCaretContainerInline( container.previousSibling ) )
			);
		};
		var isAfterInline = function ( pos ) {
			...
			return (
				...
				( pos.isAtEnd() &&
					isCaretContainerInline( container.nextSibling ) )
			);
		};
		...
		var endsWithCaretContainer = function ( node ) {
			return (
				isText$2( node ) &&
				node.data[ node.data.length - 1 ] === Zwsp.ZWSP
			);
		};
		...
		var isUnselectable = function ( node ) {
			return (
				...
				node.getAttribute( 'unselectable' ) === 'true'
			);
		};
		var isNonUiContentEditableFalse = function ( node ) {
			return (
				isUnselectable( node ) === false &&
				isContentEditableFalse$1( node )
			);
		};
		...
		var isAtomicContentEditableFalse = function ( node ) {
			...
			return (
				foldl(
					from$1( node.getElementsByTagName( '*' ) ),
					function ( result, elm ) {
		...
		var isValidOverflow = function ( overflowY, rect1, rect2 ) {
			return (
				overflowY >= 0 &&
				overflowY <= Math.min( rect1.height, rect2.height ) / 2
			);
		};
		...
		var lift3 = function ( oa, ob, oc, f ) {
			return oa.isSome() && ob.isSome() && oc.isSome()
				? Option.some(
						f( oa.getOrDie(), ob.getOrDie(), oc.getOrDie() )
				  )
				: Option.none();
		};
		...
		var isValidElementCaretCandidate = Predicate.and(
			isElement$3,
			isCaretCandidate$1,
			not( isFloated )
		);
		...
		var createRange = function ( doc ) {
			return 'createRange' in doc
				? doc.createRange()
				: DOMUtils$1.DOM.createRng();
		};
		...
		var getCaretPositionClientRects = function ( caretPosition ) {
			...
			if ( isElement$3( caretPosition.container() ) ) {
				if ( caretPosition.isAtEnd() ) {
					...
					if (
						isValidElementCaretCandidate( beforeNode ) &&
						! isBr$3( beforeNode )
					) {
						if (
							...
							addUniqueAndValidRect(
								collapseAndInflateWidth(
									getBoundingClientRect( beforeNode ),
									false
								)
							);
						}
		...
		var normalizedNodeIndex = function ( node ) {
			...
			nodes = ArrUtils.filter(
				nodes,
				NodeType.matchNodeNames( node.nodeName )
			);
			index = ArrUtils.findIndex( nodes, equal( node ), node );
		...
		var getPoint = function ( dom, trim, normalized, rng, start ) {
			...
			if ( NodeType.isText( container ) ) {
				point.push(
					normalized
						? getNormalizedTextOffset( trim, container, offset )
						: offset
				);
			} else {
				childNodes = container.childNodes;
				...
				point.push(
					dom.nodeIndex( childNodes[ offset ], normalized ) + after
				);
			}
			for (
				;
		...
		var getCaretBookmark = function ( selection ) {
			...
			return {
				start: create$2(
					...
					CaretPosition$1.fromRangeStart( rng )
				),
				end: create$2(
					selection.dom.getRoot(),
					CaretPosition$1.fromRangeEnd( rng )
				),
			};
		};
		...
		var getPersistentBookmark = function ( selection, filled ) {
			...
			if ( ! collapsed ) {
				rng2.collapse( false );
				var endBookmarkNode = createBookmarkSpan(
					dom,
					id + '_end',
					filled
				);
			...
			rng.collapse( true );
			var startBookmarkNode = createBookmarkSpan(
				dom,
				id + '_start',
				filled
			);
		...
		var removeTextAndReposition = function ( caretContainer, pos ) {
			var before = trimCount(
				caretContainer.data.substr( 0, pos.offset() )
			);
		...
		var removeElementAndReposition = function ( caretContainer, pos ) {
			...
			var newPosition = indexOf(
				from$1( parentNode.childNodes ),
				caretContainer
			)
				.map( function ( index ) {
					return index < pos.offset()
						? CaretPosition$1( parentNode, pos.offset() - 1 )
						: pos;
				} )
				.getOr( pos );
			remove$5( caretContainer );
		...
		var removeTextCaretContainer = function ( caretContainer, pos ) {
			return isText$6( caretContainer ) &&
				pos.container() === caretContainer
		...
		var removeAndReposition = function ( container, pos ) {
			return CaretPosition$1.isTextPosition( pos )
				? removeTextCaretContainer( container, pos )
				: removeElementCaretContainer( container, pos );
		};
		var remove$5 = function ( caretContainerNode ) {
			if (
				isElement$4( caretContainerNode ) &&
				isCaretContainer( caretContainerNode )
			) {
		...
		var trimInlineCaretContainers = function ( root ) {
			...
			contentEditableFalseNodes = DomQuery(
			...
			var show = function ( before, element ) {
				...
				if ( isBlock( element ) ) {
					...
					lastVisualCaret.get().each( function ( caretState ) {
						if ( before ) {
							DomQuery( caretState.caret ).addClass(
					...
					rng = element.ownerDocument.createRange();
					if (
						isContentEditableFalse$3(
							caretContainerNode.nextSibling
						)
					) {
						rng.setStart( caretContainerNode, 0 );
						rng.setEnd( caretContainerNode, 0 );
		...
		var isFakeCaretTableBrowser = function () {
			return (
				browser$2.isIE() || browser$2.isEdge() || browser$2.isFirefox()
			);
		};
		var isFakeCaretTarget = function ( node ) {
			return (
				isContentEditableFalse$3( node ) ||
				( NodeType.isTable( node ) && isFakeCaretTableBrowser() )
			);
		};

		var isContentEditableFalse$4 = NodeType.isContentEditableFalse;
		var isBlockLike = NodeType.matchStyleValues(
			'display',
		...
		var findNode = function (
			...
			if ( isBackwards( direction ) ) {
				if (
					isContentEditableFalse$4( node ) ||
					isCaretContainerBlock$1( node )
				) {
				...
				while (
					( node = skipCaretContainers( walker.prev, shallow ) )
				) {
					if ( predicateFn( node ) ) {
						return node;
					}
			...
			if ( isForwards( direction ) ) {
				...
				while (
					( node = skipCaretContainers( walker.next, shallow ) )
				) {
					if ( predicateFn( node ) ) {
						return node;
					}
		...
		var isInSameBlock = function (
			caretPosition1,
			caretPosition2,
			rootNode
		) {
			return (
				getParentBlock( caretPosition1.container(), rootNode ) ===
				getParentBlock( caretPosition2.container(), rootNode )
			);
		};
		var getChildNodeAtRelativeOffset = function (
			relativeOffset,
			caretPosition
		) {
		...
		var normalizeRange = function ( direction, root, range ) {
			...
			if ( NodeType.isText( container ) ) {
				...
				if (
					endsWithCaretContainer( container ) &&
		...
		var getRelativeCefElm = function ( forward, caretPosition ) {
			...
			).filter( isContentEditableFalse$4 );
		};
		...
		var getCaretCandidatePosition = function ( direction, node ) {
			if ( isForwards( direction ) ) {
				if (
					isCaretCandidate$3( node.previousSibling ) &&
					! isText$7( node.previousSibling )
				) {
			...
			if ( isBackwards( direction ) ) {
				if (
					isCaretCandidate$3( node.nextSibling ) &&
					! isText$7( node.nextSibling )
				) {
		...
		var moveForwardFromBr = function ( root, nextNode ) {
			...
			if ( nextSibling && isCaretCandidate$3( nextSibling ) ) {
				...
				return findCaretPosition(
					HDirection.Forwards,
					CaretPosition$1.after( nextNode ),
					root
				);
			}
		};
		var findCaretPosition = function ( direction, startPos, root ) {
			...
			if (
				startPos.isEqual( CaretPosition$1.after( root ) ) &&
				root.lastChild
			) {
				caretPosition = CaretPosition$1.after( root.lastChild );
				if (
					isBackwards( direction ) &&
					isCaretCandidate$3( root.lastChild ) &&
					isElement$6( root.lastChild )
				) {
					return isBr$4( root.lastChild )
						? CaretPosition$1.before( root.lastChild )
						: caretPosition;
				}
			} else {
			...
			rootContentEditableFalseElm = ArrUtils.last(
				filter(
					getParents( container, root ),
					isContentEditableFalse$5
				)
			);
			if (
				rootContentEditableFalseElm &&
				( ! nextNode ||
					! rootContentEditableFalseElm.contains( nextNode ) )
			) {
				if ( isForwards( direction ) ) {
					caretPosition = CaretPosition$1.after(
						rootContentEditableFalseElm
					);
				} else {
					caretPosition = CaretPosition$1.before(
						rootContentEditableFalseElm
					);
				}
				return caretPosition;
		...
		var walkToPositionIn = function ( forward, root, start ) {
			var position = forward
				? CaretPosition$1.before( start )
				: CaretPosition$1.after( start );
			return fromPosition( forward, root, position );
		};
		var afterElement = function ( node ) {
			return NodeType.isBr( node )
				? CaretPosition$1.before( node )
				: CaretPosition$1.after( node );
		};
		...
		var isBeforeAfterSameElement = function ( from, to ) {
			return (
				! CaretPosition$1.isTextPosition( from ) &&
				! CaretPosition$1.isTextPosition( to ) &&
				from.getNode() === to.getNode( true )
			);
		};
		var isAtBr = function ( position ) {
			return (
				! CaretPosition$1.isTextPosition( position ) &&
				NodeType.isBr( position.getNode() )
			);
		};
		...
		var fromPosition = function ( forward, root, pos ) {
			...
			return Option.from(
				forward ? walker.next( pos ) : walker.prev( pos )
			);
		};
		...
		var positionIn = function ( forward, element ) {
			...
			if ( NodeType.isText( startNode ) ) {
				return Option.some(
					CaretPosition$1(
						startNode,
						forward ? 0 : startNode.data.length
					)
				);
			} else if ( startNode ) {
				if ( isCaretCandidate( startNode ) ) {
					return Option.some(
						forward
							? CaretPosition$1.before( startNode )
							: afterElement( startNode )
					);
				} else {
		...
		var restoreEndPoint = function ( dom, suffix, bookmark ) {
			...
			if ( marker ) {
				...
				if ( suffix === 'start' ) {
					if ( ! keep ) {
						...
						if ( marker.hasChildNodes() ) {
							...
							isValidTextNode( marker.previousSibling )
						) {
				...
				if ( ! keep ) {
					...
					while (
						( marker = dom.get( bookmark.id + '_' + suffix ) )
					) {
		...
		var resolveId = function ( dom, bookmark ) {
			...
			return lift2(
				...
				function ( spos, epos ) {
					var rng = dom.createRng();
					rng.setStart(
						addBogus( dom, spos.container() ),
						spos.offset()
					);
					rng.setEnd(
						addBogus( dom, epos.container() ),
						epos.offset()
					);
					return rng;
				}
		...
		var resolveIndex$1 = function ( dom, bookmark ) {
			return Option.from(
				dom.select( bookmark.name )[ bookmark.index ]
			).map( function ( elm ) {
				var rng = dom.createRng();
				rng.selectNode( elm );
				return rng;
		...
		var resolve$3 = function ( selection, bookmark ) {
			...
			if ( bookmark ) {
				if ( isPathBookmark( bookmark ) ) {
					...
					return Option.some(
						resolveCaretPositionBookmark( dom, bookmark )
					);
		...
		var moveToBookmark = function ( selection, bookmark ) {
			ResolveBookmark.resolve( selection, bookmark ).each( function (
				rng
			) {
		...
		var findParentContentEditable = function ( dom, node ) {
			...
			while ( parent ) {
				if (
					parent.nodeType === 1 &&
					dom.getContentEditable( parent )
				) {
					return dom.getContentEditable( parent ) === 'false'
						? parent
						: node;
				}
				parent = parent.parentNode;
		...
		var findWordEndPoint = function (
			...
			walker = new TreeWalker(
				container,
				dom.getParent( container, dom.isBlock ) || body
			);
			while ( ( node = walker[ start ? 'prev' : 'next' ]() ) ) {
				if (
					node.nodeType === 3 &&
					! isBookmarkNode$2( node.parentNode )
				) {
					lastTextNode = node;
		...
		var findBlockEndPoint = function (
			...
			if ( node && format[ 0 ].wrapper ) {
				node =
					getParents$2( dom, node, 'ul,ol' ).reverse()[ 0 ] || node;
			}
			if ( ! node ) {
				node = container;
				while (
					node[ siblingName ] &&
					! dom.isBlock( node[ siblingName ] )
				) {
					node = node[ siblingName ];
		...
		var findParentContainer = function (
			...
			if (
				container.nodeType === 3 &&
				! isWhiteSpaceNode$1( container )
			) {
				if (
					start
						? startOffset > 0
						: endOffset < container.nodeValue.length
				) {
					return container;
				}
			}
			while ( true ) {
				...
				for (
					...
					if (
						! isBookmarkNode$2( sibling ) &&
						! isWhiteSpaceNode$1( sibling ) &&
						! isBogusBr( sibling )
					) {
						return parent;
					}
		...
		var expandRng = function ( editor, rng, format, remove ) {
			...
			if (
				isBookmarkNode$2( startContainer.parentNode ) ||
				isBookmarkNode$2( startContainer )
			) {
				startContainer = isBookmarkNode$2( startContainer )
					? startContainer
					: startContainer.parentNode;
				if ( rng.collapsed ) {
					startContainer =
						startContainer.previousSibling || startContainer;
				} else {
					startContainer =
						startContainer.nextSibling || startContainer;
				}
				if ( startContainer.nodeType === 3 ) {
			...
			if (
				isBookmarkNode$2( endContainer.parentNode ) ||
				isBookmarkNode$2( endContainer )
			) {
				endContainer = isBookmarkNode$2( endContainer )
					? endContainer
			...
			if ( format[ 0 ].inline ) {
				endContainer = remove
					? endContainer
					: excludeTrailingWhitespace( endContainer, endOffset );
			}
		...
		var walk$1 = function ( dom, rng, callback ) {
			...
			var walkBoundary = function ( startNode, endNode, next ) {
				...
				for (
					node = startNode, parent = node.parentNode;
					node && node !== endNode;
					node = parent
				) {
					parent = node.parentNode;
			...
			startPoint =
				findEndPoint( startContainer, ancestor ) || startContainer;
			endPoint = findEndPoint( endContainer, ancestor ) || endContainer;
			walkBoundary( startContainer, startPoint, true );
			siblings = collectSiblings(
				startPoint === startContainer
					? startPoint
		...
		var shouldApplyToTrailingSpaces = function ( rng ) {
			return (
				...
				rng.startContainer.nodeValue[ rng.startOffset ] === '\xA0'
			);
		};
		var applyWordGrab = function ( editor, rng ) {
			var r = ExpandRange.expandRng(
				editor,
				rng,
				[ { inline: true } ],
		...
		var annotateWithBookmark = function ( editor, name, settings, data ) {
			editor.undoManager.transact( function () {
				...
				if ( editor.selection.getRng().collapsed ) {
					...
					var bookmark = GetBookmark.getPersistentBookmark(
						editor.selection,
						false
					);
					var rng = editor.selection.getRng();
		...
		function Annotator( editor ) {
			...
			return {
				...
				remove: function ( name ) {
					identify( editor, Option.some( name ) ).each( function (
						_a
					) {
		...
		var isEmptyFragmentElement = function ( schema, node ) {
			...
			return (
				node &&
				( node.isEmpty( nonEmptyElements ) ||
					isPaddedEmptyBlock( schema, node ) )
			);
		};
		...
		var getParentLi = function ( dom, node ) {
			...
			return parentBlock && parentBlock.nodeName === 'LI'
				? parentBlock
				: null;
		};
		...
		var insertAtCaret = function ( serializer, dom, rng, fragment ) {
			...
			var isAt = function ( location ) {
				...
				return newPos
					? getParentLi( dom, newPos.getNode() ) !== liTarget
					: true;
			};
			if ( isAt( BEGINNING ) ) {
		...
		var ElementUtils = function ( dom ) {
			this.compare = function ( node1, node2 ) {
				...
				if (
					! compareObjects( getAttribs( node1 ), getAttribs( node2 ) )
				) {
					return false;
				}
				if (
				...
				return (
					! Bookmarks.isBookmarkNode( node1 ) &&
					! Bookmarks.isBookmarkNode( node2 )
				);
			};
		};
		...
		var trimBlockTrailingBr = function ( elm ) {
			lastChild( elm ).each( function ( lastChild ) {
				prevSibling( lastChild ).each( function (
					lastChildPrevSibling
				) {
					if (
						isBlock( elm ) &&
						isBr( lastChild ) &&
						isBlock( lastChildPrevSibling )
					) {
						remove$1( lastChild );
					}
				} );
		...
		function HtmlSerializer( settings, schema ) {
			...
			settings = settings || {};
			settings.validate =
				'validate' in settings ? settings.validate : true;
					...
					if ( ! handler ) {
						...
						if ( validate && attrs && attrs.length > 1 ) {
							...
							if ( elementRule ) {
								for (
									i = 0,
										l = elementRule.attributesOrder.length;
									i < l;
									i++
								) {
		...
		var isAfterNbsp = function ( container, offset ) {
			return (
				NodeType.isText( container ) &&
				container.nodeValue[ offset - 1 ] === '\xA0'
			);
		};
		var trimOrPadLeftRight = function ( rng, html ) {
			...
			var hasSiblingText = function ( siblingName ) {
				return (
					container[ siblingName ] &&
					container[ siblingName ].nodeType === 3
				);
			};
			if ( container.nodeType === 3 ) {
				if ( offset > 0 ) {
		...
		var reduceInlineTextElements = function ( editor, merge ) {
			...
			if ( merge ) {
				...
				Tools.each(
					dom.select( '*[data-mce-fragment]' ),
					function ( node ) {
						for (
							var testNode = node.parentNode;
							testNode && testNode !== root_1;
							...
							if (
								...
								elementUtils_1.compare( testNode, node )
							) {
		...
		var canHaveChildren = function ( editor, node ) {
			return (
				node && ! editor.schema.getShortEndedElements()[ node.nodeName ]
			);
		};
		...
		var insertHtmlAtCaret = function ( editor, value, details ) {
			...
			if ( ! selection.isCollapsed() ) {
				...
				value = trimNbspAfterDeleteAndPadValue(
		...
		var getBodySetting = function ( editor, name, defaultValue ) {
			...
			if ( value.indexOf( '=' ) !== -1 ) {
				...
				return bodyObj.hasOwnProperty( editor.id )
					? bodyObj[ editor.id ]
					: defaultValue;
			} else {
				return value;
		...
		var getFontStyleValues = function ( editor ) {
			return Tools.explode(
				editor.getParam( 'font_size_style_values', '' )
			);
		};
		...
		var isRtl = function ( element ) {
			return (
				DOMUtils$1.DOM.getStyle( element, 'direction', true ) ===
					'rtl' || hasStrongRtl( element.textContent )
			);
		};
		var findInlineParents = function ( isInlineTarget, rootNode, pos ) {
			return filter(
				DOMUtils$1.DOM.getParents( pos.container(), '*', rootNode ),
				isInlineTarget
			);
		};
		...
		var normalizePosition = function ( forward, pos ) {
			...
			if ( forward ) {
				if ( isCaretContainerInline( container ) ) {
					...
					return isBeforeInline( pos )
						? CaretPosition$1( container, offset + 1 )
						: pos;
				}
			} else {
				if ( isCaretContainerInline( container ) ) {
		...
		var getParentBlock$1 = function ( rootNode, elm ) {
			return contains$3( rootNode, elm )
				? closest(
						elm,
						function ( element ) {
							return (
		...
		var willDeleteLastPositionInElement = function (
			forward,
			fromPos,
			elm
		) {
			return lift2(
				...
				function ( firstPos, lastPos ) {
					var normalizedFirstPos = InlineUtils.normalizePosition(
						true,
						firstPos
					);
					var normalizedLastPos = InlineUtils.normalizePosition(
						false,
						lastPos
					);
					var normalizedFromPos = InlineUtils.normalizePosition(
						false,
						fromPos
					);
					if ( forward ) {
						return CaretFinder.nextPosition(
							elm,
							normalizedFromPos
						)
							.map( function ( nextPos ) {
								return (
						...
						return CaretFinder.prevPosition(
							elm,
							normalizedFromPos
						)
							.map( function ( prevPos ) {
								return (
		...
		var isBogusAll$1 = NodeType.hasAttributeValue(
			'data-mce-bogus',
			'all'
		);
		...
		var getBlockPosition = function ( rootNode, pos ) {
			...
			return DeleteUtils.getParentBlock( rootElm, containerElm ).map(
				function ( block ) {
		...
		var hasSameParent = function ( blockBoundary ) {
			return parent( blockBoundary.from().block() )
				.bind( function ( parent1 ) {
					return parent( blockBoundary.to().block() ).filter(
						function ( parent2 ) {
		...
		var isEditable = function ( blockBoundary ) {
			return (
				NodeType.isContentEditableFalse(
					blockBoundary.from().block().dom()
				) === false &&
				NodeType.isContentEditableFalse(
					blockBoundary.to().block().dom()
				) === false
			);
		};
		var skipLastBr = function ( rootNode, forward, blockPosition ) {
			if (
				...
				return CaretFinder.positionIn(
					false,
					blockPosition.block().dom()
				)
					.bind( function ( lastPositionInBlock ) {
						if (
							lastPositionInBlock.isEqual(
		...
		var readFromRange = function ( rootNode, forward, rng ) {
			...
			var toBlockPos = fromBlockPos.bind( function ( blockPos ) {
				return CaretFinder.fromPosition(
					...
					return getBlockPosition( rootNode, to ).map( function (
						blockPos
					) {
		...
		var getChildrenUntilBlockBoundary = function ( block ) {
			...
			return findIndex( children$1, isBlock ).fold(
				function () {
					return children$1;
				},
				function ( index ) {
		...
		var isEmptyBefore = function ( el ) {
			return (
				filter( prevSiblings( el ), function ( el ) {
		...
		var nestedBlockMerge = function (
			...
			if (
				isEmptyBefore( insertionPoint ) &&
				Empty.isEmpty( fromBlock )
			) {
				before( insertionPoint, Element.fromTag( 'br' ) );
			}
			var position = CaretFinder.prevPosition(
				toBlock.dom(),
				CaretPosition$1.before( insertionPoint.dom() )
			);
		...
		var getInsertionPoint = function ( fromBlock, toBlock ) {
			return contains$3( toBlock, fromBlock )
		...
		var backspaceDelete = function ( editor, forward ) {
			...
			position = BlockMergeBoundary.read(
				rootNode.dom(),
				forward,
				editor.selection.getRng()
		...
		var isEverythingSelected = function ( root, rng ) {
			var noPrevious = CaretFinder.prevPosition(
				root.dom(),
				CaretPosition$1.fromRangeStart( rng )
			).isNone();
			var noNext = CaretFinder.nextPosition(
				root.dom(),
				CaretPosition$1.fromRangeEnd( rng )
			).isNone();
		...
		var findBr = function ( forward, root, pos ) {
			var parentBlocks = filter(
				Parents.parentsAndSelf(
			...
			return CaretFinder.fromPosition( forward, scope.dom(), pos ).filter(
				isBr$5
			);
		};
		var isBeforeBr = function ( root, pos ) {
			return (
				getElementFromPosition( pos ).exists( isBr ) ||
				findBr( true, root, pos ).isSome()
			);
		};
		var isAfterBr = function ( root, pos ) {
			return (
				getElementFromPrevPosition( pos ).exists( isBr ) ||
		...
		var isChar = function ( forward, predicate, pos ) {
			return Option.from( pos.container() )
				.filter( NodeType.isText )
				.exists( function ( text ) {
					var delta = forward ? 0 : -1;
					return predicate(
						text.data.charAt( pos.offset() + delta )
					);
				} );
		};
		...
		var isNextToContentEditableFalse = function (
			relativeOffset,
			caretPosition
		) {
			var node = getChildNodeAtRelativeOffset(
				relativeOffset,
				caretPosition
			);
			return (
				NodeType.isContentEditableFalse( node ) &&
				! NodeType.isBogusAll( node )
			);
		};
		var isBeforeContentEditableFalse = curry(
			isNextToContentEditableFalse,
			0
		);
		var isAfterContentEditableFalse = curry(
			isNextToContentEditableFalse,
			-1
		);
		var isNextToTable = function ( relativeOffset, caretPosition ) {
			return NodeType.isTable(
				getChildNodeAtRelativeOffset( relativeOffset, caretPosition )
			);
		};
		...
		var isCompoundElement = function ( node ) {
			return (
				isTableCell( Element.fromDom( node ) ) ||
				isListItem( Element.fromDom( node ) )
			);
		};
		...
		var isAtContentEditableBlockCaret = function ( forward, from ) {
			...
			return (
				NodeType.isElement( elm ) &&
				elm.getAttribute( 'data-mce-caret' ) === caretLocation
			);
		};
		var isDeleteFromCefDifferentBlocks = function (
			root,
			forward,
			from,
			to
			...
			var inSameBlock = function ( elm ) {
				return (
					isInline( Element.fromDom( elm ) ) &&
					! isInSameBlock( from, to, root )
				);
			};
			return getRelativeCefElm( ! forward, from ).fold( function () {
				return getRelativeCefElm( forward, to ).fold(
					constant( false ),
					inSameBlock
				);
			}, inSameBlock );
		...
		var deleteEmptyBlockOrMoveToCef = function ( root, forward, from, to ) {
			...
			return DeleteUtils.getParentBlock(
				Element.fromDom( root ),
				Element.fromDom( from.getNode() )
			)
				.map( function ( blockElm ) {
					return Empty.isEmpty( blockElm )
		...
		var findCefPosition = function ( root, forward, from ) {
			return CaretFinder.fromPosition( forward, root, from ).bind(
				function ( to ) {
					if ( isCompoundElement( to.getNode() ) ) {
						...
						forward &&
						NodeType.isContentEditableFalse( to.getNode() )
					) {
						return deleteEmptyBlockOrMoveToCef(
							root,
							forward,
							from,
							to
						...
						forward === false &&
						NodeType.isContentEditableFalse( to.getNode( true ) )
					) {
						return deleteEmptyBlockOrMoveToCef(
							root,
							forward,
		...
		var getContentEditableBlockAction = function ( forward, elm ) {
			if (
				forward &&
				NodeType.isContentEditableFalse( elm.nextSibling )
			) {
				return Option.some(
					DeleteAction.moveToElement( elm.nextSibling )
				);
			} else if (
				forward === false &&
				NodeType.isContentEditableFalse( elm.previousSibling )
			) {
				return Option.some(
					DeleteAction.moveToElement( elm.previousSibling )
				);
			} else {
				return Option.none();
			}
		...
		var getContentEditableAction = function ( root, forward, from ) {
			if ( isAtContentEditableBlockCaret( forward, from ) ) {
				...
				return findCefPosition( root, forward, from ).bind( function (
					deleteAction
				) {
					return skipMoveToActionFromInlineCefToContent(
						root,
						from,
						deleteAction
					);
		...
		var read$2 = function ( root, forward, rng ) {
			...
			if ( forward === false && isAfterContentEditableFalse( from ) ) {
				return Option.some(
					DeleteAction.remove( from.getNode( true ) )
				);
				...
				return findPreviousBr( rootElement, from ).map( function (
					br
				) {
				...
				function ( acc, c ) {
					if ( isCollapsibleWhitespace( c ) || c === '\xA0' ) {
						if (
							...
							( acc.str.length === content.length - 1 &&
								isEndOfContent )
						) {
							return {
		...
		var needsReposition = function ( pos, elm ) {
			...
			return (
				CaretPosition$1.isTextPosition( pos ) === false &&
				container === elm.parentNode &&
				offset > CaretPosition$1.before( elm ).offset()
			);
		};
		var reposition = function ( elm, pos ) {
			return needsReposition( pos, elm )
				? CaretPosition$1( pos.container(), pos.offset() - 1 )
				: pos;
		};
		var beforeOrStartOf = function ( node ) {
			return NodeType.isText( node )
				? CaretPosition$1( node, 0 )
				: CaretPosition$1.before( node );
		};
		var afterOrEndOf = function ( node ) {
			return NodeType.isText( node )
				? CaretPosition$1( node, node.data.length )
				: CaretPosition$1.after( node );
		};
		var getPreviousSiblingCaretPosition = function ( elm ) {
			if ( isCaretCandidate( elm.previousSibling ) ) {
				...
				return elm.previousSibling
					? CaretFinder.lastPositionIn( elm.previousSibling )
					: Option.none();
			}
		};
		var getNextSiblingCaretPosition = function ( elm ) {
			if ( isCaretCandidate( elm.nextSibling ) ) {
				...
				return elm.nextSibling
					? CaretFinder.firstPositionIn( elm.nextSibling )
					: Option.none();
			}
		};
		var findCaretPositionBackwardsFromElm = function ( rootElement, elm ) {
			var startPosition = CaretPosition$1.before(
		...
		var findCaretPositionBackwards = function ( rootElement, elm ) {
			return getPreviousSiblingCaretPosition( elm )
				...
				.orThunk( function () {
					return findCaretPositionBackwardsFromElm(
						rootElement,
						elm
					);
				} );
		...
		var findCaretPosOutsideElmAfterDelete = function (
			forward,
			rootElement,
			elm
		) {
			return findCaretPosition$1( forward, rootElement, elm ).map(
		...
		var isBlock$2 = function ( editor, elm ) {
			return (
				elm &&
				editor.schema.getBlockElements().hasOwnProperty( name( elm ) )
			);
		};
			...
			return lift3(
				...
				function ( prev, next, pos ) {
					...
					return pos.container() === nextNode
						? CaretPosition$1( prevNode, offset )
						: pos;
				}
			).orThunk( function () {
				if ( normalizeWhitespace ) {
					prevTextOpt.each( function ( elm ) {
						return normalizeWhitespaceBefore(
							elm.dom(),
							elm.dom().length
						);
					} );
		...
		var isInlineElement = function ( editor, element ) {
			return has(
				editor.schema.getTextInlineElements(),
				name( element )
			);
		};
		var deleteElement = function ( editor, forward, elm, moveCaret ) {
			...
			var parentBlock = ancestor(
				elm,
				curry( isBlock$2, editor ),
		...
		var moveToElement = function ( editor, forward ) {
			return function ( element ) {
				var pos = forward
					? CaretPosition$1.before( element )
					: CaretPosition$1.after( element );
				editor.selection.setRng( pos.toRange() );
				return true;
			};
		};
		...
		var deleteOffscreenSelection = function ( rootElement ) {
			each(
				descendants$1( rootElement, '.mce-offscreen-selection' ),
				remove$1
			);
		};
		...
		var paddEmptyElement = function ( editor ) {
			var br;
			var ceRoot = getContentEditableRoot(
				editor.getBody(),
			...
			if (
				...
				editor.selection.setRng(
					CaretPosition$1.before( br ).toRange()
				);
			}
			return true;
		};
		...
		var renderCaret = function ( caret, location ) {
			return location.fold(
				function ( element ) {
					...
					caret.set( text );
					return Option.some(
						CaretPosition$1( text, text.length - 1 )
					);
				},
				function ( element ) {
					return CaretFinder.firstPositionIn( element ).map(
						function ( pos ) {
				...
				function ( element ) {
					return CaretFinder.lastPositionIn( element ).map( function (
						pos
					) {
						if ( ! isPosCaretContainer( pos, caret ) ) {
							CaretContainerRemove.remove( caret.get() );
		...
		var before$3 = function ( isInlineTarget, rootNode, pos ) {
			...
			return InlineUtils.findRootInline(
				isInlineTarget,
				scope,
				nPos
			).fold( function () {
		...
		var findInsideRootInline = function ( isInlineTarget, rootNode, pos ) {
			return InlineUtils.findRootInline(
				...
				rootNode,
				pos
			).filter( curry( isNotInsideFormatCaretContainer, rootNode ) );
		};
			...
			return findInsideRootInline( isInlineTarget, rootNode, nPos ).bind(
				function ( inline ) {
					...
					return prevPos.isNone()
						? Option.some( Location.start( inline ) )
						: Option.none();
				}
			);
		};
			...
			return findInsideRootInline( isInlineTarget, rootNode, nPos ).bind(
				function ( inline ) {
					...
					return nextPos.isNone()
						? Option.some( Location.end( inline ) )
						: Option.none();
				}
			);
		};
		...
		var betweenInlines = function (
			...
			return lift2(
				...
				function ( fromInline, toInline ) {
					if (
						fromInline !== toInline &&
						InlineUtils.hasSameParentBlock(
							rootNode,
							fromInline,
							toInline
						)
		...
		var findLocationTraverse = function (
			...
			var to = CaretFinder.fromPosition( forward, rootNode, from ).map(
				curry( InlineUtils.normalizePosition, forward )
			);
		...
		var findLocationSimple = function ( forward, location ) {
			if ( forward ) {
				return location.fold(
					compose( Option.some, Location.start ),
					Option.none,
					compose( Option.some, Location.after ),
					Option.none
				);
			} else {
				return location.fold(
					Option.none,
					compose( Option.some, Location.before ),
					Option.none,
					compose( Option.some, Location.end )
				);
			}
		};
		...
		var moveRel = function ( forward, selection, pos ) {
			...
			selection
				.getSel()
				.modify( 'move', forward ? 'forward' : 'backward', 'word' );
			return true;
		};
		...
		var renderCaretLocation = function ( editor, caret, location ) {
			return BoundaryCaret.renderCaret( caret, location ).map( function (
				pos
			) {
		...
		var findLocation$1 = function ( editor, caret, forward ) {
			...
			var from = CaretPosition$1.fromRangeStart(
				editor.selection.getRng()
			);
			var isInlineTarget = curry( InlineUtils.isInlineTarget, editor );
			var location = BoundaryLocation.findLocation(
				forward,
				isInlineTarget,
				rootNode,
				from
		...
		var toggleInlines = function ( isInlineTarget, dom, elms ) {
			var selectedInlines = filter(
				dom.select( '*[data-mce-selected="inline-boundary"]' ),
				isInlineTarget
			);
		...
		var safeRemoveCaretContainer = function ( editor, caret ) {
			if (
				...
				var pos = CaretPosition$1.fromRangeStart(
					editor.selection.getRng()
				);
				if (
					CaretPosition$1.isTextPosition( pos ) &&
					InlineUtils.isAtZwsp( pos ) === false
				) {
					setCaretPosition(
						editor,
						CaretContainerRemove.removeAndReposition(
							caret.get(),
							pos
						)
					);
		...
		var renderInsideInlineCaret = function (
			isInlineTarget,
			editor,
			caret,
			elms
		...
		var move = function ( editor, caret, forward ) {
			return function () {
				return isFeatureEnabled( editor )
					? findLocation$1( editor, caret, forward ).isSome()
					: false;
			};
		};
		var moveWord = function ( forward, editor, caret ) {
			return function () {
				return isFeatureEnabled( editor )
					? WordSelection.moveByWord( forward, editor )
					: false;
			};
		};
		...
		var deleteFromTo = function ( editor, caret, from, to ) {
			...
			editor.undoManager.ignore( function () {
				...
				BoundaryLocation.readLocation(
					isInlineTarget,
					rootNode,
					CaretPosition$1.fromRangeStart( editor.selection.getRng() )
				)
		...
		var backspaceDeleteCollapsed = function (
			editor,
			caret,
			forward,
			from
			...
			var isInlineTarget = curry( InlineUtils.isInlineTarget, editor );
			var fromLocation = BoundaryLocation.readLocation(
				isInlineTarget,
				rootNode,
				from
			);
			return fromLocation
				.bind( function ( location ) {
					if ( forward ) {
						return location.fold(
							constant(
								Option.some(
									BoundaryLocation.inside( location )
								)
							),
							Option.none,
							constant(
								Option.some(
									BoundaryLocation.outside( location )
								)
							),
							Option.none
						);
				...
				.map( setCaretLocation( editor, caret ) )
				.getOrThunk( function () {
					var toPosition = CaretFinder.navigate(
						forward,
						rootNode,
						from
					);
		...
		var getTableFromCellRng = function ( cellRng, isRoot ) {
			return getClosestTable( cellRng.start(), isRoot ).bind( function (
				...
				return getClosestTable( cellRng.end(), isRoot ).bind( function (
					...
					return eq( startParentTable, endParentTable )
						? Option.some( startParentTable )
						: Option.none();
				} );
			} );
		};
		...
		var getCellRangeFromStartTable = function ( cellRng, isRoot ) {
			return getClosestTable( cellRng.start(), isRoot ).bind( function (
				...
				return last( getTableCells( table ) ).map( function (
					endCell
				) {
		...
		var partialSelection = function ( isRoot, rng ) {
			...
			return rng.collapsed
				? Option.none()
				: lift2( startCell, endCell, tableCellRng ).fold(
						function () {
							return startCell.fold(
								function () {
									return endCell.bind( function ( endCell ) {
										return getClosestTable(
											endCell,
											isRoot
										).bind( function ( table ) {
											return head(
												getTableCells( table )
											).map( function ( startCell ) {
												return tableCellRng(
													startCell,
													endCell
												);
						...
						function ( cellRng ) {
							return isWithinSameTable( isRoot, cellRng )
								? Option.none()
								: getCellRangeFromStartTable( cellRng, isRoot );
						}
				  );
		};
		...
		var getTableSelectionFromCellRng = function ( cellRng, isRoot ) {
			return getTableFromCellRng( cellRng, isRoot ).map( function (
				table
			) {
		...
		var getSelectedCells = function ( tableSelection ) {
			return lift2(
				getCellIndex(
					tableSelection.cells(),
					tableSelection.rng().start()
				),
				getCellIndex(
					tableSelection.cells(),
					tableSelection.rng().end()
				),
		...
		var getAction = function ( tableSelection ) {
			return getSelectedCells( tableSelection ).map( function (
				selected
			) {
				var cells = tableSelection.cells();
				return selected.length === cells.length
					? deleteAction.removeTable( tableSelection.table() )
		...
		var getCellsFromElement = function ( elm ) {
			var selectedCells = descendants$1(
				elm,
		...
		var deleteCellRange = function ( editor, rootElm, rng ) {
			return TableDeleteAction.getActionFromRange( rootElm, rng ).map(
				function ( action ) {
					return action.fold(
						curry( deleteTableElement, editor ),
						curry( emptyCells, editor )
					);
				}
			);
		};
		...
		var getParentCaption = function ( rootElm, elm ) {
			return find(
				Parents.parentsAndSelf( elm, rootElm ),
				function ( elm ) {
					return name( elm ) === 'caption';
				}
			);
		};
		var deleteBetweenCells = function (
			editor,
			rootElm,
			forward,
			fromCell,
			...
			return CaretFinder.navigate( forward, editor.getBody(), from ).bind(
				function ( to ) {
					return getParentCell(
						rootElm,
						Element.fromDom( to.getNode() )
					).map( function ( toCell ) {
		...
		var isDeleteOfLastCharPos = function (
			fromCaption,
			forward,
			from,
			to
		) {
			return CaretFinder.firstPositionIn( fromCaption.dom() )
				.bind( function ( first ) {
					return CaretFinder.lastPositionIn( fromCaption.dom() ).map(
						function ( last ) {
							return forward
		...
		var deleteCaretInsideCaption = function (
			...
			return CaretFinder.navigate( forward, editor.getBody(), from )
				.bind( function ( to ) {
					return isDeleteOfLastCharPos(
						fromCaption,
						forward,
						from,
						to
		...
		var deleteCaretCells = function ( editor, forward, rootElm, startElm ) {
			...
			return getParentCell( rootElm, startElm ).bind( function (
				fromCell
			) {
				return Empty.isEmpty( fromCell )
		...
		var deleteCaretCaption = function (
			editor,
			forward,
			rootElm,
			fromCaption
		...
		var matchParents = function ( editor, node, name, vars ) {
			...
			node = editor.dom.getParent( node, function ( node ) {
				...
				return (
					...
					!! matchNode( editor, node, name, vars, true )
				);
			} );
			...
			editor.dom.getParent(
				...
				function ( node ) {
					...
					for ( i = 0; i < names.length; i++ ) {
						...
						if (
							! checkedMap[ name ] &&
							matchNode( editor, node, name, vars )
						) {
		...
		var getEmptyCaretContainers = function ( node ) {
			...
			while ( node ) {
				if (
					( node.nodeType === 3 && node.nodeValue !== ZWSP$1 ) ||
		...
		var insertCaretContainerNode = function (
			editor,
			caretContainer,
			formatNode
		) {
			var dom = editor.dom,
				block = dom.getParent(
					formatNode,
					curry( FormatUtils.isTextBlock, editor )
				);
			if ( block && dom.isEmpty( block ) ) {
		...
		var removeCaretFormat = function ( editor, name, vars, similar ) {
			...
			while ( node ) {
				if (
					MatchFormat.matchNode( editor, node, name, vars, similar )
				) {
					formatNode = node;
					break;
				}
			...
			if ( hasContentAfter ) {
				bookmark = selection.getBookmark();
				rng.collapse( true );
				var expandedRng = ExpandRange.expandRng(
					editor,
					rng,
					editor.formatter.get( name ),
					true
				...
				var caretNode = insertFormatNodesIntoCaretContainer(
					parents,
					newCaretContainer
				);
				if ( caretContainer ) {
		...
		var disableCaretContainer = function ( editor, keyCode ) {
			...
			if (
				...
				selection.getStart().innerHTML === ZWSP$1
			) {
		...
		var isFormatElement = function ( editor, element ) {
			...
			return (
				inlineElements.hasOwnProperty( name( element ) ) &&
		...
		var isEmptyCaretFormatElement = function ( element ) {
			return (
				isCaretNode( element.dom() ) &&
				isCaretContainerEmpty( element.dom() )
			);
		};

		var getParentInlines = function ( rootElm, startElm ) {
			var parents = Parents.parentsAndSelf( startElm, rootElm );
			return findIndex( parents, isBlock ).fold(
				constant( parents ),
				function ( index ) {
		...
		var deleteLastPosition = function (
			forward,
			editor,
			target,
			parentInlines
			...
			var formatNodes = map(
				filter( parentInlines, isFormatElement$1 ),
				function ( elm ) {
					return elm.dom();
				}
			);
		...
		var deleteCaret$1 = function ( editor, forward ) {
			...
			var parentInlines = filter(
				getParentInlines( rootElm, startElm ),
				hasOnlyOneChild$1
			);
			return last( parentInlines )
				.map( function ( target ) {
					var fromPos = CaretPosition$1.fromRangeStart(
						editor.selection.getRng()
					);
					if (
		...
		var scrollElementIntoView = function ( editor, elm, alignToTop ) {
			...
			if ( root.nodeName !== 'BODY' ) {
				...
				if ( scrollContainer ) {
					y =
						getPos$1( elm ).y -
						getPos$1( scrollContainer ).y +
						offsetY;
					viewPortH = scrollContainer.clientHeight;
					viewPortY = scrollContainer.scrollTop;
					if ( y < viewPortY || y + 25 > viewPortY + viewPortH ) {
						scrollContainer.scrollTop =
							y < viewPortY ? y : y - viewPortH + 25;
					}
					return;
				}
			}
		...
		var scrollRangeIntoView = function ( editor, rng ) {
			head( CaretPosition.fromRangeStart( rng ).getClientRects() ).each(
				function ( rngRect ) {
		...
		var showCaret = function (
			...
			return editor._selectionOverrides.showCaret(
				direction,
				node,
				before,
				scrollIntoView
		...
		var renderCaretAtRange = function ( editor, range, scrollIntoView ) {
			...
			var caretPosition =
				CaretPosition$1.fromRangeStart( normalizedRange );
			...
			if ( isContentEditableFalse$6( caretPositionBeforeNode ) ) {
				return showCaret(
					1,
					editor,
					caretPositionBeforeNode,
					false,
					false
				);
			}
			var ceRoot = editor.dom.getParent(
				caretPosition.getNode(),
				function ( node ) {
					return (
						isContentEditableFalse$6( node ) ||
						isContentEditableTrue$2( node )
					);
				}
			);
		...
		var renderRangeCaret = function ( editor, range, scrollIntoView ) {
			...
			var caretRange = renderCaretAtRange(
				editor,
				range,
				scrollIntoView
			);
		...
		var moveToRange = function ( editor, rng ) {
			editor.selection.setRng( rng );
			ScrollIntoView.scrollRangeIntoView(
				editor,
				editor.selection.getRng()
			);
		};
		...
		var deleteCefBoundaryText = function ( editor, forward ) {
			...
			var direction = forward
				...
				: HDirection.Backwards;
			var caretWalker = CaretWalker( editor.getBody() );
			var getNextVisualCaretPosition = curry(
				getVisualCaretPosition,
				caretWalker.next
			);
			var getPrevVisualCaretPosition = curry(
				getVisualCaretPosition,
				caretWalker.prev
			);
			var getNextPosFn = forward
				? getNextVisualCaretPosition
				: getPrevVisualCaretPosition;
			var isBeforeContentEditableFalseFn = forward
			...
			var nextCaretPosition = InlineUtils.normalizePosition(
				forward,
			...
			if (
				peekCaretPosition &&
				isBeforeContentEditableFalseFn( peekCaretPosition )
			) {
				if (
		...
		var deleteCommand = function ( editor ) {
			if ( CefDelete.backspaceDelete( editor, false ) ) {
				...
				InlineBoundaryDelete.backspaceDelete( editor, false )
			) {
				return;
		...
		var getComputedFontProp = function ( propName, elm ) {
			return Option.from(
				DOMUtils$1.DOM.getStyle( elm, propName, true )
			);
		};
		...
		var findFirstCaretElement = function ( editor ) {
			return CaretFinder.firstPositionIn( editor.getBody() ).map(
				function ( caret ) {
					var container = caret.container();
					return NodeType.isText( container )
		...
		var getCaretElement = function ( editor ) {
			return Option.from( editor.selection.getRng() ).bind( function (
				rng
			) {
				var root = editor.getBody();
		...
		var fontNameQuery = function ( editor ) {
			return getCaretElement( editor ).fold(
				function () {
					return findFirstCaretElement( editor )
						.map( function ( caretElement ) {
		...
		var isCeFalseCaretContainer = function ( node, rootNode ) {
			return (
				isCaretContainer( node ) &&
				hasParent( node, rootNode, isCaretNode ) === false
			);
		};
		...
		var isPrevNode = function ( node, name ) {
			return (
				node.previousSibling && node.previousSibling.nodeName === name
			);
		};
		...
		var normalizeEndPoint = function ( dom, collapsed, start, rng ) {
			...
			if ( container === body ) {
				...
				if ( container.hasChildNodes() ) {
					...
					container = container.childNodes[ offset ];
					offset =
						NodeType.isText( container ) && isAfterNode
			...
			return normalized
				? Option.some( CaretPosition( container, offset ) )
				: Option.none();
		};
		var normalize$2 = function ( dom, rng ) {
			...
			normalizeEndPoint( dom, collapsed, true, normRng ).each( function (
				pos
			) {
			...
			if ( ! collapsed ) {
				normalizeEndPoint( dom, collapsed, false, normRng ).each(
					function ( pos ) {
			...
			return RangeCompare.isEq( rng, normRng )
				? Option.none()
				: Option.some( normRng );
		};
		...
		var hasBrAfter = function ( rootNode, startNode ) {
			if ( isBeforeBr$1( CaretPosition$1.after( startNode ) ) ) {
				...
				return CaretFinder.nextPosition(
					rootNode,
					CaretPosition$1.after( startNode )
				)
					.map( function ( pos ) {
						return NodeType.isBr( pos.getNode() );
					} )
		...
		var readInlineAnchorLocation = function ( editor ) {
			var isInlineTarget = curry( InlineUtils.isInlineTarget, editor );
			var position = CaretPosition$1.fromRangeStart(
				editor.selection.getRng()
			);
			return BoundaryLocation.readLocation(
				isInlineTarget,
		...
		var clamp = function ( offset, element ) {
			var max = isText( element )
				? get$4( element ).length
		...
		var isOrContains = function ( root, elm ) {
			return (
				...
				( contains$3( root, elm ) || eq( root, elm ) )
			);
		};
		var isRngInRoot = function ( root ) {
			return function ( rng ) {
				return (
					isOrContains( root, rng.start() ) &&
					isOrContains( root, rng.finish() )
				);
			};
		};
		...
		var store = function ( editor ) {
			var newBookmark = shouldStore( editor )
				? getBookmark$2( Element.fromDom( editor.getBody() ) )
				: Option.none();
			editor.bookmark = newBookmark.isSome()
				? newBookmark
				: editor.bookmark;
		};
		var storeNative = function ( editor, rng ) {
			var root = Element.fromDom( editor.getBody() );
			var range = shouldStore( editor )
				? Option.from( rng )
				: Option.none();
		...
		var getRng = function ( editor ) {
			...
			return bookmark
				.bind( curry( validate, Element.fromDom( editor.getBody() ) ) )
		...
		var indentElement = function (
			...
			indentStyleName =
				element.nodeName === 'TABLE' ? 'margin' : indentStyleName;
			indentStyleName +=
		...
		function EditorCommands( editor ) {
			...
			var execCommand = function ( command, ui, value, args ) {
				...
				each$a( editor.plugins, function ( p ) {
					if (
						p.execCommand &&
						p.execCommand( command, ui, value )
					) {
						editor.fire( 'ExecCommand', {
							command: command,
							ui: ui,
			...
			var addCommand = function ( command, callback, scope ) {
				command = command.toLowerCase();
				commands.exec[ command ] = function (
					command,
					ui,
					value,
					args
			...
			addCommands(
				{
					...
					'InsertUnorderedList,InsertOrderedList': function (
						command
					) {
						var list = dom.getParent(
							selection.getNode(),
							'ul,ol'
						);
						return (
		...
		var Dispatcher = function ( settings ) {
			...
			var has = function ( name ) {
				name = name.toLowerCase();
				return ! (
					! bindings[ name ] || bindings[ name ].length === 0
				);
			};
			self.fire = fire;
			self.on = on;
		...
		var getEventTarget = function ( editor, eventName ) {
			...
			if ( editor.settings.event_root ) {
				if ( ! editor.eventRoot ) {
					editor.eventRoot = DOM$1.select(
						editor.settings.event_root
					)[ 0 ];
				}
		...
		var normalizePlugins = function ( plugins ) {
			var pluginNames = isArray( plugins )
				? plugins.join( ' ' )
				: plugins;
			var trimmedPlugins = map(
				isString( pluginNames ) ? pluginNames.split( ' ' ) : [],
				trim$2
			);
		...
		var getSection = function ( sectionResult, name, defaults ) {
			...
			var sectionSettings = sections.hasOwnProperty( name )
				? sections[ name ]
				: {};
		...
		var processPlugins = function (
			...
			var forcedPlugins = normalizePlugins(
				defaultOverrideSettings.forced_plugins
			);
			var plugins = normalizePlugins( settings.plugins );
			var platformPlugins =
				isTouchDevice && hasSection( sectionResult, 'mobile' )
					? filterMobilePlugins( plugins )
					: plugins;
			var combinedPlugins = combinePlugins(
				forcedPlugins,
				platformPlugins
			);
		...
		var isOnMobile = function ( isTouchDevice, sectionResult ) {
			...
			return (
				isTouchDevice &&
				hasSection( sectionResult, 'mobile' ) &&
				! isInline
			);
		};
		var combineSettings = function (
			...
			var extendedSettings = Tools.extend(
				...
				isOnMobile( isTouchDevice, sectionResult )
					? getSection(
							sectionResult,
							'mobile',
			...
			if ( typeof value === 'string' ) {
				each(
					...
					function ( val ) {
						var arr = val.split( '=' );
						if ( arr.length > 1 ) {
							output[ Tools.trim( arr[ 0 ] ) ] = Tools.trim(
								arr[ 1 ]
							);
						} else {
							output[ Tools.trim( arr[ 0 ] ) ] =
								Tools.trim( arr );
						}
					}
				);
		...
		var getParam = function ( editor, name, defaultVal, type ) {
			var value =
				name in editor.settings ? editor.settings[ name ] : defaultVal;
			if ( type === 'hash' ) {
				return getParamObject( value );
			} else if ( type === 'string' ) {
				return getFiltered( isString, editor, name ).getOr(
					defaultVal
				);
			} else if ( type === 'number' ) {
				return getFiltered( isNumber, editor, name ).getOr(
					defaultVal
				);
			} else if ( type === 'boolean' ) {
				return getFiltered( isBoolean, editor, name ).getOr(
					defaultVal
				);
			} else if ( type === 'object' ) {
				return getFiltered( isObject, editor, name ).getOr(
					defaultVal
				);
			} else if ( type === 'array' ) {
				return getFiltered( isArray, editor, name ).getOr( defaultVal );
			} else if ( type === 'string[]' ) {
				return getFiltered( isArrayOf( isString ), editor, name ).getOr(
					defaultVal
				);
			} else if ( type === 'function' ) {
				return getFiltered( isFunction, editor, name ).getOr(
					defaultVal
				);
			} else {
				return value;
		...
		function Shortcuts( editor ) {
			...
			var isFunctionKey = function ( e ) {
				return (
					e.type === 'keydown' && e.keyCode >= 112 && e.keyCode <= 123
				);
			};
			...
			self.add = function ( pattern, desc, cmdFunc, scope ) {
				...
				each$b(
					explode$2( Tools.trim( pattern.toLowerCase() ) ),
					function ( pattern ) {
		...
		var getCollapsedNode = function ( rng ) {
			return rng.collapsed
				? Option.from(
						getNode( rng.startContainer, rng.startOffset )
				  ).map( Element.fromDom )
				: Option.none();
		};
		...
		var normalizeSelection = function ( editor, rng ) {
			getFocusInElement( Element.fromDom( editor.getBody() ), rng )
				.bind( function ( elm ) {
		...
		var hasFocus$1 = function ( editor ) {
			return editor.inline
				? hasInlineFocus( editor )
				: hasIframeFocus( editor );
		};
		var focusEditor = function ( editor ) {
			...
			if (
				editor.bookmark !== undefined &&
				hasFocus$1( editor ) === false
			) {
				SelectionBookmark.getRng( editor ).each( function (
					bookmarkRng
				) {
		...
		var isInsideElementContentArea = function (
			bodyElm,
			clientX,
			clientY
		) {
		...
		var isXYInContentArea = function ( editor, clientX, clientY ) {
			...
			var targetElm = editor.inline
				? bodyElm
				: documentElement( bodyElm );
			var transposedPoint = transpose(
				editor.inline,
				targetElm,
				clientX,
				clientY
		...
		function NotificationManager( editor ) {
			...
			var open = function ( args ) {
				if (
					editor.removed ||
					! EditorView.isEditorAttachedToDom( editor )
				) {
					return;
				}
				return find( notifications, function ( notification ) {
					return isEqual(
						getImplementation().getArgs( notification ),
						args
					);
				} ).getOrThunk( function () {
					editor.editorManager.setActive( editor );
					var notification = getImplementation().open(
						args,
						function () {
		...
		function WindowManager( editor ) {
			...
			var alert = function ( message, callback, scope ) {
				var win = getImplementation().alert(
					message,
			...
			var confirm = function ( message, callback, scope ) {
				var win = getImplementation().confirm(
					message,
					funcBind( scope ? scope : this, callback ),
					closeWindow
				);
			...
			var getParams = function () {
				return getTopWindow()
					.map( getImplementation().getParams )
					.getOr( null );
			};
			...
			translate: function ( text ) {
				...
				var isEmpty = function ( text ) {
					return (
						text === '' ||
						text === null ||
						Tools.is( text, 'undefined' )
					);
				};
				var getLangData = function ( text ) {
					text = toString( text );
					return Tools.hasOwn( langData, text )
						? toString( langData[ text ] )
						: text;
				};
				if ( isEmpty( text ) ) {
					return '';
				...
				if (
					Tools.is( text, 'object' ) &&
					Tools.hasOwn( text, 'raw' )
				) {
					return toString( text.raw );
				}
				if ( Tools.is( text, 'array' ) ) {
					var values_1 = text.slice( 1 );
					text = getLangData( text[ 0 ] ).replace(
						/\{([0-9]+)\}/g,
						function ( $1, $2 ) {
							return Tools.hasOwn( values_1, $2 )
								? toString( values_1[ $2 ] )
								: $1;
						}
					);
		...
		var resolvePluginName = function ( targetUrl, suffix ) {
			for ( var name in PluginManager.urls ) {
				var matchUrl =
					PluginManager.urls[ name ] + '/plugin' + suffix + '.js';
				...
				return new promiseObj( function ( resolve ) {
					...
					try {
						...
						var success = function ( url ) {
							closeNotification_1();
							uploadStatus.markUploaded(
								blobInfo.blobUri(),
								url
							);
							resolvePending(
								blobInfo.blobUri(),
								handlerSuccess( blobInfo, url )
							);
		...
		var dataUriToBlob = function ( uri ) {
			return new promiseObj( function ( resolve ) {
				...
				resolve(
					new domGlobals.Blob( [ arr ], { type: uriParts.type } )
				);
			} );
		};
		...
		var imageToBlobInfo = function ( blobCache, img, resolve, reject ) {
			...
			if ( img.src.indexOf( 'blob:' ) === 0 ) {
				...
				if ( blobInfo ) {
					...
					Conversions.uriToBlob( img.src ).then(
						function ( blob ) {
							Conversions.blobToDataUri( blob ).then( function (
								dataUri
							) {
								base64 =
									Conversions.parseDataUri( dataUri ).data;
			...
			var findAll = function ( elm, predicate ) {
				...
				images = filter( getAllImages( elm ), function ( img ) {
					...
					if ( src.indexOf( 'blob:' ) === 0 ) {
						return (
							! uploadStatus.isUploaded( src ) && predicate( img )
						);
					}
				...
				var promises = map( images, function ( img ) {
					if ( cachedPromises[ img.src ] ) {
						return new promiseObj( function ( resolve ) {
							cachedPromises[ img.src ].then( function (
								imageInfo
							) {
					...
					var newPromise = new promiseObj( function (
						resolve,
						reject
					) {
		...
		function BlobCache() {
			...
			var toBlobInfo = function ( o ) {
				...
				return {
					...
					blobUri: constant(
						o.blobUri || URL.createObjectURL( o.blob )
					),
					uri: constant( o.uri ),
				};
			};
		...
		function UploadStatus() {
			...
			var markUploaded = function ( blobUri, resultUri ) {
				blobUriStatuses[ blobUri ] = createStatus(
					UPLOADED,
					resultUri
				);
			};
		...
		function EditorUpload( editor ) {
			...
			var uploadImages = function ( callback ) {
				if ( ! uploader ) {
					uploader = Uploader( uploadStatus, {
						...
						credentials:
							Settings.getImagesUploadCredentials( editor ),
				...
				return scanForImages().then(
					aliveGuard( function ( imageInfos ) {
						...
						return uploader
							.upload( blobInfos, openNotification )
							.then(
								aliveGuard( function ( result ) {
									var filteredResult = map(
										result,
										function ( uploadInfo, index ) {
											var image =
												imageInfos[ index ].image;
											if (
												uploadInfo.status &&
												Settings.shouldReplaceBlobUris(
													editor
												)
											) {
												replaceImageUri(
			...
			var isValidDataUriImage = function ( imgElm ) {
				...
				if ( imgElm.getAttribute( 'src' ).indexOf( 'data:' ) === 0 ) {
					var dataImgFilter =
						Settings.getImagesDataImgFilter( editor );
			...
			var scanForImages = function () {
				...
				return imageScanner
					.findAll( editor.getBody(), isValidDataUriImage )
					.then(
						aliveGuard( function ( result ) {
							...
							each( result, function ( resultItem ) {
								...
								resultItem.image.src =
									resultItem.blobInfo.blobUri();
								resultItem.image.removeAttribute(
									'data-mce-src'
								);
							} );
							return result;
							...
							blobInfo = foldl(
								...
								function ( result, editor ) {
									return (
										result ||
										( editor.editorUpload &&
											editor.editorUpload.blobCache.getByUri(
												blobUri
											) )
									);
								},
		...
		var getStartNode = function ( rng ) {
			...
			if ( NodeType.isText( sc ) ) {
				return so === 0
					? Option.some( Element.fromDom( sc ) )
					: Option.none();
			} else {
				return Option.from( sc.childNodes[ so ] ).map(
					Element.fromDom
				);
			}
		};
		var getEndNode = function ( rng ) {
			...
			if ( NodeType.isText( ec ) ) {
				...
				return Option.from( ec.childNodes[ eo - 1 ] ).map(
					Element.fromDom
				);
			}
		};
		var getFirstChildren = function ( node ) {
			return firstChild( node ).fold(
				constant( [ node ] ),
				function ( child ) {
		...
		var moveEndPoint$1 = function ( dom, rng, node, start ) {
			...
			do {
				if (
					node.nodeType === 3 &&
					Tools.trim( node.nodeValue ).length !== 0
				) {
					if ( start ) {
						rng.setStart( node, 0 );
					} else {
		...
		function NodeChange( editor ) {
			...
			if ( ! ( 'onselectionchange' in editor.getDoc() ) ) {
				editor.on(
					'NodeChange Click MouseUp KeyUp Focus',
					function ( e ) {
						...
						if (
							e.type === 'nodechange' ||
							! RangeCompare.isEq( fakeRng, lastRng )
						) {
			...
			editor.on( 'SelectionChange', function () {
				...
				if (
					! startElm ||
					( ! Env.range && editor.selection.isCollapsed() )
				) {
					return;
				}
				if (
		...
		var walkUntil = function (
			...
			var add = function ( node ) {
				...
				for ( i = 0; i < clientRects.length; i++ ) {
					...
					if (
						result.length > 0 &&
						isAboveFn( clientRect, ArrUtils.last( result ) )
					) {
						line++;
					}
		...
		var positionsUntil = function ( direction, root, predicateFn, node ) {
			...
			do {
				...
				if (
					result.length > 0 &&
					isBelowFn( clientRect, ArrUtils.last( result ) )
				) {
					line++;
				}
				clientRect = clone$1( clientRect );
		...
		var findClosestClientRect = function ( clientRects, clientX ) {
			return ArrUtils.reduce(
				clientRects,
				function ( oldClientRect, clientRect ) {
					...
					if (
						...
						isContentEditableFalse$7( clientRect.node )
					) {
						return clientRect;
					}
		...
		var findLineNodeRects = function ( root, targetNodeRect ) {
			...
			walkUntil$1(
				VDirection.Up,
				root,
				curry( collect, isAbove ),
				targetNodeRect.node
			);
			walkUntil$1(
				VDirection.Down,
				root,
				curry( collect, isBelow ),
				targetNodeRect.node
			);
			return clientRects;
		};
		var getFakeCaretTargets = function ( root ) {
			return filter(
				from$1( root.getElementsByTagName( '*' ) ),
				isFakeCaretTarget
			);
		};
		...
		var closestCaret = function ( root, clientX, clientY ) {
			var closestNodeRect;
			var contentEditableFalseNodeRects = getClientRects(
				getFakeCaretTargets( root )
			);
			var targetNodeRects = filter(
				contentEditableFalseNodeRects,
				function ( rect ) {
			...
			if ( closestNodeRect ) {
				closestNodeRect = findClosestClientRect(
					findLineNodeRects( root, closestNodeRect ),
					clientX
				);
				if (
					closestNodeRect &&
					isFakeCaretTarget( closestNodeRect.node )
				) {
		...
		var getMousePosition = function ( editor, event ) {
			if ( event.target.ownerDocument !== editor.getDoc() ) {
				var iframePosition = getAbsolutePosition(
					editor.getContentAreaContainer()
				);
				var scrollPosition = getBodyScroll( editor );
				return {
					left:
						event.pageX - iframePosition.left + scrollPosition.left,
		...
		var calculatePosition = function (
			...
			return {
				pageX:
					...
					scrollPosition.left,
				pageY:
					mousePosition.top - bodyPosition.top + scrollPosition.top,
			};
		};
		...
		var isValidDropTarget = function (
			editor,
			targetElement,
			dragElement
		) {
				...
				if ( isLeftMouseButtonPressed( e ) ) {
					...
					if ( isDraggable( editor.getBody(), ceElm ) ) {
						...
						state.maxX =
							( editor.inline
								? bodyElm.scrollWidth
								: docElm.offsetWidth ) - 2;
						state.maxY =
							( editor.inline
								? bodyElm.scrollHeight
								: docElm.offsetHeight ) - 2;
		...
		var move$1 = function ( state, editor ) {
			var throttledPlaceCaretAt = Delay.throttle( function (
				clientX,
				clientY
			) {
				editor._selectionOverrides.hideFakeCaret();
			...
			return function ( e ) {
				...
				if ( state.dragging ) {
					var targetPos = applyRelPos(
						state,
						MousePosition.calc( editor, e )
					);
						...
						if ( ! args.isDefaultPrevented() ) {
							...
							editor.undoManager.transact( function () {
								...
								editor.insertContent(
									editor.dom.getOuterHTML( targetClone_1 )
								);
								editor._selectionOverrides.hideFakeCaret();
							} );
						}
			...
			editor.on( 'drop', function ( e ) {
				var realTarget =
					typeof e.clientX !== 'undefined'
						...
						: null;
				if (
					isContentEditableFalse$8( realTarget ) ||
					isContentEditableFalse$8(
						editor.dom.getContentEditableParent( realTarget )
					)
				) {
					e.preventDefault();
				}
		...
		var setup$4 = function ( editor ) {
			var renderFocusCaret = first( function () {
				if ( ! editor.removed ) {
					var rng = editor.selection.getRng();
					if ( rng.collapsed ) {
						var caretRange = renderRangeCaret(
							editor,
							editor.selection.getRng(),
							false
						);
		...
		var getContentEditableRoot$1 = function ( editor, node ) {
			...
			while ( node && node !== root ) {
				if (
					isContentEditableTrue$4( node ) ||
					isContentEditableFalse$9( node )
				) {
					return node;
				}
				node = node.parentNode;
		...
		var SelectionOverrides = function ( editor ) {
			...
			var getRealSelectionElement = function () {
				...
				return container
					? container.getElementsByTagName( '*' )[ 0 ]
					: container;
			};
			...
			var registerEvents = function () {
				...
				editor.on( 'click', function ( e ) {
					...
					contentEditableRoot = getContentEditableRoot$1(
						editor,
						e.target
					);
				...
				var handleTouchSelect = function ( editor ) {
					...
					editor.on( 'touchend', function ( e ) {
						var contentEditableRoot = getContentEditableRoot$1(
							editor,
							e.target
						);
						if ( isContentEditableFalse$9( contentEditableRoot ) ) {
							if ( ! moved ) {
								...
								setContentEditableSelection(
									selectNode( editor, contentEditableRoot )
								);
							}
						}
					} );
				...
				var isInSameBlock = function ( node1, node2 ) {
					var block1 = editor.dom.getParent(
						node1,
						editor.dom.isBlock
					);
					var block2 = editor.dom.getParent(
						node2,
						editor.dom.isBlock
					);
				...
				var hasBetterMouseTarget = function ( targetNode, caretNode ) {
					var targetBlock = editor.dom.getParent(
						targetNode,
						editor.dom.isBlock
					);
					var caretBlock = editor.dom.getParent(
						caretNode,
						editor.dom.isBlock
					);
					if (
						...
						isContentEditableFalse$9(
							getContentEditableRoot$1( editor, targetBlock )
						) === false
					) {
						return true;
					}
				...
				editor.on( 'AfterSetSelectionRange', function ( e ) {
					...
					if (
						...
						! isPasteBin( rng.startContainer.parentNode )
					) {
						hideFakeCaret();
					}
					if (
						! isFakeSelectionElement(
							rng.startContainer.parentNode
						)
					) {
						removeContentEditableSelection();
					}
					...
					if (
						...
						if ( realSelectionElement ) {
							...
							clipboardData.setData(
								'text/html',
								realSelectionElement.outerHTML
							);
							clipboardData.setData(
								'text/plain',
			...
			var isWithinCaretContainer = function ( node ) {
				return (
					isCaretContainer( node ) ||
					startsWithCaretContainer( node ) ||
					endsWithCaretContainer( node )
				);
			};
			...
			var setContentEditableSelection = function ( range, forward ) {
				...
				if ( range.collapsed ) {
					if ( ! isRangeInCaretContainer( range ) ) {
						if ( forward === false ) {
							caretPosition = getNormalizedRangeEndPoint(
								-1,
								range
							);
							if (
								isFakeCaretTarget(
									caretPosition.getNode( true )
								)
							) {
								return showCaret(
									-1,
							...
							if (
								isFakeCaretTarget( caretPosition.getNode() )
							) {
								return showCaret(
									-1,
									caretPosition.getNode(),
				...
				if (
					startContainer.nodeType === 3 &&
					startOffset === 0 &&
					isContentEditableFalse$9( startContainer.parentNode )
				) {
					startContainer = startContainer.parentNode;
					startOffset = dom.nodeIndex( startContainer );
					startContainer = startContainer.parentNode;
				...
				$realSelectionContainer = descendant(
					Element.fromDom( editor.getBody() ),
					'#' + realSelectionId
				).fold(
					function () {
				...
				if ( targetClone === origTargetClone && Env.ie ) {
					$realSelectionContainer
						...
						.append( targetClone );
					range.setStartAfter(
						$realSelectionContainer[ 0 ].firstChild.firstChild
					);
					...
					$realSelectionContainer
						...
						.append( '\xA0' );
					range.setStart(
						$realSelectionContainer[ 0 ].firstChild,
						1
					);
			...
			var removeContentEditableSelection = function () {
				if ( selectedContentEditableNode ) {
					selectedContentEditableNode.removeAttribute(
						'data-mce-selected'
					);
					descendant(
						Element.fromDom( editor.getBody() ),
		...
		var isInvalidUri = function ( settings, uri ) {
			if ( settings.allow_html_data_urls ) {
				...
				return (
					settings.allow_svg_data_urls === false &&
		...
		var checkBogusAttribute = function ( regExp, attrString ) {
			...
			if ( matches ) {
				...
				return typeof name === 'string' &&
					name.toLowerCase() === 'data-mce-bogus'
					? value
					: null;
			} else {
				return null;
		...
		function SaxParser( settings, schema ) {
			...
			var parse = function ( html ) {
				...
				shortEndedElements = schema.getShortEndedElements();
				selfClosing =
					settings.self_closing_elements ||
				...
				while ( ( matches = tokenRegExp.exec( processHtml ) ) ) {
					if ( index < matches.index ) {
						text(
							decode(
								html.substr( index, matches.index - index )
							)
						);
					}
					if ( ( value = matches[ 6 ] ) ) {
						...
						var bogusValue = checkBogusAttribute(
							attrRegExp,
							matches[ 8 ]
						);
						if ( bogusValue !== null ) {
							if ( bogusValue === 'all' ) {
								index = findEndTagIndex(
									schema,
									html,
						...
						if (
							! validate ||
							( elementRule = schema.getElementRule( value ) )
						) {
							isValidElement = true;
							if ( validate ) {
							...
							if ( ( attribsValue = matches[ 8 ] ) ) {
								isInternalElement =
									attribsValue.indexOf( 'data-mce-type' ) !==
									-1;
								if (
									isInternalElement &&
									removeInternalElements
							...
							if ( validate && ! isInternalElement ) {
								...
								anyAttributesRequired =
									elementRule.removeEmptyAttrs;
								if (
									anyAttributesRequired &&
									! attrList.length
								) {
								...
								if (
									( attr = attrList.map[ 'data-mce-bogus' ] )
								) {
									if ( attr === 'all' ) {
										index = findEndTagIndex(
											schema,
						...
						if ( ( endRegExp = specialElements[ value ] ) ) {
							endRegExp.lastIndex = index =
								matches.index + matches[ 0 ].length;
							if ( ( matches = endRegExp.exec( html ) ) ) {
								if ( isValidElement ) {
									textData = html.substr(
										index,
										matches.index - index
									);
								}
						...
						if ( ! isShortEnded ) {
							if (
								! attribsValue ||
								attribsValue.indexOf( '/' ) !==
									attribsValue.length - 1
							) {
								stack.push( {
									name: value,
		...
		var trimInternal = function ( serializer, html ) {
			...
			while ( ( matches = bogusAllRegExp.exec( content ) ) ) {
				...
				if ( shortEndedElements[ matches[ 1 ] ] ) {
					...
					endTagIndex = SaxParser$1.findEndTag(
						schema,
						content,
						index
					);
		...
		var read$3 = function ( elm ) {
			return filter(
				map( from$1( elm.childNodes ), getOuterHtml ),
				function ( item ) {
					return item.length > 0;
				}
			);
		...
		var write = function ( fragments, elm ) {
			var currentFragments = map(
				from$1( elm.childNodes ),
				getOuterHtml
			);
		...
		var lazyTempDocument = function () {
			return undoLevelDocument.get().getOrThunk( function () {
				var doc =
					domGlobals.document.implementation.createHTMLDocument(
						'undo'
					);
		...
		var hasEqualCleanedContent = function ( level1, level2 ) {
			return (
				getCleanLevelContent( level1 ) ===
				getCleanLevelContent( level2 )
			);
		};
		...
		function UndoManager( editor ) {
			...
			editor.on(
				'SaveContent ObjectResized blur',
			...
			editor.on( 'KeyUp', function ( e ) {
				...
				if (
					...
					self.typing &&
					Levels.isEq(
						Levels.createFromEditor( editor ),
						data[ 0 ]
					) === false
				) {
			...
			self = {
				...
				beforeChange: function () {
					if ( isUnlocked() ) {
						beforeBookmark = GetBookmark.getUndoBookmark(
							editor.selection
						);
					}
				},
				add: function ( level, event ) {
					...
					level.bookmark = GetBookmark.getUndoBookmark(
						editor.selection
					);
		...
		var postProcessHooks = {},
			filter$2 = ArrUtils.filter,
			each$c = ArrUtils.each;
		...
		addPostProcessHook( 'pre', function ( editor ) {
			...
			if ( ! rng.collapsed ) {
				...
				each$c(
					filter$2( filter$2( blocks, isPre ), hasPreSibling ),
					function ( pre ) {
		...
		var getContainer = function ( ed, rng, start ) {
			...
			if ( NodeType.isElement( container ) ) {
				...
				container =
					container.childNodes[ offset > lastIdx ? lastIdx : offset ];
			}
			if (
				NodeType.isText( container ) &&
				start &&
				...
				container =
					new TreeWalker( container, ed.getBody() ).next() ||
					container;
			}
			if ( NodeType.isText( container ) && ! start && offset === 0 ) {
				container =
					new TreeWalker( container, ed.getBody() ).prev() ||
					container;
			}
			return container;
		};
			...
			if ( format.block ) {
				if ( ! forcedRootBlock ) {
					if ( dom.isBlock( node ) && ! dom.isBlock( parentNode ) ) {
						if (
							! find$3( dom, node, false ) &&
							! find$3( dom, node.firstChild, true, 1 )
						) {
							node.insertBefore(
								dom.create( 'br' ),
								node.firstChild
						...
						if (
							! find$3( dom, node, true ) &&
							! find$3( dom, node.lastChild, false, 1 )
						) {
					...
					if ( parentNode === dom.getRoot() ) {
						if (
							! format.list_block ||
							! isEq$5( node, format.list_block )
						) {
							each$d(
								Tools.grep( node.childNodes ),
								function ( node ) {
									if (
										FormatUtils.isValid(
											ed,
			...
			if ( format.remove !== 'all' ) {
				each$d( format.styles, function ( value, name ) {
					...
					if (
						...
						! compareNode ||
						isEq$5(
							FormatUtils.getStyle( dom, compareNode, name ),
							value
						)
					) {
				...
				each$d( format.attributes, function ( value, name ) {
					...
					if (
						! compareNode ||
						isEq$5( dom.getAttrib( compareNode, name ), value )
					) {
						if ( name === 'class' ) {
		...
		var wrapAndSplit = function (
			...
			if ( formatRoot ) {
				...
				if (
					split &&
					( ! format.mixed || ! dom.isBlock( formatRoot ) )
				) {
		...
		var remove$6 = function ( ed, name, vars, node, similar ) {
			...
			var removeRngStyle = function ( rng ) {
					...
					if ( startContainer !== endContainer ) {
						if (
							...
							if ( startContainer.nodeName === 'TR' ) {
								startContainer =
									startContainer.firstChild.firstChild ||
									startContainer;
							} else {
								startContainer =
						...
						if (
							...
							/^T(HEAD|BODY|FOOT|R)$/.test(
								commonAncestorContainer.nodeName
							) &&
							isTableCell$4( endContainer ) &&
						...
						if (
							...
							var marker = Option.from(
								startContainer.firstChild
							).getOr( startContainer );
							splitToFormatRoot(
				...
				RangeWalk.walk( dom, rng, function ( nodes ) {
					each$d( nodes, function ( node ) {
						...
						if (
							NodeType.isElement( node ) &&
							ed.dom.getStyle( node, 'text-decoration' ) ===
								'underline' &&
							node.parentNode &&
							FormatUtils.getTextDecoration(
								dom,
								node.parentNode
							) === 'underline'
						) {
			...
			if (
				! selection.isCollapsed() ||
				! format.inline ||
				dom.select( 'td[data-mce-selected],th[data-mce-selected]' )
					.length
			) {
				bookmark = GetBookmark.getPersistentBookmark(
					ed.selection,
					true
				);
		...
		var findElementSibling = function ( node, siblingName ) {
			...
			for ( sibling = node; sibling; sibling = sibling[ siblingName ] ) {
				if (
					sibling.nodeType === 3 &&
					sibling.nodeValue.length !== 0
				) {
					return node;
				}
				if (
					sibling.nodeType === 1 &&
					! Bookmarks.isBookmarkNode( sibling )
				) {
					return sibling;
				}
			}
		...
		var processUnderlineAndColor = function ( dom, node ) {
			...
			if (
				...
				textDecoration = FormatUtils.getTextDecoration(
					dom,
					node.parentNode
				);
				if ( dom.getStyle( node, 'color' ) && textDecoration ) {
					...
					dom.getStyle( node, 'text-decoration' ) === textDecoration
				) {
		...
		var mergeUnderlineAndColor = function ( dom, format, vars, node ) {
			if ( format.styles.color || format.styles.textDecoration ) {
				Tools.walk(
					node,
					curry( processUnderlineAndColor, dom ),
					'childNodes'
				);
				processUnderlineAndColor( dom, node );
			}
		};
		var mergeBackgroundColorAndFontSize = function (
			dom,
			format,
			vars,
			node
		...
		var mergeWithChildren = function ( editor, formatList, vars, node ) {
			each$e( formatList, function ( format ) {
				each$e(
					editor.dom.select( format.inline, node ),
					function ( child ) {
		...
		var mergeWithParents = function ( editor, format, name, vars, node ) {
			if (
				MatchFormat.matchNode( editor, node.parentNode, name, vars )
			) {
			...
			var applyRngStyle = function ( dom, rng, bookmark, nodeSpecific ) {
				...
				RangeWalk.walk( dom, rng, function ( nodes ) {
					...
					var process = function ( node ) {
						...
						if (
							format.wrapper &&
							MatchFormat.matchNode( ed, node, name, vars )
						) {
							currentWrapElm = 0;
							return;
						}
				...
				each$f( newWrappers, function ( node ) {
					...
					var getChildCount = function ( node ) {
						...
						each$f( node.childNodes, function ( node ) {
							if (
								! FormatUtils.isWhiteSpaceNode( node ) &&
								! Bookmarks.isBookmarkNode( node )
							) {
								count++;
							}
					...
					if ( format.inline || format.wrapper ) {
						...
						MergeFormats.mergeWithChildren(
							ed,
							formatList,
							vars,
							node
						);
						MergeFormats.mergeWithParents(
							ed,
							format,
							name,
							vars,
						...
						MergeFormats.mergeBackgroundColorAndFontSize(
							dom,
							format,
							vars,
							node
			...
			if ( format ) {
				if ( node ) {
					...
					if (
						...
						ed.selection.setRng(
							RangeNormalizer.normalize( ed.selection.getRng() )
						);
						bookmark = GetBookmark.getPersistentBookmark(
							ed.selection,
							true
						...
						if ( format.styles ) {
							MergeFormats.mergeUnderlineAndColor(
								dom,
								format,
								vars,
								curSelNode
			...
			editor.on( 'NodeChange', function ( e ) {
				...
				each$g( formatChangeData.get(), function ( callbacks, format ) {
					each$g( parents, function ( node ) {
						...
						if (
							MatchFormat.matchesUnInheritedFormatSelector(
								editor,
								node,
								format
							)
		...
		var parsedSelectorToHtml = function ( ancestry, editor ) {
			...
			var getRequiredParent = function ( elm, candidate ) {
				var name =
					typeof elm !== 'string' ? elm.nodeName.toLowerCase() : elm;
				var elmRule = schema.getElementRule( name );
				var parentsRequired = elmRule && elmRule.parentsRequired;
				if ( parentsRequired && parentsRequired.length ) {
					return candidate &&
						Tools.inArray( parentsRequired, candidate ) !== -1
						? candidate
						: parentsRequired[ 0 ];
				} else {
					return false;
			...
			if ( ancestry && ancestry.length ) {
				...
				fragment.appendChild(
					wrapInHtml( elm, ancestry.slice( 1 ), item.siblings )
				);
				return fragment;
			} else {
				return '';
			...
			htmlParser.addAttributeFilter(
				...
				function ( nodes, name ) {
					...
					while ( i-- ) {
						...
						node.attr(
							'tabindex',
							node.attributes.map[ 'data-mce-tabindex' ]
						);
						node.attr( name, null );
					}
				}
				...
				function ( nodes, name ) {
					...
					while ( i-- ) {
						...
						if ( value !== undefined ) {
							...
							if ( name === 'style' ) {
								value = dom.serializeStyle(
									dom.parseStyle( value ),
									node.name
								);
			...
			htmlParser.addAttributeFilter(
				...
				function ( nodes, name, args ) {
					...
					while ( i-- ) {
						node = nodes[ i ];
						if (
							node.attributes.map[ 'data-mce-type' ] ===
								'bookmark' &&
							! args.cleanup
						) {
							var hasChildren = Option.from(
								node.firstChild
							).exists( function ( firstChild ) {
			...
			htmlParser.addAttributeFilter( 'data-mce-type', function ( nodes ) {
				each( nodes, function ( node ) {
					if ( node.attr( 'data-mce-type' ) === 'format-caret' ) {
						if (
							node.isEmpty(
								htmlParser.schema.getNonEmptyElements()
							)
						) {
							node.remove();
						} else {
					...
					do {
						if ( node.type === 1 ) {
							...
							while ( i-- ) {
								...
								if (
									name === 'name' ||
									name.indexOf( 'data-mce-bookmark' ) === 0
								) {
									return false;
								}
							}
						...
						if (
							node.type === 3 &&
							! whiteSpaceRegExp$3.test( node.value )
						) {
							return false;
						}
						if (
		...
		var register$2 = function ( parser, settings ) {
			...
			if ( settings.remove_trailing_brs ) {
				parser.addNodeFilter( 'br', function ( nodes, _, args ) {
					...
					var l = nodes.length;
					var node;
					var blockElements = Tools.extend(
						{},
					...
					for ( i = 0; i < l; i++ ) {
						...
						parent = node.parent;
						if (
							blockElements[ node.parent.name ] &&
							...
							if ( node ) {
								...
								if (
									...
									elementRule = schema.getElementRule(
										parent.name
									);
									if ( elementRule ) {
							...
							if (
								lastParent === parent &&
								settings.padd_empty_with_br !== true
							) {
		...
		function DomParser( settings, schema ) {
			...
			var fixInvalidChildren = function ( nodes ) {
				...
				var nonEmptyElements,
					...
					sibling,
					nextNode;
				nonSplitableElements = makeMap$4(
				...
				for ( ni = 0; ni < nodes.length; ni++ ) {
					...
					if ( parent && parents.length > 1 ) {
						parents.reverse();
						newParent = currentNode = filterNode(
							parents[ 0 ].clone()
						);
			...
			var parse = function ( html, args ) {
				...
				matchedNodes = {};
				matchedAttributes = {};
				blockElements = extend$3(
					makeMap$4( 'script,style,head,html,body,title,meta,param' ),
					schema.getBlockElements()
				);
				var nonEmptyElements = schema.getNonEmptyElements();
				...
				var removeWhitespaceBefore = function ( node ) {
					...
					for (
						...
						textVal = textNode.value.replace(
							endWhiteSpaceRegExp,
							''
						);
						if ( textVal.length > 0 ) {
				...
				parser = SaxParser$1(
					{
						...
						cdata: function ( text ) {
							node.append( createNode( '#cdata', 4 ) ).value =
								text;
						},
						text: function ( text, raw ) {
							var textNode;
						...
						comment: function ( text ) {
							node.append( createNode( '#comment', 8 ) ).value =
								text;
						},
						pi: function ( name, text ) {
				...
				if (
					rootBlockName &&
					( rootNode.name === 'body' || args.isRootContent )
				) {
					addRootBlocks();
				}
				if ( ! args.invalid ) {
		...
		var postProcess$1 = function ( editor, args, content ) {
			if ( ! args.no_events && editor ) {
				var outArgs = Events.firePostProcess(
					editor,
					merge( args, { content: content } )
				);
		...
		var getHtmlFromNode = function ( dom, node, args ) {
			...
			return args.selection ||
				isWsPreserveElement( Element.fromDom( node ) )
				? html
				: Tools.trim( html );
		};
		var parseHtml = function ( htmlParser, html, args ) {
			var parserArgs = args.selection
				? merge( { forced_root_block: false }, args )
				: args;
		...
		function DomSerializer( settings, editor ) {
			...
			schema =
				editor && editor.schema ? editor.schema : Schema( settings );
			settings.entity_encoding = settings.entity_encoding || 'named';
			settings.remove_trailing_brs =
				'remove_trailing_brs' in settings
					? settings.remove_trailing_brs
					: true;
			...
			var serialize = function ( node, parserArgs ) {
				...
				var targetNode = DomSerializerPreProcess.process(
					editor,
					node,
					args
				);
		...
		var ControlSelection = function ( selection, editor ) {
			...
			var isImage = function ( elm ) {
				return (
					elm &&
					( elm.nodeName === 'IMG' ||
						editor.dom.is( elm, 'figure.image' ) )
				);
			};
			...
			var getResizeTarget = function ( elm ) {
				return editor.dom.is( elm, 'figure.image' )
			...
			var resizeGhostElement = function ( e ) {
				...
				if (
					selectedHandle[ 2 ] < 0 &&
					selectedElmGhost.clientWidth <= width
				) {
					dom.setStyle(
						selectedElmGhost,
						'left',
						selectedElmX + ( startW - width )
					);
				}
				if (
					selectedHandle[ 3 ] < 0 &&
					selectedElmGhost.clientHeight <= height
				) {
					dom.setStyle(
						selectedElmGhost,
						'top',
						selectedElmY + ( startH - height )
					);
				}
				...
				var setSizeProp = function ( name, value ) {
					if ( value ) {
						if (
							...
							! editor.schema.isValid(
								selectedElm.nodeName.toLowerCase(),
								name
							)
						) {
							dom.setStyle(
								getResizeTarget( selectedElm ),
								name,
								value
							);
						} else {
				...
				if ( isResizable( targetElm ) && ! e.isDefaultPrevented() ) {
					each( resizeHandles, function ( handle, name ) {
						...
						var startDrag = function ( e ) {
							...
							startH =
								getResizeTarget( selectedElm ).clientHeight;
							ratio = startH / startW;
			...
			var updateResizeRect = function ( e ) {
				...
				controlElm =
					e.type === 'mousedown' ? e.target : selection.getNode();
				controlElm = dom
					.$( controlElm )
					.closest( 'table,img,figure.image,hr' )[ 0 ];
			...
			var isWithinContentEditableFalse = function ( elm ) {
				return isContentEditableFalse$a(
					getContentEditableRoot$2( editor.getBody(), elm )
				);
			};
		...
		var moveOutOfContentEditableFalse = function ( rng, rootNode ) {
			var parentElement =
				rng && rng.parentElement ? rng.parentElement() : null;
		...
		var subsection = function ( table, startElement, endElement ) {
			return findElementPos( table, startElement ).bind( function (
				startPos
			) {
				return findElementPos( table, endElement ).map( function (
					endPos
				) {
		...
		var getFullySelectedListWrappers = function ( parents, rng ) {
			return find( parents, function ( elm ) {
				return (
					name( elm ) === 'li' && hasAllContentsSelected( elm, rng )
				);
		...
		var directListWrappers = function ( commonAnchorContainer ) {
			if ( isListItem( commonAnchorContainer ) ) {
				...
				return isList( commonAnchorContainer )
					? [ commonAnchorContainer ]
					: [];
			}
		};
		var getWrapElements = function ( rootNode, rng ) {
			var commonAnchorContainer = Element.fromDom(
				rng.commonAncestorContainer
			);
			var parents = Parents.parentsAndSelf(
				commonAnchorContainer,
				rootNode
			);
		...
		var getTableFragment = function ( rootNode, selectedTableCells ) {
			return getParentTable( rootNode, selectedTableCells[ 0 ] )
				.bind( function ( tableElm ) {
					...
					var lastCell =
						selectedTableCells[ selectedTableCells.length - 1 ];
		...
		var read$4 = function ( rootNode, ranges ) {
			var selectedCells = TableCellSelection.getCellsFromElementOrRanges(
				ranges,
				rootNode
			);
			return selectedCells.length > 0
		...
		var getContent = function ( editor, args ) {
			...
			if ( args.format === 'text' ) {
				...
				if ( args.format === 'tree' ) {
					...
					args.content = editor.selection.isCollapsed()
						? ''
						: content;
					editor.fire( 'GetContent', args );
					return args.content;
		...
		var setContent = function ( editor, content, args ) {
			...
			if ( rng.insertNode ) {
				...
				if ( rng.startContainer === doc && rng.endContainer === doc ) {
					...
					if ( doc.body.childNodes.length === 0 ) {
						...
						if ( rng.createContextualFragment ) {
							rng.insertNode(
								rng.createContextualFragment( content )
							);
						} else {
				...
				if ( /^\s+/.test( content ) ) {
					anyRng.pasteHTML(
						'<span id="__mce_tmp">_</span>' + content
					);
			...
			return Option.from( container )
				...
				.map( function ( elm ) {
					return ! real || ! rng.collapsed
						? child( elm, resolve( elm, offset ) ).getOr( elm )
						: elm;
				} )
				.bind( function ( elm ) {
		...
		var getSelectedBlocks = function ( dom, rng, startElm, endElm ) {
			...
			startElm = dom.getParent(
				startElm || getStart$2( root, rng, rng.collapsed ),
				dom.isBlock
			);
		...
		var isAttachedToDom = function ( node ) {
			return (
				...
				contains$3(
					Element.fromDom( node.ownerDocument ),
					Element.fromDom( node )
				)
			);
		};
		var isValidRange = function ( rng ) {
			if ( ! rng ) {
				...
				return (
					isAttachedToDom( rng.startContainer ) &&
					isAttachedToDom( rng.endContainer )
				);
			}
		};
		var Selection$1 = function ( dom, win, serializer, editor ) {
			...
			var getRng = function () {
				...
				var tryCompareBoundaryPoints = function (
					...
					try {
						return sourceRange.compareBoundaryPoints(
							how,
							destinationRange
						);
					} catch ( ex ) {
				...
				if (
					editor.bookmark !== undefined &&
					EditorFocus.hasFocus( editor ) === false
				) {
					var bookmark = SelectionBookmark.getRng( editor );
					if ( bookmark.isSome() ) {
						return bookmark
							.map( function ( r ) {
								return EventProcessRanges.processRanges(
									editor,
									[ r ]
								)[ 0 ];
							} )
				...
				try {
					if (
						( selection = getSel() ) &&
						! NodeType.isRestrictedNode( selection.anchorNode )
					) {
			...
			var normalize = function () {
				...
				var sel = getSel();
				if (
					! MultiRange.hasMultipleRanges( sel ) &&
					hasAnyRanges( editor )
				) {
			...
			var scrollIntoView = function ( elm, alignToTop ) {
				return ScrollIntoView.scrollElementIntoView(
					editor,
					elm,
					alignToTop
				);
			...
			var getBoundingClientRect = function () {
				var rng = getRng();
				return rng.collapsed
					? CaretPosition$1.fromRangeStart(
							rng
					  ).getClientRects()[ 0 ]
		...
		var getBreakType = function ( scope, direction, currentPos, nextPos ) {
			if (
				NodeType.isBr(
					nextPos.getNode( direction === HDirection.Forwards )
				)
			) {
				return BreakType.Br;
		...
		var getPositionsUntil = function (
			predicate,
			direction,
			scope,
			start
			...
			while ( currentPos ) {
				...
				if ( NodeType.isBr( nextPos.getNode( false ) ) ) {
					if ( direction === HDirection.Forwards ) {
						return {
							positions: flip( direction, positions ).concat( [
								nextPos,
							] ),
		...
		var getAdjacentLinePositions = function (
			...
			return getPositionsUntilBreak( scope, start )
				.breakAt.map( function ( pos ) {
					var positions = getPositionsUntilBreak(
						scope,
						pos
					).positions;
					return direction === HDirection.Backwards
		...
		var getFirstLinePositions = function ( scope ) {
			return CaretFinder.firstPositionIn( scope )
				.map( function ( pos ) {
					return [ pos ].concat(
						getPositionsUntilNextLine( scope, pos ).positions
					);
				} )
				.getOr( [] );
		};
		var getLastLinePositions = function ( scope ) {
			return CaretFinder.lastPositionIn( scope )
				.map( function ( pos ) {
					return getPositionsUntilPreviousLine(
						scope,
						pos
					).positions.concat( pos );
				} )
		...
		var moveToCeFalseHorizontally = function (
			direction,
			editor,
			getNextPosFn,
			range
			...
			var isBeforeContentEditableFalseFn = forwards
				? isBeforeContentEditableFalse
				: isAfterContentEditableFalse;
			if ( ! range.collapsed ) {
			...
			if ( isBeforeContentEditableFalseFn( caretPosition ) ) {
				return selectNode(
					editor,
					caretPosition.getNode( ! forwards )
				);
			}
			var nextCaretPosition = InlineUtils.normalizePosition(
				forwards,
			...
			if ( rangeIsInContainerBlock ) {
				return renderRangeCaret(
					editor,
					nextCaretPosition.toRange(),
					true
				);
			}
			return null;
		};
		var moveToCeFalseVertically = function (
			direction,
			editor,
			walkerFn,
			range
			...
			closestNextLineRect = findClosestClientRect(
				nextLinePositions,
				clientX
			);
			...
			if ( contentEditableFalseNode ) {
				var caretPositions = positionsUntil(
					...
					editor.getBody(),
					isAboveLine( 1 ),
					contentEditableFalseNode
				);
				closestNextLineRect = findClosestClientRect(
					filter( caretPositions, isLine( 1 ) ),
					clientX
				);
				if ( closestNextLineRect ) {
					return renderRangeCaret(
				...
				closestNextLineRect = ArrUtils.last(
					filter( caretPositions, isLine( 0 ) )
				);
				if ( closestNextLineRect ) {
					return renderRangeCaret(
						editor,
		...
		var createTextBlock = function ( editor ) {
			var textBlock = editor.dom.create(
		...
		var exitPreBlock = function ( editor, direction, range ) {
			...
			if ( range.collapsed && editor.settings.forced_root_block ) {
				...
				if ( direction === 1 ) {
					caretPos = getNextVisualCaretPosition(
						CaretPosition$1.fromRangeStart( range )
					);
				} else {
					caretPos = getPrevVisualCaretPosition(
						CaretPosition$1.fromRangeStart( range )
					);
				}
				if ( ! caretPos ) {
		...
		var getHorizontalRange = function ( editor, forward ) {
			...
			var getNextPosFn = forward
				...
				: getPrevVisualCaretPosition;
			var range = editor.selection.getRng();
			newRange = moveToCeFalseHorizontally(
				direction,
				editor,
				getNextPosFn,
				range
		...
		var getVerticalRange = function ( editor, down ) {
			...
			var range = editor.selection.getRng();
			newRange = moveToCeFalseVertically(
				direction,
				editor,
				walkerFn,
				range
		...
		var moveToLineEndPoint = function ( editor, forward ) {
			return function () {
				var from = forward
					? CaretPosition$1.fromRangeEnd( editor.selection.getRng() )
					: CaretPosition$1.fromRangeStart(
							editor.selection.getRng()
					  );
				var result = forward
					? getPositionsUntilNextLine( editor.getBody(), from )
					: getPositionsUntilPreviousLine( editor.getBody(), from );
				var to = forward
					? last( result.positions )
					: head( result.positions );
				return to
					.filter( isCefPosition( forward ) )
					.fold( constant( false ), function ( pos ) {
						editor.selection.setRng( pos.toRange() );
						return true;
					} );
			};
					...
					return acc.fold(
						...
						function ( oldCorner ) {
							var oldDist = Math.sqrt(
								Math.abs( oldCorner.x - x ) +
									Math.abs( oldCorner.y - y )
							);
							var newDist = Math.sqrt(
								Math.abs( newCorner.x - x ) +
									Math.abs( newCorner.y - y )
							);
							return Option.some(
								newDist < oldDist ? newCorner : oldCorner
							);
						}
					);
				},
		...
		var getClosestCellAbove = curry(
			getClosestCell$1,
			getBottomValue,
			isAbove$1
		);
		var getClosestCellBelow = curry(
			getClosestCell$1,
			getTopValue,
			isBelow$1
		);
		var findClosestPositionInAboveCell = function ( table, pos ) {
			return head( pos.getClientRects() )
				...
				.bind( function ( cell ) {
					return findClosestHorizontalPosition(
						getLastLinePositions( cell ),
						pos
					);
				} );
		};
		var findClosestPositionInBelowCell = function ( table, pos ) {
			return last( pos.getClientRects() )
				.bind( function ( rect ) {
					return getClosestCellBelow( table, rect.left, rect.top );
				} )
				.bind( function ( cell ) {
					return findClosestHorizontalPosition(
						getFirstLinePositions( cell ),
						pos
					);
				} );
		};
		...
		var hasNextBreak = function ( getPositionsUntil, scope, lineInfo ) {
			return lineInfo.breakAt
				.map( function ( breakPos ) {
					return getPositionsUntil(
						scope,
						breakPos
					).breakAt.isSome();
				} )
		...
		var startsWithWrapBreak = function ( lineInfo ) {
			return (
				lineInfo.breakType === BreakType.Wrap &&
				lineInfo.positions.length === 0
			);
		};
		var startsWithBrBreak = function ( lineInfo ) {
			return (
				lineInfo.breakType === BreakType.Br &&
				lineInfo.positions.length === 1
			);
		};
		var isAtTableCellLine = function ( getPositionsUntil, scope, pos ) {
			...
			if (
				...
				( ! NodeType.isBr( pos.getNode() ) &&
					startsWithBrBreak( lineInfo ) )
			) {
		...
		var isCaretAtStartOrEndOfTable = function ( forward, rng, table ) {
			var caretPos = CaretPosition$1.fromRangeStart( rng );
			return CaretFinder.positionIn( ! forward, table )
				.map( function ( pos ) {
		...
		var getClosestAbovePosition = function ( root, table, start ) {
			return findClosestPositionInAboveCell( table, start )
				.orThunk( function () {
					return head( start.getClientRects() ).bind( function (
						rect
					) {
						return findClosestHorizontalPositionFromPoint(
							getPositionsAbove(
								root,
								CaretPosition$1.before( table )
							),
							rect.left
						);
					} );
		...
		var getTable = function ( previous, pos ) {
			...
			return NodeType.isElement( node ) && node.nodeName === 'TABLE'
				? Option.some( node )
				: Option.none();
		};
		...
		var moveCaret = function ( editor, down, pos ) {
			...
			table.fold(
				...
				function ( table ) {
					return CaretFinder.positionIn( last, editor.getBody() )
		...
		var moveH$1 = function ( editor, forward ) {
			return function () {
				return Option.from(
					...
					.bind( function ( td ) {
						return Option.from(
							editor.dom.getParent( td, 'table' )
						).map( function ( table ) {
							return navigateHorizontally(
								editor,
								forward,
								table
							);
		...
		var isAtFirstOrLastLine = function ( root, forward, pos ) {
			return forward
				? isAtLastLine( root.dom(), pos )
				: isAtFirstLine( root.dom(), pos );
		};
		...
		var moveToCaretPosition = function ( editor, root ) {
			...
			if ( /^(LI|DT|DD)$/.test( root.nodeName ) ) {
				var firstChild = firstNonWhiteSpaceNodeSibling(
					root.firstChild
				);
				if (
					firstChild &&
					/^(UL|OL|DL)$/.test( firstChild.nodeName )
				) {
					root.insertBefore(
						dom.doc.createTextNode( '\xA0' ),
						root.firstChild
					);
				}
			}
		...
		var getParentBlockName = function ( editor ) {
			return getParentBlock$2( editor ).fold(
				constant( '' ),
		...
		var isListItemParentBlock = function ( editor ) {
			return getParentBlock$2( editor )
				.filter( function ( elm ) {
		...
		var hasParent$1 = function ( elm, parentName ) {
			return (
				elm && elm.parentNode && elm.parentNode.nodeName === parentName
			);
		};
		...
		var insert$1 = function (
			...
			if (
				...
				if ( hasParent$1( containerBlock, 'LI' ) ) {
					dom.insertAfter(
						newBlock,
						getContainerBlock( containerBlock )
					);
				} else {
				...
				if ( hasParent$1( containerBlock, 'LI' ) ) {
					...
					containerBlock.parentNode.insertBefore(
						newBlock,
						containerBlock
					);
				}
		...
		var trimZwsp = function ( fragment ) {
			each(
				descendants( Element.fromDom( fragment ), isText ),
				function ( text ) {
		...
		var normalizeZwspOffset = function ( start, container, offset ) {
			if ( NodeType.isText( container ) === false ) {
				...
				return offset === 1 &&
					container.data.charAt( offset - 1 ) === Zwsp.ZWSP
					? 0
					: offset;
			} else {
		...
		var applyAttributes = function ( editor, node, forcedRootBlockAttrs ) {
			Option.from( forcedRootBlockAttrs.style )
				...
				.each( function ( attrStyles ) {
					...
					var newStyles = __assign(
						__assign( {}, currentStyles ),
						attrStyles
					);
					editor.dom.setStyles( node, newStyles );
				} );
			var attrClassesOpt = Option.from( forcedRootBlockAttrs.class ).map(
				function ( attrClasses ) {
					return attrClasses.split( /\s+/ );
				}
			);
			var currentClassesOpt = Option.from( node.className ).map(
				function ( currentClasses ) {
					return filter(
						currentClasses.split( /\s+/ ),
						function ( clazz ) {
							return clazz !== '';
						}
					);
			...
			lift2(
				...
				function ( attrClasses, currentClasses ) {
					var filteredClasses = filter(
						currentClasses,
						function ( clazz ) {
							return ! contains( attrClasses, clazz );
						}
					);
					var newClasses = __spreadArrays(
						attrClasses,
						filteredClasses
					);
		...
		var wrapSelfAndSiblingsInDefaultBlock = function (
			...
			if ( ! parentBlock || ! canSplitBlock( dom, parentBlock ) ) {
				...
				if (
					parentBlock === editor.getBody() ||
					isTableCell$5( parentBlock )
				) {
					rootBlockName = parentBlock.nodeName.toLowerCase();
				} else {
					rootBlockName =
						parentBlock.parentNode.nodeName.toLowerCase();
				}
		...
		var addBrToBlockIfNeeded = function ( dom, block ) {
			...
			if (
				! lastChild ||
				/^(left|right)$/gi.test(
					dom.getStyle( lastChild, 'float', true )
				)
			) {
				dom.add( block, 'br' );
			}
		...
		var insert$2 = function ( editor, evt ) {
			...
			var isCaretAtStartOrEndOfBlock = function ( start ) {
				...
				normalizedOffset = normalizeZwspOffset(
					start,
					container,
					offset
				);
			...
			if ( isCaretContainerBlock( parentBlock ) ) {
				...
				NewLineUtils.moveToCaretPosition(
					editor,
					containerAndSiblingName( parentBlock, 'HR' )
						? newBlock
						: parentBlock
				);
			} else {
				...
				trimInlineElementsOnLeftSideOfBlock(
					dom,
					nonEmptyElementsMap,
					newBlock
				);
		...
		var matchesSelector = function ( editor, selector ) {
			return NewLineUtils.getParentBlock( editor )
				.filter( function ( parentBlock ) {
					return (
						selector.length > 0 &&
						is$1( Element.fromDom( parentBlock ), selector )
					);
				} )
				.isSome();
		};
		var shouldInsertBr = function ( editor ) {
			return matchesSelector(
				editor,
				Settings.getBrNewLineSelector( editor )
			);
		};
		var shouldBlockNewLine = function ( editor ) {
			return matchesSelector(
				editor,
				Settings.getNoNewLineSelector( editor )
			);
		};
		...
		var inListBlock = function ( requiredState ) {
			return function ( editor, shiftKey ) {
				return (
					NewLineUtils.isListItemParentBlock( editor ) ===
					requiredState
				);
			};
		};
		var inBlock = function ( blockName, requiredState ) {
			return function ( editor, shiftKey ) {
				var state =
					NewLineUtils.getParentBlockName( editor ) ===
		...
		var canInsertIntoEditableRoot = function ( editor ) {
			var forcedRootBlock = Settings.getForcedRootBlock( editor );
			var rootEditable = NewLineUtils.getEditableRoot(
				editor.dom,
				editor.selection.getStart()
			);
			return (
		...
		var getAction$1 = function ( editor, evt ) {
			return LazyEvaluator.evaluateUntil(
				[
					...
					match$2(
						[ inPreBlock( true ), shouldPutBrInPre$1( false ) ],
						newLineAction.block()
					),
					match$2(
						[
					...
					match$2(
						[ inPreBlock( true ), shouldPutBrInPre$1( true ) ],
						newLineAction.br()
					),
					match$2(
						[ inListBlock( true ), hasShiftKey ],
						newLineAction.br()
					),
		...
		var insertTextAtPosition = function ( text, pos ) {
			...
			if ( NodeType.isText( container ) ) {
				...
				return Option.some(
					CaretPosition( container, offset + text.length )
				);
			} else {
		...
		var navigateIgnoreEmptyTextNodes = function ( forward, root, from ) {
			return CaretFinder.navigateIgnore(
				forward,
				root,
				from,
				isEmptyText
		...
		var isAtBeforeAfterBlockBoundary = function ( forward, root, pos ) {
			return navigateIgnoreEmptyTextNodes(
				forward,
				root.dom(),
				pos
			).forall( function ( newPos ) {
				return getClosestBlock( root, pos ).fold(
					...
					function ( fromBlock ) {
						return (
							...
							contains$3(
								fromBlock,
								Element.fromDom( newPos.container() )
							)
						);
					}
				);
		...
		var hasSpaceBefore = function ( root, pos ) {
			if ( isInMiddleOfText( pos ) ) {
				...
				return (
					isAfterSpace( pos ) ||
					CaretFinder.prevPosition(
						getClosestBlock$1( root, pos ).dom(),
						pos
					).exists( isAfterSpace )
				);
			}
		...
		var hasSpaceAfter = function ( root, pos ) {
			if ( isInMiddleOfText( pos ) ) {
				...
				return (
					isBeforeSpace( pos ) ||
					CaretFinder.nextPosition(
						getClosestBlock$1( root, pos ).dom(),
						pos
					).exists( isBeforeSpace )
				);
			}
		};
		...
		var needsToBeNbsp = function ( root, pos ) {
			return (
				needsToBeNbspLeft( root, pos ) ||
				needsToBeNbspRight( root, pos )
			);
		};
		...
		var hasNbsp = function ( pos ) {
			...
			return (
				NodeType.isText( container ) &&
				contains$2( container.data, nbsp )
			);
		};
		...
		var insertSpaceOrNbspAtSelection = function ( editor ) {
			...
			var root = Element.fromDom( editor.getBody() );
			if ( editor.selection.isCollapsed() ) {
				var isInlineTarget = curry(
					InlineUtils.isInlineTarget,
					editor
				);
				var caretPosition = CaretPosition$1.fromRangeStart(
					editor.selection.getRng()
				);
				return BoundaryLocation.readLocation(
					isInlineTarget,
		...
		var setup$b = function ( editor ) {
			editor.on(
				...
				curry( handleBlockContainer, editor )
			);
		};
		...
		function Quirks( editor ) {
			...
			var getMceInternalContent = function ( e ) {
				...
				if ( e.dataTransfer ) {
					internalContent =
						e.dataTransfer.getData( mceInternalDataType );
					if (
						internalContent &&
			...
			var insertClipboardContents = function ( content, internal ) {
				if (
					editor.queryCommandSupported( 'mceInsertClipboardContent' )
				) {
				...
				if ( ! editor.settings.content_editable ) {
					dom.bind(
						...
						function ( e ) {
							var rng;
							if (
								e.target === editor.getDoc().documentElement
							) {
								rng = selection.getRng();
								editor.getBody().focus();
								if ( e.type === 'mousedown' ) {
									if (
										isCaretContainer( rng.startContainer )
									) {
										return;
									}
			...
			var removeHrOnBackspace = function () {
				editor.on( 'keydown', function ( e ) {
					if (
						...
						if (
							! editor.getBody().getElementsByTagName( 'hr' )
								.length
						) {
							return;
						}
					...
					if (
						...
						if (
							...
							if (
								previousSibling &&
								previousSibling.nodeName &&
								previousSibling.nodeName.toLowerCase() ===
									'table'
							) {
								e.preventDefault();
								return false;
			...
			var restoreFocusOnKeyDown = function () {
				if ( ! editor.inline ) {
					editor.on( 'keydown', function () {
						if (
							domGlobals.document.activeElement ===
							domGlobals.document.body
						) {
							editor.getWin().focus();
						}
			...
			var ieInternalDragAndDrop = function () {
				...
				editor.on( 'drop', function ( e ) {
					if ( ! isDefaultPrevented( e ) ) {
						...
						if (
							internalContent &&
							internalContent.id !== editor.id
						) {
							e.preventDefault();
							var rng = CaretRangeFromPoint.fromPoint(
								e.x,
								e.y,
								editor.getDoc()
							);
		...
		var isTextBlockNode = function ( node ) {
			return (
				NodeType.isElement( node ) &&
				isTextBlock( Element.fromDom( node ) )
			);
		};
		var normalizeSelection$1 = function ( editor ) {
			...
			if ( CaretPosition.isElementPosition( startPos ) ) {
				...
				if ( isTextBlockNode( container ) ) {
					CaretFinder.firstPositionIn( container ).each( function (
						pos
					) {
			...
			if ( CaretPosition.isElementPosition( endPos ) ) {
				var container = startPos.container();
				if ( isTextBlockNode( container ) ) {
					CaretFinder.lastPositionIn( container ).each( function (
						pos
					) {
					...
					while ( i-- ) {
						...
						if ( ! node.attributes.map[ internalName ] ) {
							...
							if ( name === 'style' ) {
								...
								node.attr(
									internalName,
									editor.convertURL( value, name, node.name )
								);
							}
						}
					}
				...
				Delay.setEditorTimeout(
					...
					function () {
						...
						if ( editor.settings.auto_focus === true ) {
							...
							focusEditor = editor.editorManager.get(
								editor.settings.auto_focus
							);
						}
		...
		var initContentBody = function ( editor, skipWrite ) {
			...
			if ( settings.content_editable ) {
				...
				editor.contentDocument = doc =
					settings.content_document || domGlobals.document;
				editor.contentWindow =
					settings.content_window || domGlobals.window;
			...
			if ( ! editor.readonly ) {
				if (
					editor.inline &&
					DOM$2.getStyle( body, 'position', true ) === 'static'
				) {
					body.style.position = 'relative';
				}
				body.contentEditable = editor.getParam(
			...
			editor.fire( 'PreInit' );
			if (
				! settings.browser_spellcheck &&
				! settings.gecko_spellcheck
			) {
		...
		var relaxDomain = function ( editor, ifr ) {
			if (
				domGlobals.document.domain !==
					domGlobals.window.location.hostname &&
				Env.ie &&
				Env.ie < 12
			) {
		...
		var getIframeHtml = function ( editor ) {
			...
			if (
				Settings.getDocumentBaseUrl( editor ) !== editor.documentBaseUrl
			) {
				iframeHTML +=
					'<base href="' + editor.documentBaseURI.getURI() + '" />';
			}
			iframeHTML +=
		...
		var init$1 = function ( editor, boxInfo ) {
			...
			if ( boxInfo.editorContainer ) {
				DOM$3.get( boxInfo.editorContainer ).style.display =
					editor.orgDisplay;
		...
		var initPlugin = function ( editor, initializedPlugins, plugin ) {
			...
			var pluginUrl =
				PluginManager$1.urls[ plugin ] ||
				editor.documentBaseUrl.replace( /\/$/, '' );
			plugin = Tools.trim( plugin );
			if (
				Plugin &&
				Tools.inArray( initializedPlugins, plugin ) === -1
			) {
				Tools.each(
					PluginManager$1.dependencies( plugin ),
					function ( dep ) {
		...
		var renderFromLoadedTheme = function ( editor ) {
			...
			if ( ! settings.content_editable ) {
				h =
					( info.iframeHeight || h ) +
					( typeof h === 'number' ? info.deltaHeight || 0 : '' );
				if ( h < minHeight ) {
					h = minHeight;
				}
			}
		...
		var init$2 = function ( editor ) {
			...
			settings.aria_label =
				...
				DOM$4.getAttrib(
					elm,
					'aria-label',
					editor.getLang( 'aria.rich_text_area' )
				);
			...
			if ( settings.content_css ) {
				Tools.each(
					Tools.explode( settings.content_css ),
					function ( u ) {
						editor.contentCSS.push(
							editor.documentBaseURI.toAbsolute( u )
						);
					}
				);
			}
		...
		var loadLanguage = function ( scriptLoader, editor ) {
			...
			if (
				settings.language_url &&
				! editor.editorManager.i18n.data[ settings.language ]
			) {
		...
		var loadTheme = function ( scriptLoader, editor, suffix, callback ) {
			...
			if ( isString( theme ) ) {
				if (
					...
					! ThemeManager.urls.hasOwnProperty( theme )
				) {
		...
		var render = function ( editor ) {
			...
			var form =
				editor.getElement().form || DOM$5.getParent( id, 'form' );
			if ( form ) {
				editor.formElement = form;
				if (
					settings.hidden_input &&
					! /TEXTAREA|INPUT/i.test( editor.getElement().nodeName )
				) {
					DOM$5.insertAfter(
						DOM$5.create( 'input', {
							type: 'hidden',
		...
		var URI = function ( url, settings ) {
			...
			if ( url.indexOf( '/' ) === 0 && ! isProtocolRelative ) {
				url =
					( baseUri ? baseUri.protocol || 'http' : 'http' ) +
					'://mce_host' +
					url;
			}
			if ( ! /^[\w\-]*:?\/\//.test( url ) ) {
				baseUrl = settings.base_uri
					...
					: new URI( domGlobals.document.location.href ).directory;
		...
		URI.prototype = {
			...
			toRelative: function ( uri ) {
				...
				if (
					tu === uu ||
					( tu.charAt( tu.length - 1 ) === '/' &&
						tu.substr( 0, tu.length - 1 ) === uu )
				) {
					return tu;
				}
			...
			toRelPath: function ( base, path ) {
				...
				for (
					i = 0, l = base.length - ( breakPoint - 1 );
					i < l;
					i++
				) {
					out += '../';
		...
		var getContentFromBody = function ( editor, args, body ) {
			...
			if (
				args.format !== 'text' &&
				! isWsPreserveElement( Element.fromDom( body ) )
			) {
		...
		var getContent$1 = function ( editor, args ) {
			...
			return Option.from( editor.getBody() ).fold(
				constant(
					args.format === 'tree' ? new Node$1( 'body', 11 ) : ''
				),
				function ( body ) {
		...
		var findMatchingNodes = function (
			nodeFilters,
			attributeFilters,
			node
		) {
		...
		var moveSelection = function ( editor ) {
			if ( EditorFocus.hasFocus( editor ) ) {
				CaretFinder.firstPositionIn( editor.getBody() ).each( function (
					pos
				) {
					var node = pos.getNode();
					var caretPos = NodeType.isTable( node )
		...
		var extend$4 = Tools.extend,
			each$l = Tools.each;
		var resolve$4 = Tools.resolve;
		var ie$2 = Env.ie;
		var Editor = function ( id, settings, editorManager ) {
			var self = this;
			var documentBaseUrl = ( self.documentBaseUrl =
				editorManager.documentBaseURL );
		...
		Editor.prototype = {
			...
			execCallback: function ( name ) {
				...
				if (
					self.callbackLookup &&
					( scope = self.callbackLookup[ name ] )
				) {
					callback = scope.func;
					scope = scope.scope;
				}
			...
			translate: function ( text ) {
				if ( text && Tools.is( text, 'string' ) ) {
					var lang_1 = this.settings.language || 'en',
						i18n_1 = this.editorManager.i18n;
					text =
						i18n_1.data[ lang_1 + '.' + text ] ||
						text.replace( /\{\#([^\}]+)\}/g, function ( a, b ) {
							return (
								i18n_1.data[ lang_1 + '.' + b ] ||
								'{#' + b + '}'
							);
						} );
				}
			...
			addButton: function ( name, settings ) {
				...
				if (
					settings.stateSelector &&
					typeof settings.active === 'undefined'
				) {
			...
			addQueryStateHandler: function ( name, callback, scope ) {
				this.editorCommands.addQueryStateHandler(
					name,
					callback,
					scope
				);
			...
			addQueryValueHandler: function ( name, callback, scope ) {
				this.editorCommands.addQueryValueHandler(
					name,
					callback,
					scope
				);
			...
			addVisual: function ( elm ) {
				...
				each$l( dom.select( 'table,a', elm ), function ( elm ) {
					...
					switch ( elm.nodeName ) {
						case 'TABLE':
							cls =
								settings.visual_table_class || 'mce-item-table';
		...
		var isUIElement = function ( editor, elm ) {
			var customSelector = editor
				? editor.settings.custom_ui_selector
				: '';
			var parent = DOM$8.getParent( elm, function ( elm ) {
				return (
					isEditorUIElement$1( elm ) ||
					( customSelector
						? editor.dom.is( elm, customSelector )
						: false )
				);
			} );
			return parent !== null;
		...
		var registerEvents = function ( editorManager, e ) {
			...
			editor.on( 'focusout', function () {
				...
				Delay.setEditorTimeout( self, function () {
					var focusedEditor = editorManager.focusedEditor;
					if (
						! isUIElement( self, getActiveElement() ) &&
						focusedEditor === self
					) {
			...
			if ( ! documentFocusInHandler ) {
				documentFocusInHandler = function ( e ) {
					...
					if (
						activeEditor &&
						target.ownerDocument === domGlobals.document
					) {
						if (
							target !== domGlobals.document.body &&
		...
		var setup$h = function ( editorManager ) {
			editorManager.on(
				'AddEditor',
				curry( registerEvents, editorManager )
			);
			editorManager.on(
				'RemoveEditor',
		...
		var removeEditorFromList = function ( targetEditor ) {
			...
			if ( EditorManager.activeEditor === targetEditor ) {
				EditorManager.activeEditor =
					editors.length > 0 ? editors[ 0 ] : null;
			}
		...
		EditorManager = {
			...
			setup: function () {
				...
				var baseURL,
					documentBaseURL,
					suffix = '';
				documentBaseURL = URI.getDocumentBaseUrl(
					domGlobals.document.location
				);
				...
				if ( preInit ) {
					...
					var scripts =
						domGlobals.document.getElementsByTagName( 'script' );
			...
			overrideDefaults: function ( defaultSettings ) {
				...
				for ( var name in pluginBaseUrls ) {
					AddOnManager.PluginManager.urls[ name ] =
						pluginBaseUrls[ name ];
				}
			},
			init: function ( settings ) {
				...
				var isInvalidInlineTarget = function ( settings, elm ) {
					return (
						settings.inline &&
						elm.tagName.toLowerCase() in invalidInlineTargets
					);
				};
			...
			var normalize = function ( rng ) {
				return NormalizeRange.normalize( dom, rng ).fold(
					...
					function ( normalizedRng ) {
						rng.setStart(
							normalizedRng.startContainer,
							normalizedRng.startOffset
						);
						rng.setEnd(
							normalizedRng.endContainer,
							normalizedRng.endOffset
						);
						return true;
					}
				);
		...
		var Factory = {
			...
			get: function ( type ) {
				...
				var controlType = types.hasOwnProperty( lctype )
					? types[ lctype ]
					: null;
				if ( controlType === null ) {
					throw new Error(
						'Could not find module for type: ' + type
					);
				}
				return controlType;
			},
			create: function ( type, settings ) {
				...
				if ( ! ControlType ) {
					throw new Error(
						'Could not find control by type: ' + type
					);
				}
		...
		var each$n = Tools.each,
			extend$6 = Tools.extend;
			...
			var parse = function ( value ) {
				...
				if ( typeof value === 'object' ) {
					...
					if (
						...
						( matches = /#([0-F]{2})([0-F]{2})([0-F]{2})/gi.exec(
							value
						) )
					) {
						...
						( matches = /#([0-F])([0-F])([0-F])/gi.exec( value ) )
					) {
		...
		var XHR = {
			send: function ( settings ) {
				...
				settings.success_scope =
					settings.success_scope || settings.scope;
