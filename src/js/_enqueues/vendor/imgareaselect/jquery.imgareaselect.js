...
( function ( $ ) {
	...
	$.imgAreaSelect = function ( img, options ) {
		...
		function evX( event ) {
			return (
				max( event.pageX || 0, touchCoords( event ).x ) - parOfs.left
			);
		}
			...
			parOfs = /absolute|relative/.test( $parent.css( 'position' ) )
				? {
						left:
							...
							$parent.scrollLeft(),
						top:
							floor( $parent.offset().top ) - $parent.scrollTop(),
				  }
				: position == 'fixed'
				? {
		...
		function update( resetKeyPress ) {
			...
			$border
				...
				.height(
					max( h - $border.outerHeight() + $border.innerHeight(), 0 )
				);
		...
		function setOptions( newOptions ) {
			if ( newOptions.parent )
				( $parent = $( newOptions.parent ) ).append(
					$box.add( $outer )
				);
			...
			$box.append( $area.add( $border ).add( $areaOpera ) ).append(
				$handles
			);
