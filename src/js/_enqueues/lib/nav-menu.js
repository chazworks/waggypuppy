	...
	api = window.wpNavMenu = {
				...
				updateParentMenuItemDBId: function () {
					return this.each( function () {
						var item = $( this ),
							...
							parentDepth = depth - 1,
							parent = item
								.prevAll( '.menu-item-depth-' + parentDepth )
								.first();
						...
						if ( 0 === depth ) {
							...
							input.val(
								parent.find( '.menu-item-data-db-id' ).val()
							);
						}
					} );
				},
					...
					return this.each( function () {
						...
						api.addItemToMenu(
							...
							function () {
								...
								t.find( '.button-controls .select-all' ).prop(
									'checked',
									false
								);
								t.find(
		...
		moveMenuItem: function ( $this, dir ) {
			var items,
				...
				a11ySpeech =
					menus[
						'moved' + dir.charAt( 0 ).toUpperCase() + dir.slice( 1 )
					];
			...
			switch ( dir ) {
				...
				case 'down':
					...
					if ( thisItemChildren ) {
						...
						if ( nextItemChildren ) {
							newDepth =
								parseInt( nextItem.menuItemDepth(), 10 ) + 1;
							thisItem.moveHorizontally(
								newDepth,
								thisItemDepth
							);
						...
						items
							.detach()
							.insertAfter(
								menuItems.eq( thisItemPosition + items.length )
							)
							.updateParentMenuItemDBId();
					} else {
		...
		refreshAdvancedAccessibilityOfItem: function ( itemToRefresh ) {
			...
			if ( isPrimaryMenuItem ) {
				...
				( parentItem = menuItem
					...
					( itemPosition =
						$(
							subItems.parents( '.menu-item' ).get().reverse()
						).index( menuItem ) + 1 );
				...
				.on( 'focus', function () {
					$( this )
						...
						.on( 'keydown', function ( e ) {
							...
							$(
								'#edit-' + thisItemData[ 'menu-item-db-id' ]
							).trigger( 'focus' );
							return false;
						} );
				} );
			...
			api.menuList.sortable( {
				...
				start: function ( e, ui ) {
					...
					children.each( function () {
						...
						maxChildDepth =
							depth > maxChildDepth ? depth : maxChildDepth;
					} );
				...
				sort: function ( e, ui ) {
					var offset = ui.helper.offset(),
						edge = api.isRTL
							...
							: offset.left,
						depth =
							api.negateIfRTL * api.pxToDepth( edge - menuEdge );
		...
		disableBulkSelection: function () {
			...
			$( '#nav-menu-bulk-actions-bottom' ).removeClass(
				'bulk-selection'
			);
					...
					$.each(
						...
						function ( index, element ) {
							var itemName = $( element )
								...
								.text();
							var itemSpeech = menus.menuItemDeletion.replace(
								'%s',
								itemName
							);
		...
		attachTabsPanelListeners: function () {
			$( '#menu-settings-column' ).on( 'click', function ( e ) {
				...
				if ( target.hasClass( 'nav-tab-link' ) ) {
					...
					if (
						! wrapper.find( '.tabs-panel-active .menu-item-title' )
							.length
					) {
			...
			if ( matchedSection && matchedSection[ 1 ] ) {
				...
				if ( 0 !== item.length ) {
					if ( item.hasClass( 'menu-item-edit-inactive' ) ) {
						...
						item.removeClass( 'menu-item-edit-inactive' ).addClass(
							'menu-item-edit-active'
						);
					} else {
						settings.slideUp( 'fast' );
						item.removeClass( 'menu-item-edit-active' ).addClass(
