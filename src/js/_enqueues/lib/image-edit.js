...
( function ( $ ) {
	...
	var imageEdit = ( window.imageEdit = {
			...
			t.iasapi = $image.imgAreaSelect( {
				...
				onSelectChange: function ( img, c ) {
					...
					selW.val(
						Math.min(
							imageEdit.hold.w,
							imageEdit.round( c.width / sizer )
						)
					);
					selH.val(
						Math.min(
							imageEdit.hold.h,
							imageEdit.round( c.height / sizer )
						)
					);
		...
		setCropSelection: function ( postid, c ) {
			...
			var excessW =
				hold.w -
				( Math.round( c.x1 / sizer ) + parseInt( selW.val() ) );
			var excessH =
				hold.h -
				( Math.round( c.y1 / sizer ) + parseInt( selH.val() ) );
		...
		setNumSelection: function ( postid, el ) {
			...
			if (
				...
				if ( x2 > imgw ) {
					...
					x2 = imgw;
					elX.val(
						Math.min( this.hold.w, Math.round( x2 / sizer ) )
					);
				}
				...
				if ( y2 > imgh ) {
					...
					y2 = imgh;
					elY.val(
						Math.min( this.hold.h, Math.round( y2 / sizer ) )
					);
				}
