...
( function ( QUnit ) {
				...
				wp.api.loadPromise.done( function () {
					...
					theCollection.fetch().done( function () {
						...
						if ( ! _.isUndefined( collectionHelperTest ) ) {
							...
							_.each(
								collectionHelperTest.supportsMethods,
								function ( method ) {
									assert.equal(
										'function',
	...
	_.each( modelsWithIdsClassNames, function ( className ) {
		QUnit.test( 'Checking ' + className + ' model.', function ( assert ) {
			...
			wp.api.loadPromise.done( function () {
				...
				assert.ok(
					theModel,
					'We can instantiate wp.api.models.' + className
				);
	...
	var modelsWithIndexes = [ 'Taxonomy', 'Status', 'Type' ];
	...
	_.each( collectionsToFetchByRoute, function ( collection ) {
		QUnit.test(
			'Test fetching ' + collection + ' by route.',
			function ( assert ) {
				var done = assert.async();
				...
				QUnit.test(
					...
					function ( assert ) {
						...
						wp.api.loadPromise.done( function () {
							...
							var theCollection = new wp.api.collections[
								className
							]();
							assert.ok(
								theCollection,
								'We can instantiate wp.api.collections.' +
									className
							);
							// Trigger Qunit async completion.
							done();
	...
	wp.api.loadPromise.done( function () {
		QUnit.test(
			'Checking connecting to a remote url.',
			function ( assert ) {
				var done = assert.async();
				...
				wp.api
					...
					.done( function () {
						...
						assert.equal(
							'http://remotehost/wp-json/wp/v2/posts',
							post.url(),
							'The remote API objects should have their own URLs'
						);
						...
						wp.api
							...
							.done( function () {
								...
								assert.equal(
									...
									post.url(),
									'The local API objects should have their own URLs'
								);
	...
	_.each( modelsThatNeedrequireForceForDelete, function ( model ) {
		QUnit.test(
			'Test requireForceForDelete is correct for ' + model.name,
			function ( assert ) {
				var done = assert.async();
				assert.expect( 1 );
				wp.api.loadPromise.done( function () {
					...
					assert.equal(
						...
						'wp.api.models.' +
							model.name +
							'.requireForceForDelete should be ' +
							model.expect +
							'.'
					);
	...
	var theModelTypesWithMeta = [ 'Posts', 'Comments', 'Tags', 'Users' ];

	_.each( theModelTypesWithMeta, function ( modelType ) {
		// Test post meta.
		wp.api.loadPromise.done( function () {
			QUnit.test(
				'Check meta support for ' + modelType + '.',
				function ( assert ) {
					...
					theModels.fetch().done( function () {
						...
						if ( 'Tags' === modelType ) {
							expectedMetas =
								'{"test_single":"","test_multi":[],"meta_key":"meta_value","test_tag_meta":""}';
						}

						// Verify the meta object returned correctly from `getMetas()`.
						assert.equal(
							JSON.stringify( endpoint.getMetas() ),
							expectedMetas,
							'Full meta key/values object should be readable.'
						);

						// Verify single meta returned correctly from `getMeta()`
						assert.equal(
							endpoint.getMeta( 'meta_key' ),
							'meta_value',
						...
						assert.equal(
							endpoint.getMeta( 'test_key' ),
							'test_value',
							'Multiple meta should be writable via setMetas.'
						);

						// Verify setting a single meta value with `setMeta()`.
						endpoint.setMeta( 'test_key2', 'test_value2' );
						assert.equal(
							endpoint.getMeta( 'test_key2' ),
							'test_value2',
							'Single meta should be writable via setMeta.'
						);
					} );
				}
			);
