( function ( $, QUnit, tinymce, setTimeout ) {
			...
			if ( keyCode === 8 ) {
				if ( editor.getDoc().selection ) {
					...
					if (
						...
						startContainer.data.length > 0
					) {
						rng.setStart(
							startContainer,
							startContainer.data.length - 1
						);
						rng.setEnd(
							startContainer,
							startContainer.data.length
						);
	...
	QUnit.test(
		'Only transform when at the cursor is at the start.',
		function ( assert ) {
	...
	QUnit.test(
		'Backspace should undo the transformation.',
		function ( assert ) {
			editor.setContent( '<p>test</p>' );
			editor.selection.setCursorLocation();
	...
	QUnit.test(
		'Backspace should undo the transformation only right after it happened.',
		function ( assert ) {
			editor.setContent( '<p>test</p>' );
			editor.selection.setCursorLocation();
	...
	QUnit.test(
		'Inline: convert with previously unconverted pattern.',
		function ( assert ) {
	...
	QUnit.test(
		'Inline: convert with previous pattern characters.',
		function ( assert ) {
			...
			type(
				...
				function () {
					assert.equal(
						editor.getContent(),
						'<p>test``` 123<strong>456</strong></p>'
					);
				},
				assert.async()
			);
	...
	QUnit.test(
		'Inline: disallow after previous pattern characters and leading space.',
		function ( assert ) {
			editor.setContent( '<p>test``` 123</p>' );
			editor.selection.setCursorLocation(
				editor.$( 'p' )[ 0 ].firstChild,
