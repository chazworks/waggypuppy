...
jQuery( 'body' ).on( 'load', function () {
	...
	testCustomizerModel = function ( model, expectedValues ) {
		...
		if ( expectedValues.hasOwnProperty( 'id' ) ) {
			QUnit.test(
				type + ' instance has the right id',
				function ( assert ) {
		...
		if ( expectedValues.hasOwnProperty( 'title' ) ) {
			QUnit.test(
				type + ' instance has the right title.',
				function ( assert ) {
		...
		if ( expectedValues.hasOwnProperty( 'description' ) ) {
			QUnit.test(
				type + ' instance has the right description.',
				function ( assert ) {
					assert.equal(
		...
		if ( expectedValues.hasOwnProperty( 'content' ) ) {
			QUnit.test(
				type + ' instance has the right content.',
				function ( assert ) {
					assert.equal(
		...
		if ( expectedValues.hasOwnProperty( 'priority' ) ) {
			QUnit.test(
				type + ' instance has the right priority.',
				function ( assert ) {
		...
		if ( expectedValues.hasOwnProperty( 'active' ) ) {
			QUnit.test(
				type + ' instance has the right active state.',
				function ( assert ) {
		...
		if ( type === 'Panel' || type === 'Section' ) {
			if ( expectedValues.hasOwnProperty( 'expanded' ) ) {
				QUnit.test(
					type + ' instance has the right expanded state.',
					function ( assert ) {
						assert.equal(
							model.expanded(),
							expectedValues.expanded
						);
					}
				);
			}
			...
			QUnit.test(
				type + ' instance is collapsed after calling .collapse()',
				function ( assert ) {
					model.collapse();
			...
			QUnit.test(
				type + ' instance is expanded after calling .expand()',
				function ( assert ) {
					model.expand();
					assert.ok( model.expanded() );
				}
	...
	QUnit.test( 'Notifications collection exists', function ( assert ) {
		...
		assert.equal(
			wp.customize.notifications.defaultConstructor,
			wp.customize.Notification
		);
	} );

	QUnit.test(
		'Notification objects are rendered as part of notifications collection',
		function ( assert ) {
			var container = jQuery( '#customize-notifications-test' ),
				items,
				collection;
			...
			collection.add(
				'mycode-1',
				new wp.customize.Notification( 'mycode-1', {
			...
			collection.add(
				'mycode-2',
				new wp.customize.Notification( 'mycode-2', {
			...
			collection.render();
			assert.ok(
				collection.container.is( ':hidden' ),
				'Notifications area is hidden.'
			);
		}
	);
	...
	QUnit.test( 'Setting has notifications', function ( assert ) {
		...
		assert.equal(
			wp.customize.Notification,
			setting.notifications.prototype.constructor.defaultConstructor
		);
	} );
	QUnit.test( 'Setting has findControls method', function ( assert ) {
		var controls,
			setting = wp.customize( 'fixture-setting' );
	...
	QUnit.test( 'Custom setting constructor is used', function ( assert ) {
		...
		assert.ok(
			setting.extended( wp.customize.settingConstructor.abbreviation )
		);
		setting.set( 'usa' );
	...
	QUnit.test(
		'Control has the section fixture section ID',
		function ( assert ) {
			var control = wp.customize.control( 'fixture-control' );
	...
	QUnit.test( 'Control has notifications', function ( assert ) {
		var control = wp.customize.control( 'fixture-control' ),
			settingNotification,
			controlOnlyNotification,
			doneEmbedded;
		assert.ok( control.notifications.extended( wp.customize.Values ) );
		assert.equal(
			wp.customize.Notification,
			control.notifications.prototype.constructor.defaultConstructor
		);
		...
		control.deferred.embedded.done( function () {
			...
			assert.equal(
				0,
				_.size( control.settings[ 'default' ].notifications._value )
			);
			...
			assert.ok(
				notificationContainerElement.is(
					'.customize-control-notifications-container'
				)
			);
			assert.equal(
				0,
				notificationContainerElement.find( '> ul > li' ).length
			);
			assert.equal( 0, notificationContainerElement.height() );

			settingNotification = new wp.customize.Notification(
				'setting_invalidity',
				{ message: 'Invalid setting' }
			);
			controlOnlyNotification = new wp.customize.Notification(
				'control_invalidity',
				{ message: 'Invalid control' }
			);
			control.settings[ 'default' ].notifications.add(
				settingNotification.code,
				settingNotification
			);
			control.notifications.add(
				controlOnlyNotification.code,
				controlOnlyNotification
			);
			...
			assert.notEqual(
				'none',
				notificationContainerElement.css( 'display' )
			);
			...
			control.settings[ 'default' ].notifications.remove(
				settingNotification.code
			);
			control.notifications.render();
			assert.equal(
	...
	QUnit.test(
		'Fixture section has control among controls()',
		function ( assert ) {
			var section = wp.customize.section( 'fixture-section' );
			assert.ok(
				-1 !==
					_.pluck( section.controls(), 'id' ).indexOf(
						'fixture-control'
					)
			);
		}
	...
	QUnit.test( 'Fixture section exists', function ( assert ) {
		assert.ok(
			wp.customize.section.has( 'fixture-section-default-templated' )
		);
	} );
	QUnit.test( 'Fixture section has expected content', function ( assert ) {
		var id = 'fixture-section-default-templated',
			section;
		...
		assert.ok(
			section.headContainer.is(
				'.control-section.control-section-default'
			)
		);
		assert.ok(
			1 ===
				section.headContainer.find( '> .accordion-section-title' )
					.length
		);
		assert.ok(
			section.contentContainer.is( '.accordion-section-content' )
		);
		assert.equal(
			section.headContainer.attr( 'aria-owns' ),
			section.contentContainer.attr( 'id' )
		);
	} );

	QUnit.module(
		'Customizer Custom Type (titleless) Section with Template in Fixture'
	);
	QUnit.test( 'Fixture section exists', function ( assert ) {
		assert.ok(
			wp.customize.section.has( 'fixture-section-titleless-templated' )
		);
	} );
	QUnit.test( 'Fixture section has expected content', function ( assert ) {
		var id = 'fixture-section-titleless-templated',
			section;
		section = wp.customize.section( id );
		assert.ok( ! section.params.content );
		assert.ok( !! section.container );
		...
		assert.ok( section.container.has( section.contentContainer ) );
		assert.ok(
			section.container.is( '.control-section.control-section-titleless' )
		);
		assert.ok(
			0 ===
				section.headContainer.find( '> .accordion-section-title' )
	...
	QUnit.test( 'Fixture section has expected content', function ( assert ) {
		var id = 'fixture-section-reusing-default-template',
			section;
		section = wp.customize.section( id );
		assert.ok( ! section.params.content );
		assert.ok( !! section.container );
		...
		assert.ok(
			section.headContainer.is(
				'.control-section.control-section-' + section.params.type
			)
		);
		assert.ok(
			1 ===
	...
	QUnit.test(
		'Fixture section has default params supplied',
		function ( assert ) {
			var id = 'fixture-section-without-params',
				section,
				defaultParams;
			section = wp.customize.section( id );
			defaultParams = {
	...
	QUnit.test(
		'Fixture panel has section among its sections()',
		function ( assert ) {
			var panel = wp.customize.panel( 'fixture-panel' );
			assert.ok(
				-1 !==
					_.pluck( panel.sections(), 'id' ).indexOf(
						'fixture-section'
					)
			);
		}
	...
	QUnit.test(
		'Focusing on a control will expand the panel and section',
		function ( assert ) {
	...
	QUnit.test( 'Fixture section exists', function ( assert ) {
		assert.ok(
			wp.customize.panel.has( 'fixture-panel-default-templated' )
		);
	} );
	QUnit.test( 'Fixture panel has expected content', function ( assert ) {
		var id = 'fixture-panel-default-templated',
			panel;
		...
		assert.ok( panel.container.has( panel.contentContainer ) );
		assert.ok(
			panel.headContainer.is( '.control-panel.control-panel-default' )
		);
		assert.ok(
			1 ===
				panel.headContainer.find( '> .accordion-section-title' ).length
		);
		assert.ok( panel.contentContainer.is( '.control-panel-content' ) );
		assert.equal(
			panel.headContainer.attr( 'aria-owns' ),
			panel.contentContainer.attr( 'id' )
		);
	} );

	QUnit.module(
		'Customizer Custom Type Panel (titleless) with Template in Fixture'
	);
	QUnit.test( 'Fixture panel exists', function ( assert ) {
		assert.ok(
			wp.customize.panel.has( 'fixture-panel-titleless-templated' )
		);
	} );
	QUnit.test( 'Fixture panel has expected content', function ( assert ) {
		var id = 'fixture-panel-titleless-templated',
			panel;
		panel = wp.customize.panel( id );
		assert.ok( ! panel.params.content );
		assert.ok( !! panel.container );
		...
		assert.ok( panel.container.has( panel.contentContainer ) );
		assert.ok(
			panel.headContainer.is( '.control-panel.control-panel-titleless' )
		);
		assert.ok(
			0 ===
				panel.headContainer.find( '> .accordion-section-title' ).length
	...
	QUnit.test( 'Fixture panel has expected content', function ( assert ) {
		var id = 'fixture-panel-reusing-default-template',
			panel;
		panel = wp.customize.panel( id );
		assert.ok( ! panel.params.content );
		assert.ok( !! panel.container );
		...
		assert.ok( panel.container.has( panel.contentContainer ) );
		assert.ok(
			panel.headContainer.is(
				'.control-panel.control-panel-' + panel.params.type
			)
		);
		assert.ok(
			1 ===
	...
	QUnit.test(
		'Fixture panel has default params supplied',
		function ( assert ) {
			var id = 'fixture-panel-without-params',
				panel,
				defaultParams;
			panel = wp.customize.panel( id );
			defaultParams = {
	...
	QUnit.test( 'Create a new setting', function ( assert ) {
		mockSetting = wp.customize.create( settingId, settingId, settingValue, {
			transport: 'refresh',
	...
	mockSection = new wp.customize.Section( sectionId, {
		params: sectionData,
	} );
	...
	QUnit.test(
		'Section instance added to the wp.customize.section object',
		function ( assert ) {
	...
	QUnit.test(
		'Section instance has right content when accessed from wp.customize.section()',
		function ( assert ) {
	...
	controlType = 'text';
	controlContent =
		'<li id="customize-control-blogname" class="customize-control customize-control-text"></li>';
	...
	QUnit.test(
		'Control instance does not yet belong to a section.',
		function ( assert ) {
	...
	QUnit.test(
		'Control instance has the right selector.',
		function ( assert ) {
			assert.equal(
				mockControl.selector,
				'#customize-control-new_blogname'
			);
		}
	);
	...
	QUnit.test(
		'Control instance was added to the control class.',
		function ( assert ) {
	...
	QUnit.test(
		'Control instance has the right id when accessed from api.control().',
		function ( assert ) {
	...
	QUnit.test(
		'Associating a control with a section allows it to be embedded',
		function ( assert ) {
	...
	QUnit.test(
		'Control is now available on section.controls()',
		function ( assert ) {
	...
	QUnit.test(
		'Panel instance is not contextuallyActive',
		function ( assert ) {
	...
	QUnit.test( 'findControlsForSettings(blogname)', function ( assert ) {
		var controlsForSettings,
			settingId = 'fixture-setting',
			controlId = 'fixture-control';
		assert.ok( wp.customize.control.has( controlId ) );
		assert.ok( wp.customize.has( settingId ) );
		controlsForSettings = wp.customize.findControlsForSettings( [
			settingId,
		] );
		assert.ok( _.isObject( controlsForSettings ), 'Response is object' );
		assert.ok(
			_.isArray( controlsForSettings[ 'fixture-setting' ] ),
			'Response has a fixture-setting array'
		);
		assert.equal( 1, controlsForSettings[ 'fixture-setting' ].length );
		assert.equal(
			wp.customize.control( controlId ),
			controlsForSettings[ 'fixture-setting' ][ 0 ]
		);
	} );
	...
	QUnit.test( 'dirtyValues() returns expected values', function ( assert ) {
		...
		assert.ok(
			_.isEmpty( wp.customize.dirtyValues( { unsaved: false } ) )
		);
		...
		assert.ok( ! _.isEmpty( wp.customize.dirtyValues() ) );
		assert.ok(
			! _.isEmpty( wp.customize.dirtyValues( { unsaved: true } ) )
		);
		assert.equal(
			'Modified',
			wp.customize.dirtyValues()[ 'fixture-setting' ]
		);
	...
	QUnit.test(
		'requestChangesetUpdate makes request and returns promise',
		function ( assert ) {
			var request,
				originalBeforeSetup = jQuery.ajaxSettings.beforeSend;

			jQuery.ajaxSetup( {
				beforeSend: function ( e, data ) {
					var queryParams, changesetData;
					queryParams = wp.customize.utils.parseQueryString(
						data.data
					);

					assert.equal( 'customize_save', queryParams.action );
					assert.ok(
						! _.isUndefined( queryParams.customize_changeset_data )
					);
					...
					changesetData = JSON.parse(
						queryParams.customize_changeset_data
					);
					assert.ok(
						! _.isUndefined( changesetData.additionalSetting )
					);
					assert.ok(
						! _.isUndefined( changesetData[ 'fixture-setting' ] )
					);

					assert.equal(
						'additionalValue',
						changesetData.additionalSetting.value
					);
					assert.equal(
						'requestChangesetUpdate',
						changesetData[ 'fixture-setting' ].value
					);
	...
	QUnit.test(
		'utils.getRemainingTime calculates time correctly',
		function ( assert ) {
			var datetime = '2599-08-06 12:12:13',
				timeRemaining,
				timeRemainingWithDateInstance,
				timeRemainingWithTimestamp;

			timeRemaining = wp.customize.utils.getRemainingTime( datetime );
			timeRemainingWithDateInstance = wp.customize.utils.getRemainingTime(
				new Date( datetime.replace( /-/g, '/' ) )
			);
			timeRemainingWithTimestamp = wp.customize.utils.getRemainingTime(
				new Date( datetime.replace( /-/g, '/' ) ).getTime()
			);

			assert.equal( typeof timeRemaining, 'number', timeRemaining );
			assert.equal(
				typeof timeRemainingWithDateInstance,
				'number',
				timeRemaining
			);
			assert.equal(
				typeof timeRemainingWithTimestamp,
				'number',
				timeRemaining
			);
	...
	QUnit.test(
		'utils.getCurrentTimestamp returns timestamp',
		function ( assert ) {
	...
	QUnit.test(
		'Test DateTimeControl creation and its methods',
		function ( assert ) {
			var control,
				controlId = 'date_time',
				section,
				sectionId = 'fixture-section',
				datetime = '2599-08-06 18:12:13',
				dateTimeArray,
				dateTimeArrayInampm,
				timeString,
				day,
			...
			assert.ok(
				...
				'#customize-control-date_time-content'
			);
			assert.ok( control.section(), sectionId );
			assert.equal(
				_.size( control.inputElements ),
				control.elements.length
			);
			...
			assert.equal(
				day(),
				20,
				'Should not update if its less the correct number of days'
			);

			// Test control.convertHourToTwentyFourHourFormat().
			assert.equal(
				control.convertHourToTwentyFourHourFormat( 11, 'pm' ),
				23
			);
			assert.equal(
				control.convertHourToTwentyFourHourFormat( 12, 'pm' ),
				12
			);
			assert.equal(
				control.convertHourToTwentyFourHourFormat( 12, 'am' ),
				0
			);
			assert.equal(
				control.convertHourToTwentyFourHourFormat( 11, 'am' ),
				11
			);
	...
	QUnit.test( 'Test OuterSection', function ( assert ) {
		var section,
			sectionId = 'test_outer_section',
			body = jQuery( 'body' ),
			defaultSection,
			defaultSectionId = 'fixture-section';
		...
		assert.equal(
			section.containerParent.selector,
	...
	QUnit.test(
		'Test PreviewLinkControl creation and its methods',
		function ( assert ) {
			var section,
				sectionId = 'publish_settings',
				newLink;
			...
			section.deferred.embedded.done( function () {
				_.each( section.controls(), function ( control ) {
					if ( 'changeset_preview_link' === control.id ) {
						assert.equal(
							control.templateSelector,
							'customize-preview-link-control'
						);
						assert.equal(
							_.size( control.previewElements ),
							control.elements.length
						);
						...
						assert.ok(
							control.notifications.has( 'changes_not_saved' )
						);
						...
						assert.equal(
							control.setting.get(),
							wp.customize.previewer.getFrontendPreviewUrl()
						);
					}
				} );
			} );
