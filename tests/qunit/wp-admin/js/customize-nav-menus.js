...
jQuery( 'body' ).on( 'load', function () {
	 ...
	 */
	QUnit.test(
		'generatePlaceholderAutoIncrementId generates unique IDs',
		function ( assert ) {
	...
	QUnit.test(
		'it should parse _wpCustomizeMenusSettings.defaults into itself',
		function ( assert ) {
			assert.deepEqual(
	...
	QUnit.test(
		'empty menus should have no Menu Item Controls',
		function ( assert ) {
			assert.ok(
				0 ===
					wp.customize.Menus.getMenuControl(
						socialMenuId
					).getMenuItemControls().length,
				'empty menus'
			);
		}
	);
	...
	QUnit.test(
		'populated menus should have no Menu Item Controls',
		function ( assert ) {
			assert.ok(
				0 !==
					wp.customize.Menus.getMenuControl(
	...
	QUnit.test(
		'there is a properly configured MenusPanel',
		function ( assert ) {
			var panel, sections;
			...
			assert.ok(
				'menu_locations' === sections[ 0 ].id,
				'first section is menu_locations'
			);
			assert.ok(
				sections[ 1 ].extended( api.Menus.MenuSection ),
				'second section is MenuSection'
			);
			assert.ok(
				sections[ sections.length - 1 ].extended(
					api.Menus.NewMenuSection
				),
				'last section is NewMenuSection'
			);
		}
	);
	...
	QUnit.test(
		'there an expected MenuSection for the primary menu',
		function ( assert ) {
			...
			assert.ok(
				section.extended( api.Menus.MenuSection ),
				'section is a api.Menus.MenuSection'
			);
			assert.ok(
				section.deferred.initSortables,
				'has section.deferred.initSortables'
			);
			assert.ok( section.active(), 'section active() is true' );
			assert.ok(
				section.active.set( false ).get(),
				'section active() cannot be set false'
			);

			controls = section.controls();
			assert.ok(
				controls[ 0 ].extended( api.Menus.MenuNameControl ),
				'first control in menu section is MenuNameControl'
			);
			assert.ok(
				controls[ 1 ].extended( api.Menus.MenuItemControl ),
				'second control in menu section is MenuItemControl'
			);

			lastControl = controls[ controls.length - 1 ];
			assert.ok(
				lastControl.extended( api.Control ),
				'last control in menu section is a base Control'
			);
			assert.ok(
				lastControl.params.templateId === 'nav-menu-delete-button',
				'last control in menu section has a delete-button template'
			);
		}
	);
	...
	QUnit.test(
		'changing a MenuNameControl change the corresponding menu value',
		function ( assert ) {
			var section, control;
			...
			assert.ok(
				control.extended( api.Menus.MenuNameControl ),
				'control is a MenuNameControl'
			);
	...
	QUnit.test( 'manipulating a MenuItemControl works', function ( assert ) {
		...
		assert.ok(
			control.extended( api.Menus.MenuItemControl ),
			'control is a MenuItemControl'
		);
		...
		assert.equal(
			control.container.find( '.edit-menu-item-title' ).val(),
			'Hello World'
		);
	...
	QUnit.test(
		'api.Menus.getMenuControl() should return the expected control',
		function ( assert ) {
			var control = api.Menus.getMenuControl( primaryMenuId );
			assert.ok( !! control, 'control is returned' );
			assert.ok(
				control.extended( api.Menus.MenuControl ),
				'control is a MenuControl'
			);
		}
	);

	QUnit.test(
		'api.Menus.getMenuItemControl() should return the expected control',
		function ( assert ) {
