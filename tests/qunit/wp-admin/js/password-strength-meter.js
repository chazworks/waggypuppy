...
jQuery( function () {
	...
	QUnit.test( 'mismatched passwords should return 5', function ( assert ) {
		assert.equal(
			passwordStrength( 'password1', 'username', 'password2' ),
			5,
			'mismatched passwords return 5'
		);
	} );
	...
	QUnit.test(
		...
		function ( assert ) {
			assert.equal(
				passwordStrength( 'abc', 'username', 'abc' ),
				0,
				'short passwords return 0'
			);
		}
	);

	QUnit.test(
		'long complicated passwords should return 4',
		function ( assert ) {
			...
			assert.equal(
				passwordStrength( twofifty, 'username', twofifty ),
				4,
				'250 character complicated password returns 4'
			);
		}
	);

	QUnit.test(
		'short uncomplicated passwords should return 0',
		function ( assert ) {
			var letters = 'aaaa',
				numbers = '1111',
				password = 'password',
				uppercase = 'AAAA';
			assert.equal(
				passwordStrength( letters, 'username', letters ),
				0,
				'password of `' + letters + '` returns 0'
			);
			assert.equal(
				passwordStrength( numbers, 'username', numbers ),
				0,
				'password of `' + numbers + '` returns 0'
			);
			assert.equal(
				passwordStrength( uppercase, 'username', uppercase ),
				0,
				'password of `' + uppercase + '` returns 0'
			);
			assert.equal(
				passwordStrength( password, 'username', password ),
				0,
				'password of `' + password + '` returns 0'
			);
		}
	);

	QUnit.test(
		'zxcvbn password tests should return the score we expect',
		function ( assert ) {
			var passwords, i;
			passwords = [
				{ pw: 'zxcvbn', score: 0 },
	...
	QUnit.test(
		'disallowed words in password should be penalized',
		function ( assert ) {
			var allowedPasswordScore,
				penalizedPasswordScore,
				allowedPassword = 'a[janedoefoe]4',
				penalizedPassword = 'a[johndoefoe]4',
				disallowedList = [ 'extra', 'johndoefoe', 'superfluous' ];

			allowedPasswordScore = passwordStrength(
				allowedPassword,
				disallowedList,
				allowedPassword
			);
			penalizedPasswordScore = passwordStrength(
				penalizedPassword,
				disallowedList,
				penalizedPassword
			);
			...
			assert.ok(
				penalizedPasswordScore < allowedPasswordScore,
				'Penalized password scored ' +
					penalizedPasswordScore +
					'; allowed password scored: ' +
					allowedPasswordScore
			);
		}
	);

	QUnit.test(
		'user input disallowed list array should contain expected words',
		function ( assert ) {
			var disallowedList = wp.passwordStrength.userInputDisallowedList();

			assert.ok(
				Array.isArray( disallowedList ),
				'disallowed list is an array'
			);
			assert.ok(
				jQuery.inArray( 'waggypuppy', disallowedList ) > -1,
				'disallowed list contains "waggypuppy" from page title'
			);
			assert.ok(
				jQuery.inArray( 'tests', disallowedList ) > -1,
				'disallowed list contains "tests" from site URL'
			);
		}
	);
} );
