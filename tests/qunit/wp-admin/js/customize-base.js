...
jQuery( function ( $ ) {
	...
	QUnit.test(
		'FooSuperClass prototype does not have staticProp',
		function ( assert ) {
			assert.equal(
				typeof FooSuperClass.prototype.staticProp,
				'undefined'
			);
		}
	);
	...
	QUnit.test(
		'FooSuperClass instance foo extended Class',
		function ( assert ) {
	...
	QUnit.test(
		'FooSuperClass instance foo ran initialize() and has supplied instanceProp',
		function ( assert ) {
	...
	QUnit.test(
		'BarSubClass prototype has parent FooSuperClass protoProp',
		function ( assert ) {
	...
	QUnit.test(
		'BarSubClass instance bar its initialize() and parent initialize() run',
		function ( assert ) {
	...
	QUnit.test(
		'BarSubClass instance bar extended FooSuperClass',
		function ( assert ) {
	...
	QUnit.test( 'New constructor added to class', function ( assert ) {
		assert.equal(
			ConstructorTestClass.prototype.constructor,
			newConstructor
		);
	} );
	QUnit.test(
		'Class with new constructor has protoPropValue',
		function ( assert ) {
			assert.equal(
				ConstructorTestClass.prototype.protoProp,
				'protoPropValue'
			);
		}
	);

	constructorTest = new ConstructorTestClass( {
		instanceProp: 'instancePropValue',
	} );
	QUnit.test(
		'ConstructorTestClass instance constructorTest has the new constructor',
		function ( assert ) {
	...
	QUnit.test(
		'ConstructorTestClass instance constructorTest extended Class',
		function ( assert ) {
			assert.equal(
				constructorTest.extended( wp.customize.Class ),
				true
			);
		}
	);
	...
	QUnit.test(
		'ConstructorTestClass instance constructorTest has the added instance property',
		function ( assert ) {
	...
	QUnit.test(
		'.bind() adds new callback that fires on set()',
		function ( assert ) {
	...
	QUnit.test(
		'Correct events are triggered when adding to or removing from Values collection',
		function ( assert ) {
	...
	QUnit.test(
		'Notification object exists and has expected properties',
		function ( assert ) {
	...
	QUnit.test(
		'wp.customize.utils.parseQueryString works',
		function ( assert ) {
			var queryParams;
			...
			queryParams = wp.customize.utils.parseQueryString(
				'a+b=1&b=Hello%20World'
			);
			assert.ok(
				_.isEqual( queryParams, { a_b: '1', b: 'Hello World' } )
			);

			queryParams = wp.customize.utils.parseQueryString(
				'a%20b=1&b=Hello+World'
			);
			assert.ok(
				_.isEqual( queryParams, { a_b: '1', b: 'Hello World' } )
			);
