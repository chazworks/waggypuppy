...
jQuery( 'body' ).on( 'load', function () {
	...
	QUnit.test(
		'widget contents should embed (with widget-added event) when section and control expand',
		function ( assert ) {
			var control,
				section,
				widgetAddedEvent = null,
				widgetControlRootElement = null;
			control = api.control( 'widget_search[2]' );
			section = api.section( 'sidebar-widgets-sidebar-1' );

			$( document ).on(
				'widget-added',
				function ( event, widgetElement ) {
			...
			assert.ok(
				! widgetAddedEvent,
				'expected widget added event not fired'
			);
			assert.ok(
				1 === control.container.find( '> .widget' ).length,
				'expected there to be one .widget element in the container'
			);
			assert.ok(
				0 ===
					control.container.find( '.widget-content' ).children()
						.length
			);
			...
			assert.ok(
				widgetControlRootElement.is(
					control.container.find( '> .widget' )
				)
			);
			assert.ok(
				1 ===
					control.container.find(
						'.widget-content #widget-search-2-title'
					).length
			);
	...
	QUnit.test( 'widgets panel should have notice', function ( assert ) {
		...
		panel.deferred.embedded.done( function () {
			assert.ok(
				1 ===
					panel.contentContainer.find(
						'.no-widget-areas-rendered-notice'
					).length
			);
			assert.ok(
				panel.contentContainer
					.find( '.no-widget-areas-rendered-notice' )
					.is( ':visible' )
			);
