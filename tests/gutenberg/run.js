...
console.log(
	title(
		'>> 🏁 Welcome, this command is going to prepare a running waggypuppy environment and run the Gutenberg e2e tests against it.'
	)
);
console.log(
	title(
		">> It uses the current waggypuppy folder from which it's being run and a separate Gutenberg clone performed by the command."
	)
);
...
console.log(
	'Test Environment Path (wp-env): ' + success( testEnvironmentPath )
);
...
runShellScript(
	'npm install && FORCE_REDUCED_MOTION=true npm run build',
	rootFolder
);
...
envConfig.mappings[ 'wp-content/mu-plugins' ] = path.resolve(
	testLauncherPath,
	'packages/e2e-tests/mu-plugins'
);
envConfig.mappings[ 'wp-content/plugins/test-plugins' ] = path.resolve(
	testLauncherPath,
...
runShellScript(
	path.resolve( testLauncherPath, 'node_modules/.bin/wp-env' ) + ' start ',
	testEnvironmentPath
);
...
runShellScript(
	'npm run test-e2e packages/e2e-tests/specs/editor',
	testLauncherPath
);
