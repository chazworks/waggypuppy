...
test.describe( 'Cache Control header directives', () => {
	...
	test( 'No private directive present in cache control when user not logged in.', async ( {
		browser,
		admin,
		editor,
	} ) => {
		...
		expect( responseHeaders ).toEqual(
			expect.not.objectContaining( { 'cache-control': 'no-store' } )
		);
		expect( responseHeaders ).toEqual(
			expect.not.objectContaining( { 'cache-control': 'private' } )
		);
	} );

	test( 'Private directive header present in cache control when logged in.', async ( {
		page,
		admin,
	} ) => {
		await admin.visitAdminPage( '/' );
