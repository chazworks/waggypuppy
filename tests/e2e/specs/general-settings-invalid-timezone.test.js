...
test.describe( 'Settings -> General', () => {
	...
	for ( const invalidTimezone of invalidTimezones ) {
		test( `Does not allow saving an invalid timezone string with "${ invalidTimezone }"`, async ( {
			admin,
			page,
		} ) => {
			...
			await page
				...
				.evaluate(
					( timezone ) => ( timezone.value = 'Europe/Lisbon' )
				);
			...
			await page
				.locator( '#timezone_string' )
				.evaluate( ( timezone, invalidTimezone ) => {
			...
			await expect( page.locator( '#timezone_string' ) ).toHaveValue(
				'Europe/Lisbon'
			);
		} );
	}
