...
abstract class WP_HTTP_UnitTestCase extends WP_UnitTestCase
{
    ...
    {
        // 5 : 5 & 301.
        $res = wp_remote_request($this->redirection_script . '?code=301&rt=' . 5, ['redirection' => 5]);
    ...
    {
        // 5 : 5 & 302.
        $res = wp_remote_request($this->redirection_script . '?code=302&rt=' . 5, ['redirection' => 5]);
    ...
    {
        // 5 > 0 & 301.
        $res = wp_remote_request($this->redirection_script . '?code=301&rt=' . 5, ['redirection' => 0]);
    ...
    {
        // 5 > 0 & 302.
        $res = wp_remote_request($this->redirection_script . '?code=302&rt=' . 5, ['redirection' => 0]);
    ...
    {
        // 5 - 5.
        $res = wp_remote_request($this->redirection_script . '?rt=' . 5, ['redirection' => 5]);
    ...
    public function test_no_head_redirections()
    {
        ...
        $res = wp_remote_request($this->redirection_script . '?code=302&rt=' . 1, ['method' => 'HEAD']);
    ...
    {
        // 10 > 5.
        $res = wp_remote_request($this->redirection_script . '?rt=' . 10, ['redirection' => 5]);
    ...
    {
        // 6 > 5 (close edge case).
        $res = wp_remote_request($this->redirection_script . '?rt=' . 6, ['redirection' => 5]);
    ...
    {
        // 4 < 5 (close edge case).
        $res = wp_remote_request($this->redirection_script . '?rt=' . 4, ['redirection' => 5]);
    ...
    public function test_send_headers()
    {
        ...
        $res = wp_remote_request($this->redirection_script . '?header-check', ['headers' => $headers]);
    ...
    public function test_file_stream()
    {
        ...
        $this->assertEquals($size,
            $res['headers']['Content-Length']);   // Check the headers are returned (and the size is the same).
        $this->assertSame($size,
            $filesize); // Check that the file is written to disk correctly without any extra characters.
    ...
    public function test_ssl()
    {
        ...
        $res = wp_remote_get('https://wp.org/');
