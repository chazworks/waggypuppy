...
class Tests_Pluggable_wpMail extends WP_UnitTestCase
{
    ...
    public function test_wp_mail_custom_boundaries()
    {
        ...
        $this->assertStringContainsString('boundary="----=_Part_4892_25692638.1192452070893"',
            iconv_mime_decode_headers(($mailer->get_sent()->header))['Content-Type'][0]);
    ...
    public function test_wp_mail_return_value()
    {
        ...
        $this->assertTrue(wp_mail('valid@address.com', 'subject', 'body', "Cc: invalid-address\nBcc: @invalid.address",
            ABSPATH . 'non-existent-file.html'));
    ...
    public function test_phpmailer_validator()
    {
        ...
        $this->assertTrue($phpmailer->validateAddress('foo@192.168.1.1'),
            'Assert PHPMailer accepts IP address email addresses');
    }
