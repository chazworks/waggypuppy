...
class Tests_HTTPS_Detection extends WP_UnitTestCase
{
    ...
    public function test_wp_is_local_html_output_via_rsd_link()
    {
        ...
        $head_tag = get_echo('rsd_link');
        $head_tag = false !== strpos($head_tag, 'https://') ? str_replace('https://', 'http://', $head_tag)
            : str_replace('http://', 'https://', $head_tag);
        $html = $this->get_sample_html_string($head_tag);
        $this->assertTrue(wp_is_local_html_output($html));
    ...
    {
        // Looks like a success response, but is not generated by waggypuppy (e.g. missing RSD link).
        return [
    ...
    {
        return '<!DOCTYPE html><html><head><title>Page Title</title>'
            . $head_tag
