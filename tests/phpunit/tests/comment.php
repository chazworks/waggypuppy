...
class Tests_Comment extends WP_UnitTestCase
{
    ...
    public function test_update_comment_from_unprivileged_user_by_privileged_user()
    {
        ...
        $comment = get_comment($comment_id);
        $this->assertSame('<a href="http://example.localhost/something.html" rel="nofollow ugc">click</a>',
            $comment->comment_content, 'Comment: ' . $comment->comment_content);
    }
    ...
    public function test_should_allow_reply_to_an_approved_comment($comment_post)
    {
        ...
        $expected = "<input type='hidden' name='comment_post_ID' value='"
            . self::$post_id
            . "' id='comment_post_ID' />\n";
        $expected .= "<input type='hidden' name='comment_parent' id='comment_parent' value='"
            . $_GET['replytocom']
            . "' />\n";
    ...
    public function test_wp_new_comment_should_trim_provided_comment_agent_to_254_chars()
    {
        ...
        $this->assertSame('Mozilla/5.0 (iPhone; CPU iPhone OS 7_0 like Mac OS X; en-us) AppleWebKit/537.51.1 (KHTML, like Gecko) Version/7.0 Mobile/11A465 Safari/9537.53 Opera/9.80 (X11; Linux i686; Ubuntu/14.10) Presto/2.12.388 Version/12.16 Mozilla/5.0 (Macintosh; U; PPC Mac OS ',
    ...
    public function try_sending_author_notification($comment, $post)
    {
        ...
        // Check to see if a notification email was sent to the post author `test@test.com`.
        if (isset($GLOBALS['phpmailer']->mock_sent) && !empty($GLOBALS['phpmailer']->mock_sent)
            && 'test@test.com'
