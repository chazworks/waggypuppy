...
class Test_oEmbed_Controller extends WP_UnitTestCase
{
    ...
    public function mock_embed_request($response, $parsed_args, $url)
    {
            ...
            return [
                ...
                'body' => wp_json_encode(
                    [
                        ...
                        'html' => '<b>Unfiltered</b><iframe width="'
                            . $query_params['maxwidth']
                            . '" height="'
                            . $query_params['maxheight']
        ...
        if (self::UNTRUSTED_PROVIDER_URL === $url) {
            return [
                ...
                'body' => '<html><head><link rel="alternate" title="oEmbed (JSON)" type="application/json+oembed" href="'
                    . self::UNTRUSTED_PROVIDER_URL
                    . '" /></head><body></body></html>',
            ];
        }
    ...
    public function test_get_oembed_endpoint_url()
    {
        ...
        $this->assertSame(home_url() . '/index.php?rest_route=%2Foembed%2F1.0%2Fembed&url=' . $url_encoded,
            get_oembed_endpoint_url($url));
        $this->assertSame(home_url()
            . '/index.php?rest_route=%2Foembed%2F1.0%2Fembed&url='
            . $url_encoded
            . '&format=xml', get_oembed_endpoint_url($url, 'xml'));
    }
    ...
    public function test_get_oembed_endpoint_url_pretty_permalinks()
    {
        ...
        $this->assertSame(home_url() . '/wp-json/oembed/1.0/embed?url=' . $url_encoded . '&format=xml',
            get_oembed_endpoint_url($url, 'xml'));
