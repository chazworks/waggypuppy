...
class Tests_Menu_Walker_Nav_Menu extends WP_UnitTestCase
{
    ...
    public function test_start_el_with_empty_attributes($value, $expected)
    {
        ...
        $this->assertSame("<li id=\"menu-item-{$post_id}\" class=\"menu-item-{$post_id}\"><a{$expected}>{$post_title}</a>",
            $output);
    }
     ...
     */
    public function test_walker_nav_menu_start_el_should_add_rel_privacy_policy_to_privacy_policy_url(
        $expected,
        $xfn = '',
        $target = '',
    ) {
