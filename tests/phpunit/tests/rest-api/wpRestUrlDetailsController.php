...
class Tests_REST_WpRestUrlDetailsController extends WP_Test_REST_Controller_Testcase
{
    ...
    public function test_get_items_fails_for_unauthenticated_user()
    {
        ...
        $this->assertSame(WP_Http::UNAUTHORIZED, $response->get_status(),
            'Response status is not ' . WP_Http::UNAUTHORIZED);

        $this->assertSame('rest_cannot_view_url_details', $data['code'],
            'Response "code" is not "rest_cannot_view_url_details"');

        $expected = 'you are not allowed to process remote urls';
        $this->assertStringContainsString($expected, strtolower($data['message']),
            'Response "message" does not contain  "' . $expected . '"');
    }
    ...
    public function test_get_items_fails_for_invalid_url($invalid_url)
    {
        ...
        $this->assertSame(WP_Http::BAD_REQUEST, $response->get_status(),
            'Response status is not ' . WP_Http::BAD_REQUEST);
    ...
    public function test_get_items_fails_for_url_which_returns_a_non_200_status_code()
    {
        ...
        $this->assertStringContainsString('not found', strtolower($data['message']),
            'Response "message" does not contain "not found"');
    }
    ...
    public function test_can_filter_http_request_args_via_filter()
    {
        ...
        $this->assertArrayHasKey('limit_response_size', $this->request_args,
            'Request args do not contain a "limit_response_size" key');
        ...
        $this->assertSame(153600, $this->request_args['limit_response_size'],
            'Request args "limit_response_size" is not 153600');
        $this->assertSame(static::URL_PLACEHOLDER, $this->request_args['body'],
            'Request args "body" is not "' . static::URL_PLACEHOLDER . '"');
    }
