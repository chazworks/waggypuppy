...
class Tests_Dependencies_Scripts extends WP_UnitTestCase
{
    ...
    public function test_after_inline_script_with_delayed_main_script($strategy)
    {
        ...
        $this->assertEqualMarkup($expected, $output,
            'Inline scripts in the "after" position, that are attached to a deferred main script, are failing to print/execute.');
    }
    ...
    public function test_after_inline_script_with_blocking_main_script()
    {
        ...
        $this->assertEqualMarkup($expected, $output,
            'Inline scripts in the "after" position, that are attached to a blocking main script, are failing to print/execute.');
    }
    ...
    public function test_before_inline_scripts_with_delayed_main_script($strategy)
    {
        ...
        wp_enqueue_script('ms-i1-1', 'http://example.org/ms-i1-1.js', ['ds-i1-1', 'ds-i1-2', 'ds-i1-3'], null,
            compact('strategy'));
        ...
        $this->assertEqualMarkup($expected, $output,
            'Inline scripts in the "before" position, that are attached to a deferred main script, are failing to print/execute.');
    }
    ...
    public function test_loading_strategy_with_valid_async_registration()
    {
        ...
        $this->assertEqualMarkup($expected, $output,
            'Scripts enqueued with an async loading strategy are failing to have the async attribute applied to the script handle when being printed.');
    }
    ...
    public function test_delayed_dependent_with_blocking_dependency($strategy)
    {
        ...
        $this->assertEqualMarkup($expected, $output,
            'Dependents of a blocking dependency are free to have any strategy.');
    }
    ...
    public function test_blocking_dependent_with_delayed_dependency($strategy)
    {
        ...
        $expected = <<<JS
            ...
            JS;
        $this->assertEqualMarkup($expected, $output,
            'Blocking dependents must force delayed dependencies to become blocking.');
    }
    ...
    public function test_delayed_dependent_with_blocking_dependency_not_enqueued($strategy)
    {
        ...
        $expected = str_replace("'", '"',
            "<script src='/main-script-a4.js' id='main-script-a4-js' {$strategy} data-wp-strategy='{$strategy}'></script>");
        $this->assertStringContainsString($expected, $output,
            'Only enqueued dependents should affect the eligible strategy.');
    }
    ...
    public function test_filter_eligible_strategies($set_up, $expected)
    {
        ...
        $this->assertSame($expected, $filter_eligible_strategies->invokeArgs(wp_scripts(), [$handle]),
            'Expected return value of WP_Scripts::filter_eligible_strategies to match.');
    }
    ...
    protected function add_test_inline_script($handle, $position)
    {
        wp_add_inline_script($handle,
            sprintf('scriptEventLog.push( %s )', wp_json_encode("{$handle}: {$position} inline")), $position);
    }
        ...
        return [
            ...
            'jquery-deferred' => [
                'set_up' => function () {
                    ...
                    wp_enqueue_script('theme-functions', 'https://example.com/theme-functions.js', ['jquery'], null,
                        ['strategy' => 'defer']);
                },
                'expected_markup' => <<<HTML
            ...
            'nested-aliases' => [
                'set_up' => function () {
                    ...
                    wp_register_script($outer_alias_handle, false, [$inner_alias_handle, 'outer-bundle-leaf-member'],
                        null);
                    $this->register_test_script('outer-bundle-leaf-member', 'blocking', []);

                    // Inner alias only contains delay scripts.
                    wp_register_script($inner_alias_handle, false,
                        ['inner-bundle-member-one', 'inner-bundle-member-two'], null);
    ...
    public function test_loading_strategy_with_defer_having_no_dependents_nor_dependencies()
    {
        ...
        $output = get_echo('wp_print_scripts');
        $expected = str_replace("'", '"',
            "<script src='http://example.com/main-script-d1.js' id='main-script-d1-js' defer data-wp-strategy='defer'></script>\n");
    ...
    {
        $this->add_html5_script_theme_support();
        wp_enqueue_script('dependency-script-d2-1', 'http://example.com/dependency-script-d2-1.js', [], null,
            ['strategy' => 'defer']);
        wp_enqueue_script('dependency-script-d2-2', 'http://example.com/dependency-script-d2-2.js', [], null);
        wp_enqueue_script('dependency-script-d2-3', 'http://example.com/dependency-script-d2-3.js',
            ['dependency-script-d2-2'], null, ['strategy' => 'defer']);
        wp_enqueue_script('main-script-d2', 'http://example.com/main-script-d2.js',
            ['dependency-script-d2-1', 'dependency-script-d2-3'], null, ['strategy' => 'defer']);
        $output = get_echo('wp_print_scripts');
        $expected = '<script src="http://example.com/main-script-d2.js" id="main-script-d2-js" defer data-wp-strategy="defer"></script>';
        $this->assertStringContainsString($expected, $output,
            'Expected defer, as all dependencies are either deferred or blocking');
    }
    ...
    public function test_loading_strategy_with_all_defer_dependencies()
    {
        ...
        wp_enqueue_script('dependent-script-d3-1', 'http://example.com/dependent-script-d3-1.js', ['main-script-d3'],
            null, ['strategy' => 'defer']);
        wp_enqueue_script('dependent-script-d3-2', 'http://example.com/dependent-script-d3-2.js',
            ['dependent-script-d3-1'], null, ['strategy' => 'defer']);
        wp_enqueue_script('dependent-script-d3-3', 'http://example.com/dependent-script-d3-3.js',
            ['dependent-script-d3-2'], null, ['strategy' => 'defer']);
        $output = get_echo('wp_print_scripts');
        $expected = '<script src="http://example.com/main-script-d3.js" id="main-script-d3-js" defer data-wp-strategy="defer"></script>';
        $this->assertStringContainsString($expected, $output,
            'Expected defer, as all dependents have defer loading strategy');
    }
    ...
    public function test_defer_with_async_dependent()
    {
        ...
        wp_enqueue_script('dependent-script-d4-1', '/dependent-script-d4-1.js', ['main-script-d4'], null,
            ['strategy' => 'defer']);
        wp_enqueue_script('dependent-script-d4-2', '/dependent-script-d4-2.js', ['dependent-script-d4-1'], null,
            ['strategy' => 'async']);
        wp_enqueue_script('dependent-script-d4-3', '/dependent-script-d4-3.js', ['dependent-script-d4-2'], null,
            ['strategy' => 'defer']);
        $output = get_echo('wp_print_scripts');
        ...
        $this->assertEqualMarkup($expected, $output,
            'Scripts registered as defer but that have dependents that are async are expected to have said dependents deferred.');
    }
    ...
    public function test_loading_strategy_with_invalid_defer_registration()
    {
        ...
        $output = get_echo('wp_print_scripts');
        $expected = str_replace("'", '"',
            "<script type='text/javascript' src='/main-script-d4.js' id='main-script-d4-js' data-wp-strategy='defer'></script>\n");
        $this->assertStringContainsString($expected, $output,
            'Scripts registered as defer but that have all dependents with no strategy, should become blocking (no strategy).');
    }
    ...
    public function test_loading_strategy_with_valid_blocking_registration()
    {
        ...
        $this->assertSame($expected, $output,
            'Scripts registered with a "blocking" strategy, and who have no dependencies, should have no loading strategy attributes printed.');
        ...
        $expected = str_replace("'", '"', $expected);
        $this->assertSame($expected, $output,
            'Scripts registered with no strategy assigned, and who have no dependencies, should have no loading strategy attributes printed.');
    }
    ...
    public function test_scripts_targeting_head()
    {
        ...
        wp_enqueue_script('enqueue-header-new', '/enqueue-header-new.js', ['enqueue-header-old'], null,
            ['in_footer' => false]);
        ...
        $this->assertEqualMarkup($expected_header, $actual_header,
            'Scripts registered/enqueued using the older $in_footer parameter or the newer $args parameter should have the same outcome.');
    ...
    public function test_scripts_targeting_footer()
    {
        ...
        wp_enqueue_script('enqueue-footer-new', '/enqueue-footer-new.js', ['enqueue-footer-old'], null,
            ['in_footer' => true]);
    ...
    public function test_concatenate_with_defer_strategy()
    {
        ...
        wp_enqueue_script('main-defer-script', '/main-script.js',
            ['one-concat-dep', 'two-concat-dep', 'three-concat-dep'], null, ['strategy' => 'defer']);
        ...
        $this->assertEqualMarkup($expected, $print_scripts,
            'Scripts are being incorrectly concatenated when a main script is registered with a "defer" loading strategy. Deferred scripts should not be part of the script concat loading query.');
    }
    ...
    public function test_concatenate_with_async_strategy()
    {
        ...
        $this->assertEqualMarkup($expected, $print_scripts,
            'Scripts are being incorrectly concatenated when a main script is registered with an "async" loading strategy. Async scripts should not be part of the script concat loading query.');
    }
    ...
    public function test_concatenate_with_blocking_script_before_and_after_script_with_defer_strategy()
    {
        ...
        $this->assertEqualMarkup($expected, $print_scripts,
            'Scripts are being incorrectly concatenated when a main script is registered as deferred after other blocking scripts are registered. Deferred scripts should not be part of the script concat loader query string. ');
    }
    ...
    )
    {
        ...
        wp_register_script('parent-footer', '/parent-footer.js',
            ['child-head', 'child-footer', 'child2-head', 'child2-footer'], null, true); // In footer.
    ...
    public function test_wp_add_inline_script_customize_dependency()
    {
        ...
        $tail = substr($print_scripts, strrpos($print_scripts,
            '<script type="text/javascript" src="/customize-dependency.js" id="customize-dependency-js">'));
    ...
    public function test_get_inline_script($position, $inline_scripts, $delayed, $expected_data, $expected_tag)
    {
        ...
        $deps = [];
        if ($delayed) {
            $wp_scripts->add('dep', 'https://example.com/dependency.js', [],
                false); // TODO: Cannot pass strategy to $args e.g. array( 'strategy' => 'defer' )
    ...
    public function test_wp_set_script_translations_for_plugin()
    {
        ...
        $expected .= str_replace(
            ...
            [
                ...
                'plugin-example',
                file_get_contents(DIR_TESTDATA
     ...
     */
    public function test_wp_scripts_move_to_footer(
        $set_up,
        $expected_header,
        $expected_footer,
        $expected_in_footer,
        ...
        $this->assertEqualSets($expected_in_footer, wp_scripts()->in_footer,
            'Expected to have the same handles for in_footer.');
        ...
        return [
            'footer-blocking-dependent-of-defer-head-script' => [
                'set_up' => static function () {
                    ...
                    wp_enqueue_script('script-b', 'https://example.com/script-b.js', ['script-a'], null,
                        ['in_footer' => true]);
                },
                'expected_header' => '',
                'expected_footer' => '
    ...
    public function test_vendor_script_versions_registered_manually($script, $handle = null)
    {
        ...
        $this->assertArrayHasKey($script, $package_json,
            "The dependency '{$script}' should be included in package.json.");
        $this->assertSame($package_json[$script], $wp_scripts->query($handle, 'registered')->ver,
            "The script '{$handle}' should be registered with version {$package_json[ $script ]}.");
    }
