...
wait_on( { resources: [ `tcp:localhost:${process.env.LOCAL_PORT}`] } )
	.then( () => {
		...
		wp_cli( `core ${ installCommand } --title="waggypuppy" --admin_user=admin --admin_password=password --admin_email=test@test.com --skip-email --url=http://localhost:${process.env.LOCAL_PORT}` );
	} );
 ...
 */
function wp_cli( cmd ) {
	execSync( `docker compose exec php wp --allow-root ${cmd}`, { stdio: 'inherit' } );
}
